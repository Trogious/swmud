...
char* format_char_attribute1(CHAR_DATA * ch, int przypadek);
...
const char *inf_name( int i )
{
	switch( i )
	{
		default:	return "Mianownik (kto/co):" PLAIN "          ";
		case 1:		return "Dope³niacz (kogo/czego):" PLAIN "     ";
		case 2:		return "Celownik (komu/czemu):" PLAIN "       ";
		case 3:		return "Biernik (kogo/co):" PLAIN "           ";
		case 4:		return "Narzêdnik (z kim/z czym):" PLAIN "    ";
		case 5:		return "Miejscownik (o kim/o czym):" PLAIN "  ";
	}
}
...
void nanny_confirm_suicide( DESCRIPTOR_DATA *d, char *argument )
{
	...
	act( FG_RED,
	"Z ciê¿k± determinacj± i ogromnym smutkiem podcinasz sobie ¿y³y...",  ch, NULL, NULL, TO_CHAR    );
	act( FG_RED,
	"Zimny dreszcz przechodzi twoje plecy widz±c, jak $n podcina sobie ¿y³y!",  ch, NULL, NULL, TO_ROOM );
...
void nanny_get_name( DESCRIPTOR_DATA *d, char *argument )
{
	...
	||   !str_cmp( argument, "Nowy" )
	||   !str_cmp( argument, "Nowe" )
	||   !str_cmp( argument, "Nowa" ) )
	{
		if (d->newstate == 0)
		{
		/* New player */
	...
	if ( !d->character )
	{
		...
		log_string( log_buf );
		write_to_buffer( d,
		"Twoj plik zostal uszkodzony..." NL
	...
	if ( IS_SET(ch->act, PLR_DENY) )
	{
		...
		{
			write_to_buffer( d,
			"Taki gracz juz u nas istnieje." NL
			"Wybierz inne imie: ", 0 );
			d->connected = CON_GET_NAME;
	...
	else
	{
		...
		{
			write_to_buffer( d,
			"Taki gracz nie istnieje na tym mudzie." NL
...
void nanny_get_new_colour( DESCRIPTOR_DATA *d, char *argument )
{
	...
	send_to_char( NL "Oto lista dostepnych standardow kodowania polskich znakow:" NL NL, d->character );
...
void nanny_get_char_set( DESCRIPTOR_DATA *d, char *argument )
{
	...
	{
	case 'n': case 'N':
		ch->charset = CHARSET_NOPOL; break;
	case 'i': case 'I':
		ch->charset = CHARSET_ISO; break;
	case 'w': case 'W':
		ch->charset = CHARSET_WIN; break;
		default:
			send_to_char( NL "Oto lista dostepnych standardow kodowania polskich znakow:" NL NL, d->character );
			write_to_buffer( d, "[I]SO       - polskie znaki w standardzie ISO-8859-2. (UNIX)" NL, 0 );
			write_to_buffer( d, "[W]IN       - polskie znaki w standardzie CP-1250. (Windows)" NL, 0 );
...
void nanny_wprowadzenie( DESCRIPTOR_DATA *d, char *argument )
{
	...
	FOREACH( race, first_race )
	{
		...
		i++;
		sprintf( buf,
		"[" FB_WHITE "%2d" PLAIN "] " FB_BLUE "%-20s" PLAIN "%s",
...
void nanny_old_pass_to_change( DESCRIPTOR_DATA *d, char *argument )
{
	...
	if ( !check_password( argument, ch->pcdata->pwd ) )
	{
		...
		d->connected = CON_PLAYING;
		write_to_buffer( d, echo_on_str, 0 );
		sprintf( log_buf, "%s!%s@%s bad password to change.",
...
void nanny_new_pass_to_change( DESCRIPTOR_DATA *d, char *argument )
{
	...
	{
		sprintf( buf, "Has³o musi mieæ przynajmniej %d znaków." NL "Has³o: ", MIN_PASS_LEN );
		write_to_buffer( d, buf, 0 );
	return;
	}
...
void nanny_confirm_new_pass_to_change( DESCRIPTOR_DATA *d, char *argument )
{
	...
	d->connected = CON_PLAYING;
	write_to_buffer( d,
	"Ok." NL "Has³o zosta³o zmienione." NL,	0 );
	sprintf( log_buf, "%s!%s@%s changing password.",
	ch->name, d->user, d->host );
	log_string( log_buf );
	save_char_obj( ch );
}
...
//	do_help( ch, (char *)(ch->race->name) );
	...
	ch_printf( ch, EOL FG_BLUE "Czy na pewno chcesz byæ " FB_WHITE "%s" FG_BLUE "? (" FB_BLUE "T" FG_BLUE "/" FB_BLUE "N" FG_BLUE "):" PLAIN " ",
...
void nanny_confirm_new_race( DESCRIPTOR_DATA *d, char *argument )
{
	...
	send_to_char( EOL FG_GREEN "Wybierz p³eæ:" EOL NL
	"[" FB_WHITE "1" PLAIN "] "
...
void nanny_get_new_class( DESCRIPTOR_DATA *d, char *argument )
{
	...
	for ( iClass = 0; iClass < MAX_ABILITY; iClass++ )
	{
		...
		|| (toupper(arg[0]) == toupper(class_table[iClass].przypadki[0][0])
	...
	||  !class_table[iClass].przypadki[0]
	...
	ch_printf( ch, NL FG_BLUE "Czy napewno chcesz by to by³a twoja zdolno¶æ podstawowa? (&BT&b/&BN&b):&w ", 0 );
...
void nanny_get_dopelniacz( DESCRIPTOR_DATA *d, char *argument )
{
	...
	send_to_char( PLAIN, ch );
	if( ( argument[0] == '\0' )
	|| ( UPPER( argument[0] ) != UPPER( ch->name[0] ) ) )
	{
...
void nanny_check_odmiane( DESCRIPTOR_DATA *d, char *argument )
{
	...
	ch_printf( ch, NL NL "      " FB_BLUE
		...
		"M±dro¶æ:     " FB_WHITE "%2d" EOL,
		ch->perm_cha, ch->perm_int, ch->perm_wis);
	ch_printf( ch,"      " FB_BLUE
		"Zrêczno¶æ:   " FB_WHITE "%2d     " FB_BLUE
		"Kondycja:    " FB_WHITE "%2d     " FB_BLUE
		"Si³a:        " FB_WHITE "%2d" EOL NL,
		ch->perm_dex, ch->perm_con, ch->perm_str) ;
...
void show_look_groups(CHAR_DATA * ch)
{
	...
	{
		ch_printf(ch, FG_GREEN " Jeste¶ rozpoznawan%s jako : " FG_YELLOW, MALE(ch) ? "y" : "a");
		ch_printf(ch, format_char_attribute1(ch,0));
		ch_printf(ch, EOL);
	}
...
void show_look_traits(CHAR_DATA * ch,int index)
{
	...
	if ( ch->attribute1 >= 0  || ch->attribute2 >= 0 )
	{
		ch_printf(ch, NL FG_GREEN " Jeste¶ rozpoznawan%s jako : " FG_YELLOW, MALE(ch) ? "y" : "a");
		ch_printf(ch,format_char_attribute1(ch,0));
		ch_printf(ch,EOL NL);
	}
	ch_printf(ch,FG_GREEN " Cechy wygl±du zebrane w kategorii " FG_YELLOW "%s." EOL NL,category_desc[descGroup]);
	...
	for(descItem=0,splitCounter=0;descItem<61;descItem++)
	{
		...
		splitCounter++;
		sprintf(buf,FG_YELLOW "%s" PLAIN, char_attribute((desc_type)descGroup,descItem,ch->sex,0));
		if (splitCounter==1)
			sprintf(buf,FG_YELLOW " %s" PLAIN, char_attribute((desc_type)descGroup,descItem,ch->sex,0));
		count= 26 - strlen(buf);
		if (splitCounter==1)
...
void nanny_stats_ok( DESCRIPTOR_DATA *d, char *argument )
{
	...
	ch_printf(ch, CLRSCR NL FB_GREEN "*" FG_GREEN " OKRE¦LENIE WYGL¡DU " FB_GREEN "*" EOL NL
...
void nanny_show_look_trait( DESCRIPTOR_DATA *d, char *argument )
{
	...
	if (ch->attribute1 >=0 && ch->attribute2 >=0)
	{
		...
		ch_printf(ch, CLRSCR NL FB_GREEN "*" FG_GREEN " USTALENIE HAS£A " FB_GREEN "*" EOL NL
		...
		FB_PINK "jest " FB_WHITE "%s" FB_PINK ". Jeste¶ %s postrzegan%s jako " FB_WHITE "%s." EOL,
			...
			MALE( ch ) ? "yn" : "±" ,
			format_char_attribute1(ch,0));
		send_to_char( EOL FG_GREEN "Ostatni± rzecz±, jak± musisz wykonaæ jest wymy¶lenie trudnego do odgadniêcia" EOL
		...
		sprintf( buf, "Podaj has³o dla postaci %s: %s",
...
void nanny_confirm_choice( DESCRIPTOR_DATA *d, char *argument )
{
	...
	else
	{
		...
		for(descItem=0;descItem<61;descItem++)
		{
			...
			if (is_name_prefix(arg,char_attribute((desc_type)descGroup,descItem,ch->sex,0)))
			{
        ...
        {
			send_to_char(FG_GREEN "Ju¿ masz tak± cechê - wybierz inn± jako drug±" EOL,ch);
			return;
        }
		if ( descGroup == FUR)
		{
        ...
        else if ( ch->attribute2 == markIndex )
        {
			...
			ch->attribute2 = markIndex;
		else ch->attribute1=markIndex;

		if ( ( ch->attribute1 <0 && ch->attribute1 != INT_MIN)
	      || ( ch->attribute2 <0 && ch->attribute2 != INT_MIN) )
		{
			ch_printf(ch, NL FG_GREEN " Czy chcesz byæ rozpoznawan%s jako : " FG_YELLOW , MALE(ch) ? "y" : "a");
			ch_printf(ch, format_char_attribute1(ch,0));
			ch_printf(ch, FG_GREEN "? (" FB_GREEN "T" FG_GREEN "/" FB_GREEN "N" FG_GREEN "):");
		}
	...
	switch ( argument[0] )
	{
		case 't': case 'T':
			if (ch->attribute1 <0 || ch->attribute2 <0)
			{
				...
				if (ch->attribute2 <0 && ch->attribute2 != INT_MIN)
	      ...
	      || ( ch->attribute2 <0 && ch->attribute2 != INT_MIN) )
		{
			ch_printf(ch, NL FG_GREEN " Czy chcesz byæ rozpoznawan%s jako : " FG_YELLOW, MALE(ch) ? "y" : "a");
			ch_printf(ch, format_char_attribute1(ch,0));
			ch_printf(ch, EOL FG_BLUE " Odpowiedz tylko " FB_BLUE "T" FG_BLUE "/" FB_BLUE "N" FG_BLUE "" PLAIN,
				CH_RACE( ch, 4));  // Pixel: przypadkiF
		}
		d->connected = CON_CONFIRM_ALL_LOOK_TRAIT_CHOICE;
	}
...
// Thanos -- zmieniam na:
		...
		for( crime = ch->first_crime; crime; crime = crime->next )
		{
			if( crime->jail_vnum && crime->jail_end )
			{
				...
				if( ( cell = get_room_index( crime->jail_vnum ) ) == NULL )
				{
					...
					char_to_room( ch,
					get_room_index( ROOM_VNUM_DEFAULT_CELL ) );
				}
				else
	...
	act( COL_ACTION, "$n $T do gry.", ch, NULL,
		MALE( ch )   ? "wszed³" :
		FEMALE( ch ) ? "wesz³a" : "wesz³o", TO_ROOM );

	sprintf( log_buf, "%s!%s@%s has entered the game.",
		ch->name, d->user,  d->host );
...
void rename_done( DESCRIPTOR_DATA *d, bool accept )
{
	...
	else
	{
		...
		if( (victim=get_char_world( ch, d->tmp_ch->name ))==NULL
		||  IS_NPC( victim ) )
...
void process_rename( DESCRIPTOR_DATA *d, char *argument )
{
	...
	{
		if( *arg )
			STRDUP( d->tmp_ch->przypadki[d->tmp_ch->top_level],
				capitalize( arg ) );
		d->tmp_ch->top_level++;
	}
...
char* format_char_attribute1(CHAR_DATA * ch, int przypadek)
{
	...
	else if (ch->attribute1 >= 0) attribute1=ch->attribute1;
	if (ch->attribute2!= INT_MIN && ch->attribute2<0)
		attribute2 = -(ch->attribute2+1);
	else if (ch->attribute2 >= 0) attribute2=ch->attribute2;

	if	(attribute1>=0)
		strcpy(buf,char_attribute((desc_type)(attribute1==0?0:attribute1/100),attribute1%100,ch->sex,przypadek));

	if	(attribute2>=0 )
	{
		if ( desc_table[attribute2/100][attribute2%100].type == FUR )
			sprintf(buf, "%s %s z %s futrem",buf, CH_RACE( ch, przypadek),  // Pixel: przypadkiF
				char_attribute((desc_type)(attribute2==0?0:attribute2/100),attribute2%100,ch->sex,przypadek));
		else
