...
void show_room_resets( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoom )
{
	...
	case 'E':
		...
		FG_CYAN "[%5lld]%-20.20s" PLAIN " %s[%-8.8s]" FB_RED "[%5d]%-20.20s" EOL,
			pReset->command == 'G' ? "[Giv]:" : "[Equ]:",
			pReset->arg1, oname,
		( mob && mob->pShop )	? "[sho]" : "	 ",
		(pReset->command == 'G' ? "carry" : bit_name(wear_types_list, pReset->arg3) ),
		( mob ? mob->vnum : -1),
		mname);
	...
	case 'P':
		...
		if ( !(obj2 = get_obj_index(reset->arg1)) )
	...
	case 'T':
			sprintf(pbuf, FG_GREEN "[TRAP]" PLAIN " %5d %5lld %5lld %5lld (%s)" NL, pReset->extra, pReset->arg1,
				pReset->arg2, pReset->arg3, flag_string( trap_flags_list, pReset->extra));
			break;
	...
	case 'H':
		...
		"[Hide] [" FB_WHITE "%5lld" PLAIN "]" FG_CYAN "%-20.20s" EOL,
				(pReset->arg1 > 0 ? pReset->arg1 : obj ? (int64)obj->vnum : 0), rname);
			break;
	...
	case 'B':
		...
		switch(pReset->arg2 & BIT_RESET_TYPE_MASK)
			{
				...
				if ( !(room = get_room_index(pReset->arg1)) )
					...
					rname = room->name;
				door =
			(pReset->arg2 & BIT_RESET_DOOR_MASK) >> BIT_RESET_DOOR_THRESHOLD;
		...
		case BIT_RESET_OBJECT:
			...
			"Object " PLAIN "[" FB_WHITE "%5lld" PLAIN "]" FG_CYAN "%-20.20s",
				(pReset->arg1 > 0 ? pReset->arg1 : obj ? (int64)obj->vnum : 0),
			rname );
		...
		case BIT_RESET_MOBILE:
			...
			"Mobile " PLAIN "[" FB_WHITE "%5lld" PLAIN "]" FG_CYAN "%-20.20s",
					(pReset->arg1 > 0 ? pReset->arg1 : mob ? (int64)mob->vnum : 0),
			rname);
				flagarray = area_flags_list;
				break;
			default:
				sprintf(pbuf, "bad type %lld", pReset->arg2 & BIT_RESET_TYPE_MASK);
				flagarray = NULL;
				break;
			}
			pbuf += strlen(pbuf);
	...
	case 'R':
		...
		sprintf(pbuf, "[Rand] exits 0 to %lld" FB_WHITE "->" PLAIN "%s (%lld)" NL, pReset->arg2,
			rname, pReset->arg1);
			break;
	}
  ...
  for ( pReset = pRoom->first_reset; pReset; pReset = pReset->next )
  {
	...
	case 'M':
		...
		if ( !(pRoomIndex = find_room( pRoom, pReset->arg3) ) )
			{
				bug( "'M': bad room vnum %d. (%s)",
				pReset->arg3, pArea->filename  );
			continue;
			}
		...
		if( !pRoomIndex->ship || !pRoomIndex->ship->vnum )
		{
			...
			if ( IS_SET( pMobIndex->act, ACT_SENTINEL ) )
			{
					...
					if( pRoomIndex->area )
						...
						for ( ch = pRoomIndex->first_person; ch; ch = ch->next_in_room )
	...
	case 'O':
		...
		if ( !(pObjIndex = get_obj_index(pReset->arg1)) )
			...
			if ( !(pRoomIndex = find_room( pRoom, pReset->arg3)) )
			{
				bug( "'O': bad room vnum %d. (%s)",
			pReset->arg3, pArea->filename  );
			continue;
			}

			pObj=NULL;
			/* Tanglor - to takie uproszecznie dal limitu obiektów
			 * count_obj_list jakos nie radzi sobie z tym limitem
			 * */
			for( pObj=pRoomIndex->first_content; pObj; pObj=pObj->next_content )
				if( pObj->pIndexData==pObjIndex && pObj->count >= pReset->arg2 )
					break;
			if (pObj)
			{
				LastObj = pObj;
		...
		/* Bugfix by Thanos: je¶li obj jest ju¿ w pokoju, reset nie
		   ³aduje go ju¿. Niby ok, ale powinni¶my zapamiêtaæ go jako
	...
	case 'P':
		...
		pObj = create_object(pObjIndex,
		number_fuzzy(UMAX(generate_itemlevel(pArea, pObjIndex),to_obj->level)));
			pObj->level	= UMIN(pObj->level, LEVEL_AVATAR);
	...
	case 'B':
		...
		default:
			{
			bug( "'B': bad options %d. (%s)",
			pReset->arg2, pArea->filename  );
			}
		continue;
			}
	...
	/* Costruct ship in room  -- added by Thanos */
		...
		bool		loop;
			...
			if( !(shrec = get_ship_index(itoa(pReset->arg1))) )
		{
			bug( "'C': bad ship vnum %d. (%s)",
			pReset->arg1, pArea->filename  );
				continue;
			}

		if( pRoom->ship
		&&  pRoom->ship->pIndexData
		 ...
		 * £adna pêtelka i trudna do wykrycia (bo przecie¿ mo¿na zaanga¿owaæ
		 * do niej jeszcze statek C), dlatego pozwólmy na zagnie¿dzenie do,
		 * powiedzmy, 3.
		 */
		count=0; loop=false;
...
DEF_DO_FUN( reset )
{
	...
	ch_printf( ch, "		reset object <obj#> equip <mob name> <location> [limit]" NL );
	...
	if ( !str_cmp( arg3, "put" ) )
	{
		...
		pReset = make_reset('P', reset->extra+1, pObj->vnum, vnum, reset->arg1);
	...
	if ( !str_cmp( arg3, "equip" ) )
	{
		...
		if ( pReset->command == 'E' && pReset->arg3 == num )
			{
				send_to_char( "Mobile already has an item equipped there." NL, ch);
				return;
			}
			}
...
RID *validate_reset( AREA_DATA *area, RESET_DATA *reset, int64 *iLastRoom, int64 *iLastObj )
{
	RID	*pRoomIndex = NULL;
	ED	*pexit;
...
DEF_DO_FUN( findresets )
{
	...
	if (!str_prefix(arg1, "mobile"))
	{
		...
		{
			send_to_char("You can't search for mobile that you can't edit." NL, ch);
			return;
		}
	...
	else if (!str_prefix(arg1, "object"))
	{
		...
		if (!is_builder( ch, get_vnum_area(vnum)))
		{
			send_to_char("You can't search for object that you can't edit." NL, ch);
			return;
		}
	}
	else
	...
	if (!strcmp(arg3, "world"))
	{
		...
		{
			if (!is_builder(ch, area)) continue;
		...
		{
			send_to_char("You can't search resets in area that you can't edit." NL, ch);
			return;
		}
	}
	...
	//Display results
	pager_printf(ch, FB_CYAN "[" FB_YELLOW "%-6s" FB_CYAN "][" FB_YELLOW "%-44s" FB_CYAN "][" FB_YELLOW "%-30s" FB_CYAN "]" NL,
			"Vnum", "Room name", "Area name");
	for (it = results.begin(); it != results.end(); ++it)
	{
		pager_printf(ch, FB_CYAN "[" PLAIN "%6d" FB_CYAN "][" PLAIN "%-44s" FB_CYAN "][" PLAIN "%-30s" FB_CYAN "]" NL,
