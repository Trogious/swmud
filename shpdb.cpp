...
DEF_DO_FUN( shlist )
{
	...
	sprintf( buf, 
	    FB_CYAN "[" PLAIN "%4d" FB_CYAN "][" PLAIN "%-24s" FB_CYAN "]"
		   "[" PLAIN "%5d" FB_CYAN "][" PLAIN "%3d" FB_CYAN "]",
		shrec->vnum, strip_colors( shrec->name, 24 ), 
		shrec->roomcount, shrec->security );
	strcat( buf1, buf );
...
void fread_ship_rooms( SHIP_INDEX_DATA *shrec, FILE *fp )
{
	...
	ln = fread_line( fp );	x1=x2=x3=x4=x5=x6=0;
    	...
    	for ( ; ; )
	{
		...
		if ( door < 0 || door > 10 )
		{
		    bug( "vnum %d has bad door number %d.", 
			vnum, door );
		    continue;
		}
		else
		{
		    ...
		    ln = fread_line( fp );     x1=x2=x3=x4=0;
		    ...
		    switch ( locks )
		    {
			case 1: pexit->flags = EX_ISDOOR;
				break;
			case 2: pexit->flags = EX_ISDOOR | EX_PICKPROOF; 
				break;
			default:pexit->flags = locks;
		    }
		  
		    LINK( pexit, sRoom->first_exit, sRoom->last_exit, 
			next, prev );
		}
	    }
	    else if ( letter == 'E' )
		...
		LINK( ed, sRoom->first_extradesc, sRoom->last_extradesc,
			  next, prev );
	    }
	    else if ( letter == '>' )
	    {
...
void fread_ship_resets( SHIP_INDEX_DATA *shrec, FILE *fp )
{
	...
	case 'C':
	    ...
	    {
	    	bug( "%s (%d) 'C': room %d doesn't exist.",
	...
	case 'M':
	    if ( (sRoom = get_sroom_ind( shrec, arg3 )) == NULL )
	    {
	    	bug( "%s (%d) 'M': room %d doesn't exist.",
		    shrec->filename, count, arg3 );
	        bad=true;
	    }
	    else
	...
	case 'O':
	    ...
	    {
		bug( "%s (%d) '%c': room %d doesn't exist.",
		    shrec->filename, count, letter, arg3 );
	        bad=true;
	    }
	    else
		iLastObj = arg3;
...
void save_ship_index( SHIP_INDEX_DATA *ship )
{
	...
	for( hangar = ship->first_hangar; hangar ; hangar = hangar->next )
		...
		hangar->vnum, hangar->capacity, hangar->status, hangar->type );
...
*/	fprintf( fp, "MaxCargo     %d\n",       ship->maxcargo          );
	...
	for( sRoom = ship->first_room; sRoom; sRoom = sRoom->next )
	{
		...
		sRoom->light,		sRoom->room_flags,	
		sRoom->sector_type,	sRoom->tele_delay,	
