...
void deal_with_crash( )
{
	...
	rsn = true;
	sprintf( bbuf,
	    "Bardzo panÛw AdminÛw przepraszam, øe pad≥em, ale" 	NL
    ...
    {
	sprintf( bbuf,
	    "Panowie. Sprawa jest taka, øe pad≥em... :("		NL
...
#define FNDELAY O_NDELAY
	...
	sprintf( log_buf, "[%d] sock.sinaddr: %s, port %hd.",
	...
	for ( pban = first_ban; pban; pban = pban->next )
	{
			...
			&&  pban->level >= LEVEL_SUPREME)
		{
			...
			"TwÛj adres jest zbanowany." NL, 0 );
			sprintf( buf, "Rejected connection from: %s due to ban: %s",
...
void close_socket( DESCRIPTOR_DATA *dclose, bool force )
{
	...
	write_to_buffer( dclose->snoop_by,
	    "Twoja ofiara wysz≥a z gry." NL, 0 );
	...
	if ( !dclose->prev )
	{
	    bug( "%s desc:%p could not be found!.",
	...
	bug( "%s desc:%p != last_desc:%p and desc->next = NULL!",
		ch ? ch->name : d->host, dclose, last_descriptor /* ? last_descriptor : 0 */ );
	dn = NULL;
	...
	{
	    act( COL_ACTION, "$n straci≥$o link do muda.", ch, NULL, NULL, TO_ROOM );
	    ch->desc = NULL;
	}
	else
	{
...
(char)0x8a, (char)0x8b, (char)0xa6, (char)0x8d, (char)0x8e, (char)0xac,
(char)0x90, (char)0x91, (char)0x92, (char)0x93, (char)0x94, (char)0x95,
(char)0x96, (char)0x97, (char)0x98, (char)0x99, (char)0x9a, (char)0x9b,
(char)0xb6, (char)0x9d, (char)0x93, (char)0xbc, (char)0xa0, (char)0xa1,
(char)0xa2, (char)0xa3, (char)0xa4, (char)0xa1, (char)0xa6, (char)0xa7,
(char)0xa8, (char)0xa9, (char)0xaa, (char)0xab, (char)0xac, (char)0xad,
(char)0xae, (char)0xaf, (char)0xb0, (char)0xb1, (char)0xb2, (char)0xb3,
(char)0xb4, (char)0xb5, (char)0xb6, (char)0xb7, (char)0xb8, (char)0xb1,
...
(char)0xe4, (char)0xe5, (char)0xe6, (char)0xe7, (char)0x68, (char)0xe9,
...
void read_from_buffer( DESCRIPTOR_DATA *d )
{
	...
	sprintf( buf, "Con: %d, Pos: %d, Cmd: %s",
	    d->connected, CH( d )->position, d->inbuf );
...
void display_map( DESCRIPTOR_DATA *d )
{
    ...
    buf[0] ='\0';
    sprintf( buf1, " %s", check_exit( ch, 7 )); strcat( buf, buf1 );
    sprintf( buf1, " %s", check_exit( ch, 0 )); strcat( buf, buf1 );
    sprintf( buf1, " %s", check_exit( ch, 6 )); strcat( buf, buf1 );
    sprintf( buf1, "    %s %s", check_exit( ch, 4 ), buf2); strcat( buf, buf1 );
    sprintf( buf1, "%s  " FB_WHITE "::" PLAIN, check_exit( ch, 3 )); strcat( buf, buf1 );
    sprintf( buf1, "  %s      |" NL, check_exit( ch, 1 )); strcat( buf, buf1 );
    sprintf( buf1, " %s", check_exit( ch, 9 )); strcat( buf, buf1 );
    sprintf( buf1, " %s", check_exit( ch, 2 )); strcat( buf, buf1 );
    sprintf( buf1, " %s", check_exit( ch, 8 )); strcat( buf, buf1 );
    sprintf( buf1, "    %s | " PLAIN, check_exit( ch, 5)); strcat( buf, buf1 );
    ch_printf( ch, buf );
    return;
}
//done by Thanos
...
bool flush_buffer( DESCRIPTOR_DATA *d, bool fPrompt )
{
	...
	ch = d->original ? d->original : d->character;
	    ...
	    {
		if ( IS_SET( ch->act, PLR_PROMPT )
		|| ch->pcdata->status_type == STATUS_TYPE_TWO )
		{
		    sprintf( buf, "\e[%d;1H", ch->pcdata->status_height+1 );
		    write_to_buffer( d, buf, 0 );
...
//Added by Ratm
		...
		act( COL_ACTION, "$n po≥±czy≥$o siÍ na nowo.", ch, NULL, NULL, TO_ROOM );
...
bool check_multi( DESCRIPTOR_DATA *d , char *name )
{
	...
	)
	{
		...
		sprintf( log_buf, "%s attempting to multiplay with %s.",
		    dold->original ? dold->original->name :
		    dold->character->name , d->character->name );
		log_string_plus( log_buf, LOG_COMM, sysdata.log_level );
	        d->character = NULL;
	        free_char( d->character );
	        return true;
...
//Added by Ratm
	    ...
	    ch->switched = NULL;
	    send_to_char( "PowtÛrne po≥±czenie." NL, ch );
	    act( COL_ACTION, "$n po≥±czy≥$o siÍ na nowo.",
 ...
 */
/* Major overhaul. -- Alty */
char *act_string(const char *color, const char *format, CHAR_DATA *to, CHAR_DATA *ch,
		 const void *arg1, const void *arg2)
{
    //added by Thanos (polskie zaimki)
    static const char * const  he_she      [ ] = { "to"  , "on"   , "ona"  };// E
    static const char * const  him_her     [ ] = { "temu", "mu" , "jej"    };// M
    static const char * const  his_her     [ ] = { "tego", "jego" , "jej"  };// S
    static const char * const  in_him_her  [ ] = { "tym" , "nim"  , "niej" };// G
    static const char * const  at_him_her  [ ] = { "nie" , "niego", "ni±"  };// R
    static const char * const  with_him_her[ ] = { "tym" , "nim"  , "ni±"  };// W
    static const char * const  from_him_her[ ] = { "tego", "niego", "niej" };// F
    static const char * const  i_see_him_her[ ]= { "to"  , "go"   , "j±"   };// I
    static const char * const  soc 	     [ ] = { "o"   , ""     , "a"    };// O
    static const char * const  socy	     [ ] = { "e"   , "y"    , "a"    };// Y
    static const char * const  socp 	     [ ] = { "e"   , ""     , "a"    };// X
    static const char * const  socr 	     [ ] = { "o"   , "e"    , "a"    };// A
    static const char * const  socz 	     [ ] = { "ie"   ,"i"    , "a"    };// Z
    //done
  ...
  while ( *str != '\0' )
  {
    ...
    case 'n': if(str[1]=='$')
		  ...
		  else i =(to ?  PERS( ch,  to, 0 ): NAME( ch, 0 ) );	break;
	...
	case 'N': if(str[1]=='$')
		  ...
		  else i =(to ?  PERS( vch,  to, 0 ): NAME( vch, 0 ) );	break;
//done
      ...
      case 'S': if (vch->sex > 2 || vch->sex < 0)
		...
		break;
      case 'q': i = (to == ch) ? "" : "";				break;
      case 'Q': i = (to == ch) ? "twoje" :
		    his_her[URANGE(0,  ch->sex, 2)];			break;
    //Added by Thanos (zaimki)
    case 'g': i = in_him_her 	[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'G': i = in_him_her 	[URANGE( 0, vch ->sex, 2 )];	break;
    case 'r': i = at_him_her 	[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'R': i = at_him_her 	[URANGE( 0, vch ->sex, 2 )];	break;
    case 'w': i = with_him_her 	[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'W': i = with_him_her 	[URANGE( 0, vch ->sex, 2 )];	break;
    case 'f': i = from_him_her 	[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'F': i = from_him_her 	[URANGE( 0, vch ->sex, 2 )];	break;
    case 'i': i = i_see_him_her [URANGE( 0, ch  ->sex, 2 )];	break;
    case 'I': i = i_see_him_her [URANGE( 0, vch ->sex, 2 )];	break;
    case 'o': i = soc  		[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'O': i = soc  		[URANGE( 0, vch ->sex, 2 )];	break;
    case 'y': i = socy 		[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'Y': i = socy 		[URANGE( 0, vch ->sex, 2 )];	break;
    case 'x': i = socp 		[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'X': i = socp 		[URANGE( 0, vch ->sex, 2 )];	break;
    case 'a': i = socr 		[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'A': i = socr	 	[URANGE( 0, vch ->sex, 2 )];	break;
    case 'z': i = socz 		[URANGE( 0, ch  ->sex, 2 )];	break;
    case 'Z': i = socz	 	[URANGE( 0, vch ->sex, 2 )];	break;
...
*/
			...
			stat = ch->gold;			break;
	    	    ...
	    	    case 'b':
			stat = (ch->pcdata ? ch->pcdata->bank : 0 ); break;
		...
		/*added by Thanos (krainka w prompcie) */
			...
			?  ch->in_room->area->planet->name : "..."): "%P" ) );
			break;

		    /* added by Thanos (piÍtrowe prompty) */
    		    case 'n':
			sprintf(pbuf, EOL PROMPT_COL_N );	break;

		    case 'o': /*(OLC)*/
			sprintf(pbuf, "%s", olc_ed_name( ch ) );break;

    		    case 'O': /*(OLC)*/
			sprintf(pbuf, "%s", olc_ed_vnum( ch ) );break;
...
int make_color_sequence(const char *col, char *buf, DESCRIPTOR_DATA *d)
{
  ...
  if ( *ctype != __FGMOD[0] && *ctype != __BGMOD[0] )
  {
...
*/
    ...
    if( d->character->charset == CHARSET_ISO )
	sprintf( buf,
	...
	if ( cl == 7 )
	    ...
	    sprintf(buf, "\033[0m\033[%d;%s%dm",
	    (cl & 8) == 8 ,		/*bold?*/
	    cl > 15 ? "5;"  : "", 	/*blink?*/
	    (cl & 7)+30 );		/*colour*/
        ret = write_to_descriptor( d->descriptor, buf, 0 );
    }
    else ret = write_to_descriptor( d->descriptor, "\033[0m", 0 );
