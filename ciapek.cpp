...
DEF_DO_FUN( dump )
{
	...
	if ((pMobIndex = get_mob_index(vnum)) != NULL)
	{
		...
		pMobIndex->killed,pMobIndex->przypadki[0]);
	}
    fclose(fp);
...
bool save_bus_list()
{
    ...
    sprintf( buf, "Save_bus_list: %d ship%s written do %s",
	i,
	i == 1 ? "" : "s",
	BUS_LIST );
...
void showcourse( CHAR_DATA *ch, SHIP_DATA *ship )
{
    ...
    {
	ch_printf( ch, "%-2d]  %-20s" PLAIN "-%-8d (%s" PLAIN ")" EOL,
	    i, course->stop_name,
	    course->stop_vnum,
...
DEF_DO_FUN( setcourse )
{
    ...
    if ( !str_cmp( arg2, "edit" ) )//edytujemy istniej±c± stacjê
    {
	int 	nr;
...
bool check_invis( CHAR_DATA *ch, LAST_DATA *last )
{
	...
	if ( IS_AFFECTED(ch, AFF_BLIND) )
	  return false;
        ...
        if ( IS_RACE( ch, "DEFEL" )
	&& IS_SET( last->invis_type, INVIS_DEFEL ) )
          return true;

	if ( IS_SET( last->invis_type, INVIS_INVIS )
	&&  !IS_AFFECTED(ch, AFF_DETECT_INVIS) )
	  return false;

	if ( IS_SET( last->invis_type, INVIS_HIDE )
...
//    sprintf( sbuf, last->text );
    ...
    {
	int speakswell =
	    check_lang(ch, last->language );

	if ( speakswell < 85 )
	{
	/*    sprintf( sbuf, */
	    strcpy( sbuf, translate(   speakswell,
				last->text,
				last->language->name ) );
	}
    }
...
DEF_DO_FUN( last )
{
    ...
    else if( !str_prefix( argument, "olctalk" ) && ( IS_OLCMAN( ch )
	          || IS_ADMIN( ch->name ) || get_trust( ch ) > 102 ) )
        channel = CHANNEL_OLCTALK;
    else if( !str_prefix( argument, "chat"  ) )	channel = CHANNEL_CHAT;
    else if( !str_prefix( argument, "tells" ) )	channel = CHANNEL_TELLS;
    else if((!str_prefix( argument, "clantalk") || !str_prefix( argument, "ctalk" ))
	...
	/*by Trog*/	channel == CHANNEL_CODERTALK 	? "na codertalku" 	:
						  "do ciebie"            );
    ...
    if(  channel == CHANNEL_TELLS )
    {
	    ...
	    sprintf( buf1,
		"%s %s" PLAIN ": %s" NL,	last->time,
		check_invis( ch, last ) ?
      does_knows_name(ch, last->teller) ? last->teller : "Kto¶"
          : "Kto¶",
	    ...
	    i < MAX_LAST_CTALK && last; last = last->next, i++ )
	{
	    ...
	    if( is_name( last->teller, ch->pcdata->ignorelist )
		&& !IS_ADMIN( ch->name ) )
		continue;
    ...
    {
    	for( i=0, last = (channel == CHANNEL_ADMINTALK) ? first_last_admin :
			 (channel == CHANNEL_OLCTALK)	? first_last_olctalk :
			 (channel == CHANNEL_CODERTALK)	? first_last_codertalk :
			 (channel == CHANNEL_IMMTALK)   ? first_last_wiztalk :
							  first_last_chat;
	     i < MAX_LAST && last;
	     last = last->next, i++ )
	{
            char *sbuf = ( channel == CHANNEL_CHAT )
...
void set_last_invis( LAST_DATA *last, CHAR_DATA *sender )
{
    ...
    if( IS_NPC(sender) && IS_SET(sender->act, ACT_MOBINVIS) )
...
void add_to_last_tell( CHAR_DATA *ch, CHAR_DATA *sender, const char *linia )
{
	...
	UNLINK( last, 	ch->pcdata->first_last_tell,
			ch->pcdata->last_last_tell, next, prev );
	free_last( last );
    }
    ...
    LINK( last, 	ch->pcdata->first_last_tell,
			ch->pcdata->last_last_tell, next, prev );
    return;
}
...
void add_to_last_ctalk( CLAN_DATA *clan, CHAR_DATA *sender, const char *linia )
{
    ...
    for( i = 0, last = clan->first_last_ctalk;
	 i < MAX_LAST_CTALK && last; last = last->next, i++ )
	if( !last )
	    break;
	...
	UNLINK( last, 	clan->first_last_ctalk,
			clan->last_last_ctalk, next, prev );
	free_last( last );
    }
    ...
    LINK( last, 	clan->first_last_ctalk,
			clan->last_last_ctalk, next, prev );
    return;
}
 ...
 */
void add_to_last_buf( CHAR_DATA *ch, char *tekst, const char *verb, int64 channel )
{
    int			i;
    LAST_DATA	*last;
    ...
    {
	case CHANNEL_IMMTALK :
		LINK( last, 	first_last_wiztalk,
				last_last_wiztalk, next, prev );
	    break;
	case CHANNEL_ADMINTALK:
		LINK( last, 	first_last_admin,
				last_last_admin, next, prev );
	    break;
	case CHANNEL_CODERTALK:
		LINK( last, 	first_last_codertalk,
				last_last_codertalk, next, prev );
	    break;
	case CHANNEL_OLCTALK:
		LINK( last,     first_last_olctalk,
                last_last_olctalk, next, prev );
		break;
	case CHANNEL_CHAT:
	case CHANNEL_OOC:
		LINK( last, 	first_last_chat,
				last_last_chat, next, prev );
	    break;
	default:
...
DEF_DO_FUN( prefi )
{
    send_to_char( "Je¶li chcesz u¿yæ prefixu, wpisz komendê 'prefix' w ca³o¶ci." NL, ch );
    return;
}
...
DEF_DO_FUN( alias )
{
	...
	if ( !str_cmp( arg, alias->name ) ) /* redefine an alias */
	{
	    ...
	    ch_printf( rch,  FB_WHITE "%s" PLAIN " zosta³o przedefiniowane na: " FG_CYAN "%s" PLAIN "." NL,
		arg, argument );
	    return;
	}
    }
    ...
    {
	send_to_char( "Przykro mi, ale twój limit aliasów zosta³ przekroczny." NL, ch );
	return;
    }
	...
	"%s" PLAIN " dzia³a od teraz jako: " FG_CYAN "%s" PLAIN "." NL,
	    arg, argument );
    return;
}
...
DEF_DO_FUN( unalias )
{
	...
	if( !str_cmp( arg, alias->name ) )
	{
	    UNLINK( alias, rch->pcdata->first_alias,
			   rch->pcdata->last_alias, next, prev );
	    free_alias( alias );
...
DEF_DO_FUN( forbid )
{
    ...
    {
	send_to_char( "Nie mo¿esz odebraæ komendy w³adcom lub graczom równym rang±.\n\r", ch );
	return;
    }
	...
	{
	    send_to_char( "Mo¿esz znów pos³ugiwaæ siê odebranymi ci wcze¶niej komendami.\n\rU¿ywaj ich z rozwag±.\n\r", victim );
...
void update_status_bar( CHAR_DATA *ch )
{
	...
	if (ch->skill_level[FORCE_ABILITY] > 1 || get_trust(ch) >= LEVEL_IMMORTAL )
	    sprintf( buf, "&W[&O %d&c/&O%d&cHp &O%d&c/&O%d&cFrc &O%d&c/&O%d&cMv &W]&c",
	...
	bufor[ i + strlen( buf ) ] = '-';
//        send_to_char( "&c", ch);
	ch_printf( ch, "\e[%d;1H\e[2K%s\e[%d;1H", st_height, bufor, st_height-1);
    }
...
void init_status_bar( CHAR_DATA *ch )
{
    ...
    ch_printf( ch, "\e[%d;1H%s\e[1;1H%s%s\e[1;%dr%s", st_height+1,
		    VT_SAVECURSOR, VT_SETWIN_CLEAR, VT_CLEAR_SCREEN,
		    st_height-1, VT_RESTORECURSOR );
...
DEF_DO_FUN( stbar )
{
	...
	{
	    send_to_char( "Szeroko¶æ linii statusu musi byæ z przedzia³u: 40 - 160.\n\r", ch );
	    return;
	}
	else
	{
	...
	{
	    send_to_char( "Wysoko¶æ linii statusu musi byæ z przedzia³u: 15 - 60.\n\r", ch );
	    return;
	}
	else
	{
    ...
    if( !str_prefix( buf, "type" ) )	//added by thanos
    {
    	int i;
	i = atoi( argument );
...
{
    int i;
    const char uchars[9] = {'¡','Æ','Ê','£','Ñ','Ó','¦','¬','¯'};
    const char lchars[9] = {'±','æ','ê','³','ñ','ó','¶','¼','¿'};
...
DEF_DO_FUN( iso )
{
    ...
    send_to_char( "Standard kodowania polskich liter zosta³ ustawiony jako: ISO-8859-2\n\r", ch );
    send_to_char( "Test polskich liter: -±-æ-ê-³-ñ-ó-¶-¼-¿- -¡-Æ-Ê-£-Ñ-Ó-¦-¬-¯-\n\r", ch );
    return;
}
...
DEF_DO_FUN( win )
{
    ...
    send_to_char( "Standard kodowania polskich liter zosta³ ustawiony jako: WIN CP-1250\n\r", ch );
    send_to_char( "Test polskich liter: -±-æ-ê-³-ñ-ó-¶-¼-¿- -¡-Æ-Ê-£-Ñ-Ó-¦-¬-¯-\n\r", ch );
    return;
}
...
DEF_DO_FUN( nopol )
{
    ...
    send_to_char( "Standard kodowania polskich liter zosta³ ustawiony jako: NOPOL\n\r", ch );
    send_to_char( "Test polskich liter: -±-æ-ê-³-ñ-ó-¶-¼-¿- -¡-Æ-Ê-£-Ñ-Ó-¦-¬-¯-\n\r", ch );
    return;
}
