...
const char *	const	dir_name	[]		=
{
	...
	"pó³nocny-wschód", "pó³nocny-zachód", "po³udniowy-wschód",
	"po³udniowy-zachód", "gdzie¶"
};

const char *	const	dir_where_name	[]		=
{
	"pó³noc", "wschód", "po³udnie", "zachód", "górê", "dó³",
	"pó³nocny-wschód", "pó³nocny-zachód", "po³udniowy-wschód",
	"po³udniowy-zachód", "gdzie¶"
};
...
{
	"z po³udnia", "z zachodu", "z pó³nocy", "ze wschodu", "z do³u", "z góry",
	"z po³udniowego-zachodu", "z po³udniowego-wschodu", "z pó³nocnego-zachodu",
	"z pó³nocnego-wschodu", "sk±d¶"
};

//Rozdaje kierunków, na potrzebe skilli
const char *	const	dir_type_name	[]		=
{
	"pó³nocny", "wschodni", "po³udniowy", "zachodni", "ku górze", "ku dole",
	"pó³nocno-wschodni", "pó³nocno-zachodni", "po³udniowo-wschodni",
...
{
	{ "Wnêtrze",	"inside"      },{ "Miejska ulica",	"cities"     },
	{ "Otwarte Pole",	"fields"      },{ "W lesie",		"forests"    },
	{ "Wzgórze",	"hills"	      },{ "Na szczycie góry",   "mountains"  },
	{ "W wodzie",	"waters"      },{ "W g³êbokiej wodzie", "waters"     },
	{ "Pod wod±", 	"underwaters" },{ "W powietrzu",	"air"	     },
	{ "Na pustyni",	"deserts"     },{ "Gdzie¶",		"unknown"    },
	{ "Tafla oceanu", 	"ocean floor" },{ "Pod ziemi±", 	"underground"}
};
...
const char *	const		room_sents[SECT_MAX][25]	=
{
	...
	{
	"D³ugie ciemne zaro¶la nie pozwalaj± ci dostrzec co jest dalej.",
	"Jest tu bardzo wiele starych i wielkich drzew przypominaj±cych dêby.",
	...
	{
	"Ziemia jest daleko daleko pod twoimi stopami.",
	"Ma³a chmurka szybuje obok ciebie."
	},

	{
	"Wszêdzie, jak daleko siêgasz wzrokiem, piasek.",
	"Zdaje ci siê, ¿e widzisz oazê gdzie¶ oddali."
	},
...
int wherehome( CHAR_DATA *ch)
{
	...
	if( get_trust(ch) >= LEVEL_IMMORTAL )	return ROOM_START_IMMORTAL;
	if( IS_RACE( ch, "HUMAN" ))			return ROOM_START_HUMAN;
	if( IS_RACE( ch, "WOOKIEE" ))			return ROOM_START_WOOKIEE;
	if( IS_RACE( ch, "RODIAN" ))			return ROOM_START_RODIAN;
	if( IS_RACE( ch, "MON CALAMARI" ))		return ROOM_START_MON_CALAMARIAN;
	if( IS_RACE( ch, "TWI'LEK" ))			return ROOM_START_TWILEK;
	if( IS_RACE( ch, "HUTT" ))				return ROOM_START_HUTT;
	if( IS_RACE( ch, "GAMORREAN" ))		return ROOM_START_GAMORREAN;
	if( IS_RACE( ch, "JAWA" ))				return ROOM_START_JAWA;
	if( IS_RACE( ch, "ADARIAN" ))			return ROOM_START_ADARIAN;
	if( IS_RACE( ch, "EWOK" ))				return ROOM_START_EWOK;
	if( IS_RACE( ch, "VERPINE" ))			return ROOM_START_VERPINE;
	if( IS_RACE( ch, "DEFEL" ))			return ROOM_START_DEFEL;
	if( IS_RACE( ch, "TRANDOSHAN" ))		return ROOM_START_TRANDOSHAN;
	if( IS_RACE( ch, "CHADRA-FAN" ))		return ROOM_START_CHADRA_FAN;
	if( IS_RACE( ch, "QUARREN" ))			return ROOM_START_QUARREN;
	if( IS_RACE( ch, "DUINUOGWUIN" ))		return ROOM_START_DUINUOGWUIN;
	if( IS_RACE( ch, "NOGHRI" ))			return ROOM_START_NOGHRI;
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
	...
	*/
	if ( IS_SET( pexit->flags, EX_WINDOW )
	&&  !IS_SET( pexit->flags, EX_ISDOOR ) )
	{
		send_to_char( "Niestety nie mo¿esz pój¶æ w tym kierunku." NL, ch );
		return rNONE;
	}
	...
	if ( IS_SET(pexit->flags, EX_NOMOB)
	&& IS_NPC(ch) )
	{
		...
		||   IS_SET(pexit->flags, EX_NOPASSDOOR)) )
	{
		...
		&&   !IS_SET( pexit->flags, EX_DIG ) )
		{
			if ( drunk )
			{
				act( PLAIN, "$n wali w $d z ca³ej si³y, ale nie mo¿e ich przenikn±æ.", ch,
					NULL, pexit->keyword, TO_ROOM );
				act( PLAIN, "Walisz w $d z ca³ej si³y, ale nie mo¿esz ich przenikn±æ.", ch,
...
/* Ale narazie niech zostanie tak: */
	...
	if ( room_is_private( ch, to_room ) && !is_invited( ch, to_room )
			&& !IS_ADMIN(ch->name) )
	{
...
*/
	...
	else
	{
		...
		if ( ch->move < move )
		{
			ch_printf( ch, "Jeste¶ zbyt wyczerpan%s." NL,
						SEX_SUFFIX_YAE( ch ) );
			return rNONE;
		}
	}
	...
	*/
	...
	&& ( IS_NPC(ch) || !IS_SET(ch->act, PLR_WIZINVIS) ) )
	{
		...
		if( ch->mount )
		{
			sprintf( buf, "$n %s %s %s na $N$5.",
			txt, door<10 ? "na":"" , dir_where_name[door] );
	...
	*/
	...
	if( IS_SET( to_room->room_flags, ROOM_PROTOTYPE )
...
EXIT_DATA *find_door( CHAR_DATA *ch, char *arg, bool quiet )
{
	...
	else
	{
		...
		{
			if ( (quiet || IS_SET(pexit->flags, EX_ISDOOR))
				&&    pexit->keyword
				&&    is_name_prefix( arg, pexit->keyword ) )
				return pexit;
		}
		if ( !quiet )
...
DEF_DO_FUN( open )
{
	...
	if ( ( pexit = find_door( ch, arg, true ) ) != NULL )
	{
		...
		if ( !IS_SET(pexit->flags, EX_ISDOOR) )
			{ send_to_char( "Nie mo¿esz tego zrobiæ." NL,      ch ); return; }
		if ( !IS_SET(pexit->flags, EX_CLOSED) )
			{ send_to_char( "To jest ju¿ otwarte." NL,      ch ); return; }
		if (  IS_SET(pexit->flags, EX_LOCKED) )
			{ send_to_char( "To jest zamkniête na klucz." NL,            ch ); return; }
...
DEF_DO_FUN( close )
{
	...
	if ( ( pexit = find_door( ch, arg, true ) ) != NULL )
	{
		...
		if ( !IS_SET(pexit->flags, EX_ISDOOR) )
			{ send_to_char( "Nie mo¿esz tego zrobiæ." NL,      ch ); return; }
		if ( IS_SET(pexit->flags, EX_CLOSED) )
			{ send_to_char( "To jest ju¿ zamkniête." NL,    ch ); return; }
...
DEF_DO_FUN( lock )
{
	...
	if ( ( pexit = find_door( ch, arg, true ) ) != NULL )
	{
		...
		if ( !IS_SET(pexit->flags, EX_ISDOOR) )
			{ send_to_char( "Nie mo¿esz tego zrobiæ." NL,      ch ); return; }
		if ( !IS_SET(pexit->flags, EX_CLOSED) )
			{ send_to_char( "To nie jest zamkniête." NL,        ch ); return; }
		if ( pexit->key < 0 )
			{ send_to_char( "To nie mo¿e zostaæ zakluczone." NL,     ch ); return; }
		if ( !has_key( ch, pexit->key) )
			{ send_to_char( "Nie masz klucza." NL,       ch ); return; }
		if ( IS_SET(pexit->flags, EX_LOCKED) )
			{ send_to_char( "To jest ju¿ zakluczone." NL,    ch ); return; }
	...
	if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
	{
		/* 'lock object' */
		if ( obj->item_type != ITEM_CONTAINER )
			{ send_to_char( "To nie jest pojemnik." NL, ch ); return; }
		if ( !IS_SET(obj->value[1], CONT_CLOSED) )
			{ send_to_char( "To nie jest zamkniête." NL,        ch ); return; }
		if ( obj->value[2] < 0 )
			{ send_to_char( "To nie mo¿e byæ zakluczone." NL,     ch ); return; }
...
DEF_DO_FUN( unlock )
{
	...
	if ( ( pexit = find_door( ch, arg, true ) ) != NULL )
	{
		...
		if ( !IS_SET(pexit->flags, EX_CLOSED) )
			{ send_to_char( "To nie jest zamkniête." NL,        ch ); return; }
		if ( pexit->key < 0 )
			{ send_to_char( "To nie mo¿e byæ odkluczone." NL,   ch ); return; }
		if ( !has_key( ch, pexit->key) )
			{ send_to_char( "Nie masz klucza." NL,       ch ); return; }
		if ( !IS_SET(pexit->flags, EX_LOCKED) )
			{ send_to_char( "To jest ju¿ odkluczone." NL,  ch ); return; }
	...
	if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
	{
		...
		if ( !IS_SET(obj->value[1], CONT_CLOSED) )
			{ send_to_char( "To nie jest zamkniête." NL,        ch ); return; }
		if ( obj->value[2] < 0 )
			{ send_to_char( "To nie mo¿e byc odkluczone." NL,   ch ); return; }
		if ( !has_key( ch, obj->value[2] ) )
			{ send_to_char( "Nie masz klucza." NL,       ch ); return; }
		if ( !IS_SET(obj->value[1], CONT_LOCKED) )
			{ send_to_char( "To jest ju¿ odkluczone." NL,  ch ); return; }
...
DEF_DO_FUN( bashdoor )
{
	...
	if ( ( pexit = find_door( ch, arg, false ) ) != NULL )
	{
		...
		{
			act(COL_ACTION, "WHAAAAM!!!  Walisz z impetem w $d, ale nie udaje ci siê tego rozwaliæ.",
				ch, NULL, keyword, TO_CHAR );
			act(COL_ACTION, "WHAAAAM!!!  $n wali z impetem w $d, ale nie udaje $m siê tego rozwaliæ.",
	...
	{
		act(COL_ACTION, "WHAAAAM!!!  Walisz z impetem w ¶cianê, ale nie udaje ci siê tego rozwaliæ.",
	...
	switch ( ch->position )
	{
		case POS_SLEEPING:
			if ( IS_AFFECTED(ch, AFF_SLEEP) )
				{ send_to_char( "Jako¶ nie mo¿esz siê obudziæ. ZZzzz.. " NL, ch ); return; }
...
// bo tak :P  -- Thanos
	...
	if((pexit = get_exit(ch->in_room, exit_dir)) == NULL )
		nogo = true;
	else if( IS_SET(pexit->flags, EX_CLOSED)
...
DEF_DO_FUN( drag )
{
	...
	if( !*arg )
	{
		send_to_char( "Kogo chcesz bezlito¶nie wyci±gn±æ za chabety w celu zapewne wymiany argumentów?" NL,
			ch );
		return;
	}
