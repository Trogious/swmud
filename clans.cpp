...
CLAN_DATA *get_clan( char *name )
{
    ...
    for ( clan = first_clan; clan; clan = clan->next )
		if( !str_cmp( name, clan->name ) || (is_number( name ) && (clan->clan_id == atoi( name ))) )
	    return clan;
...
void save_clan( CLAN_DATA *clan )
{
	...
	else
	{
		...
		FOREACH( member, clan->first_member )
		    fprintf( fp, "Member       %s~ %d %s~\n", member->name, member->status, member->bestowments );
		/* Polityka */
		FOREACH( politics, clan->first_politics )
		{
		    fprintf( fp, "Politics     %d %d\n", politics->clan->clan_id, politics->relations );
		}
		/* voting polityczny */
		if( clan->vClan )
		{
...
CLAN_DATA *load_clan2( const char *filename )
{
	...
	FOREACH( node, root->children )
	{
		...
		if( !swXmlStrcmp( node->name, "id" ) )
			swGetContentInt( &clan->clan_id, node );
		else
		if( !swXmlStrcmp( node->name, "type" ) )
		{
			int clanType;
		...
		if( !swXmlStrcmp( node->name, "description" ) )
			swGetContent( &clan->description, node );
		else
		if( !swXmlStrcmp( node->name, "penalty" ) )
			swGetContentLong( &clan->penalty, node );
		else
		if( !swXmlStrcmp( node->name, "pkills" ) )
			swGetContentInt( &clan->pkills, node );
		else
		if( !swXmlStrcmp( node->name, "pdeaths" ) )
			swGetContentInt( &clan->pdeaths, node );
		else
		if( !swXmlStrcmp( node->name, "mkills" ) )
			swGetContentInt( &clan->mkills, node );
		else
		if( !swXmlStrcmp( node->name, "mdeaths" ) )
			swGetContentInt( &clan->mdeaths, node );
		else
		if( !swXmlStrcmp( node->name, "membersno" ) )
			swGetContentInt( &clan->members, node );
		else
		if( !swXmlStrcmp( node->name, "funds" ) )
			swGetContentLong( &clan->funds, node );
		else
		if( !swXmlStrcmp( node->name, "loan" ) )
			swGetContentLong( &clan->loan, node );
		else
		if( !swXmlStrcmp( node->name, "repaydate" ) )
			swGetContentInt( (int *)&clan->repay_date, node );
		else
		if( !swXmlStrcmp( node->name, "ranks" ) )
		{
			i = 0;
		...
		if( !swXmlStrcmp( node->name, "members" ) )
		{
			FOREACH( child, node->children )
			{
				EONLY( child );
				...
				if( !swXmlStrcmp( child->name, "member" ) )
				{
					...
					FOREACH( gchild, child->children )
					{
						...
						if( !swXmlStrcmp( gchild->name, "status" ) )
							swGetContentInt( &member->status, gchild );
						else
						if( !swXmlStrcmp( gchild->name, "bestowments" ) )
		...
		if( !swXmlStrcmp( node->name, "relations" ) )
		{
			FOREACH( child, node->children )
			{
				EONLY( child );
				...
				if( !swXmlStrcmp( child->name, "relation" ) )
				{
					...
					FOREACH( gchild, child->children )
					{
						...
						if( !swXmlStrcmp( gchild->name, "relations" ) )
...
void save_clan2( CLAN_DATA *clan )
{
	...
	swNewProp( root, "xsi:schemaLocation", "http://swmud.pl/ns/swmud/1.0/clan clan.xsd" );
...
void outcast_member( CLAN_DATA *clan, CHAR_DATA *ch )
{
	...
	FOREACH( pClan, first_clan )
		if( pClan != clan && (member = get_member( pClan, ch->name )) && member->status > CLAN_WAITING )
		{
			if( !ch->pcdata->clan )
			{
				found = true;
...
void application( CHAR_DATA *ch, char *argument )
{
	...
	if( IS_SET(clan->flags, CLAN_NOINFO) )
	{
		...
		for( victim = ch->in_room->first_person; victim && !found; victim = victim->next_in_room )
	...
	{
		send_to_char( "Podanie o przyjêcie zosta³o ju¿ z³o¿one wcze¶niej." NL, ch );
		return;
	}

	if( member )
	{
		ch_printf( ch, "Jeste¶ ju¿ cz³onki%s te%s %s." NL, MALE( ch ) ? "em" : "ni±",
			CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
		return;
	}

    act( COL_FORCE, "Sk³adasz podanie o przyjêcie do $t.",  ch, clan->name, NULL, TO_CHAR );
...
DEF_DO_FUN( induct )
{
	...
	if( arg[0] == '\0' )
	{
		ch_printf( ch, "Oto lista osób ubiegaj±cych siê o wst±pienie do twoje%s %s:" NL,
			CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
		FOREACH( member, clan->first_member )
	...
	if( IS_NPC(victim) )
	{
		ch_printf( ch, "Tej osoby nie mo¿esz przyj±æ do %s." NL, CLANTYPE( clan, 1 ) );
		return;
	}

	if( !(member = get_member( clan, victim->name )) )
	{
		ch_printf( ch, "Nikt taki nie zg³asza³ chêci wst±pienia do twoje%s %s." NL,
			CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
		return;
	}
	...
	|| !str_cmp( argument, "tak" ) )
	{
		...
		else
		{
			...
			act( COL_FORCE, "Przyjmujesz $N$3 do $t.",  ch, clan->name, victim, TO_CHAR );
			act( COL_FORCE, "$n przyjmuje $N$3 do $t.", ch, clan->name, victim, TO_NOTVICT );
			act( COL_FORCE, "$n przyjmuje Ciê do $t.", ch, clan->name, victim, TO_VICT );
		}
		return;
	}

	if( !str_cmp( argument, "deny" )
	|| !str_cmp( argument, "nie" ) )
	{
		remove_member( clan, victim->name );
		act( COL_FORCE, "Odrzucasz podanie $N$1 o przyjêcie do $t.",  ch, clan->name, victim, TO_CHAR );
		act( COL_FORCE, "$n odrzuca podanie $N$1 o przyjêcie do $t.", ch, clan->name, victim, TO_NOTVICT );
		act( COL_FORCE, "$n odrzuca twoje podanie o przyjêcie do $t.",ch, clan->name, victim, TO_VICT );
		return;
	}
...
DEF_DO_FUN( outcast )
{
	...
	if( !(vmember = get_member( clan, arg )) )
	{
		ch_printf( ch, "Tw%s %s nie posiada takiego cz³onka jak %s." NL,
			CLANSUFFIX( clan, "ój", "oja" ), CLANTYPE( clan, 1 ), capitalize(arg) );
		return;
	}
	...
	if( (victim = get_player_world( ch, arg )) )
	{
		...
		//{
			...
			act( COL_FORCE, "Usuwasz $N$3 z $t." NL,  ch, clan->name, victim, TO_CHAR );
			act( COL_FORCE, "$n usuwa $N$3 z $t." NL, ch, clan->name, victim, TO_NOTVICT );
			act( COL_FORCE, "$n usuwa Ciê z $t." NL, ch, clan->name, victim, TO_VICT );
			char_from_room( ch );
	...
	{
		ch_printf( ch, "%s nie nalezy juz do %s %s" NL, vmember->name, CLANTYPE( clan, 1 ), clan->name );
...
DEF_DO_FUN( setclan )
{
	...
	if( !*arg1 || !*arg2 )
	{
		send_to_char( "Usage: setclan <clan> <field> <value> [value] ..." NL, ch );
...
*/
		send_to_char( "Chwilowo nie dzialaja organizacje podporz±dkowane (suborg)." NL, ch );
		return;
    }
	...
	if( !str_cmp( arg2, "rank" ) )
	{
		...
		{
			ch_printf( ch, "Syntax: rank <nr> <rank>" NL NL "note: %d <= nr <= %d" NL
    ...
    if ( !str_cmp( arg2, "type" ) )
    {
		...
		|| !str_cmp( argument, "gildia" ) )
			clan->type = CLAN_GUILD;
		else
		if( !str_cmp( argument, "order" )
		|| !str_cmp( argument, "zakon" ) )
			clan->type = CLAN_ORDER;
		else
		if( !str_cmp( argument, "suborg" ) )
...
DEF_DO_FUN( relations )
{
	...
	if( !IS_ORG( clan ) )
	{
		send_to_char( "Tylko organizacje niezale¿ne mog± zajmowaæ siê polityk±." NL, ch );
		return;
	}

	if( *arg1 == '\0' )
	{
		ch_printf( ch, FB_CYAN "Oto jak przedstawia siê polityka prowadzona przez twoj± organizacjê:" EOL NL );
		if( clan->diplomat )
			ch_printf( ch, FB_CYAN "Dyplomata: " PLAIN "%s" FB_CYAN "." EOL NL, clan->diplomat->name );

		FOREACH( politics, clan->first_politics )
			ch_printf( ch, "Twoja organizacja prowadzi %s politykê w stosunku do: " FG_CYAN "%s" PLAIN "." NL,
				bit_name( relation_types_list, politics->relations ), CLANNAME( politics->clan ) );

		if( !(member = get_member( clan, ch->name )) || member->status < CLAN_DIPLOMAT )
			return;
		...
		FOREACH( sclan, first_clan )
		{
			...
			FOREACH( politics, sclan->first_politics )
				...
				{
					ch_printf( ch, "Organizacja " FG_CYAN "%s" PLAIN " prowadzi %s politykê z twoj± organizacj±." NL,
						CLANNAME( sclan ), bit_name( relation_types_list, politics->relations ) );
					pnone=false;
				}

			if( pnone )
				ch_printf( ch, "Organizacja " FG_CYAN "%s" PLAIN " nie prowadzi ¿adnej polityki z twoj± organizacj±." NL,
					CLANNAME( sclan ) );
		}
	...
	if( !str_cmp( arg1, "change" ) || !str_cmp( arg1, "zmien" ) )
	{
		...
		if( *argument == '\0' )
		{
			send_to_char( "Z jak± organizacj± chcesz prowadziæ t± politykê?" NL, ch );
			return;
		}
		...
		if( clan->vClan )
		{
			send_to_char( "Musisz poczekaæ na zakoñczenie obecnego g³osowania." NL, ch );
			return;
		}
		...
		FOREACH( politics, clan->first_politics )
			...
			{
				send_to_char( "Obecnie prowadzisz w³a¶nie tak± politykê. Po co g³osowaæ drugi raz?" NL, ch );
				return;
			}

		clan->vRelations = value;
		clan->vClan = tclan;
		ch_printf( ch, "Zmiana polityki w stosunku od organizacji: " FG_CYAN "%s" EOL
	...
	if( !str_cmp( arg1, "vote" ) || !str_cmp( arg1, "glosuj" ) )
	{
		...
		if( !(vclan = clan->vClan) )
		{
			send_to_char( "Obecnie nie ma ¿adnego g³osowania dotycz±cego polityki." NL, ch );
			return;
		}
		...
		if( *arg2=='\0' )
		{
			send_to_char( FB_CYAN "Temat g³osowania dotycz±cego zmiany polityki w Twojej organizacji:" EOL NL, ch );
			ch_printf( ch, "G³osowanie dotyczy zmiany polityki na %s w stosunku do organizacji: "
				FG_CYAN "%s" PLAIN "." NL NL, bit_name( relation_types_list, vrelations ), CLANNAME( vclan ) );
			ch_printf( ch, "G³osy oddane na TAK: %s." NL, (*clan->vYes != '\0') ? clan->vYes: "" );
		...
		if( !str_cmp( arg2, "tak" ) || !str_cmp( arg2, "yes" ) )
		{
			...
			{
				send_to_char( "Nie masz uprawnieñ do g³osowania na temat polityki." NL, ch );
				return;
			}
			...
			ch_printf( ch, "Polityka w stosunku do organizacji: " FG_CYAN "%s" EOL
				"zosta³a zmieniona pomy¶lnie i wprowadzona w ¿ycie." NL, CLANNAME( vclan ) );
		...
		if( !str_cmp( arg2, "nie" ) || !str_cmp( arg2, "no" ) )
		{
			...
			ch_printf( ch, "Polityka w stosunku od organizacji: " FG_CYAN "%s" EOL
		"nie zosta³a zmieniona. G³osowanie nie przebieg³o pomy¶lnie." NL, CLANNAME( vclan ) );
			pclan_voting_end( clan );
			save_clan( clan );
...
DEF_DO_FUN( showclan )
{
	...
	ch_printf( ch, FG_CYAN "Name: " PLAIN "%s" NL FG_CYAN "Filename: " PLAIN "%s" FG_CYAN "	Id: " PLAIN "%4d"
		FG_CYAN "  Type: " PLAIN "%d" FG_CYAN " (" PLAIN "%s" FG_CYAN ")" NL, clan->name, clan->filename,
		clan->clan_id, clan->type, CLANTYPE( clan, 0 ) );

	ch_printf( ch, FG_CYAN "Flags: " PLAIN "%s" FG_CYAN "  Penalty: " PLAIN "%ld" NL,
		flag_string( clan_flags_list, clan->flags ), clan->penalty );

	ch_printf( ch, FG_CYAN "Leader: " PLAIN "%s" FG_CYAN "  Das erste:): " PLAIN "%s" FG_CYAN "  Second: "
		PLAIN "%s" FG_CYAN "  Diplomat: " PLAIN "%s" NL, LEADER_NAME( clan ),
		FIRST_NAME( clan ), SECOND_NAME( clan ), DIPLOMAT_NAME( clan ) );

	ch_printf( ch, FG_CYAN "Members(" PLAIN "%d" FG_CYAN "):" PLAIN, clan->members );
	FOREACH( member, clan->first_member )
		if( member->status != CLAN_WAITING )
	...
	i = 0;
	ch_printf( ch, NL FG_CYAN "Applicants(" PLAIN "%d" FG_CYAN "):" PLAIN, applicants );
	FOREACH( member, clan->first_member )
		if( member->status == CLAN_WAITING )
			ch_printf( ch, "%s%s", (i++)%4?"    " : NL, member->name );
	...
	ch_printf( ch, FG_CYAN "Funds: " PLAIN "%ld" FG_CYAN "  Loan: " PLAIN "%ld" FG_CYAN "  repay_date: " PLAIN "%s" NL,
		clan->funds, clan->loan, (int)clan->repay_date ? static_cast<SWString>(SWTimeStamp(clan->repay_date)).c_str() : "" );
	ch_printf( ch, FG_CYAN "PKills: " PLAIN "%6d" FG_CYAN " PDeaths: " PLAIN "%6d" NL
		FG_CYAN "MKills: " PLAIN "%6d" FG_CYAN " MDeaths: " PLAIN "%6d" NL, clan->pkills,
		clan->pdeaths, clan->mkills, clan->mdeaths );
...
DEF_DO_FUN( clans )
{
		...
		if( IS_ORG( clan ) )
		{
			...
			if( clan->first_suborg )
			{
				...
				for( suborg = clan->first_suborg; suborg; suborg = suborg->next_suborg )
					ch_printf( ch, "   %-22s  [Id:%2d]" NL, suborg->name, suborg->clan_id );
			}
			count++;
		}
	...
	if( !count )
		send_to_char( "Nie ma ¿adnych organizacji, gildii ani zakonów... jeszcze." NL, ch );
...
DEF_DO_FUN( claninfo )
{
	...
	if( !clan )
	{
		ch_printf( ch, "Brak informacji w bazie danych na temat %s." NL, argument );
		return;
	}
	else
	if( !(is_member = (((member = get_member( clan, ch->name )) != NULL
	&& member->status > CLAN_WAITING)
	|| IS_IMMORTAL( ch ))) && IS_SET(clan->flags, CLAN_NOINFO) )
	{
		ch_printf( ch, "Brak informacji w bazie danych na temat %s." NL, argument );
		return;
	}
	...
	ch_printf( ch, FB_YELLOW
		...
		clan->name, clan->clan_id, CLANTYPE( clan, 0 ), CRANK( clan, CLAN_LEADER ), LEADER_NAME(clan) );
	...
	if( clan->first )
	{
		...
		if( clan->second )
			ch_printf( ch, FG_CYAN " i " PLAIN "%s" FG_CYAN "." EOL, SECOND_NAME(clan) );
		else
	...
	if( IS_ORG( clan ) && clan->diplomat )
		ch_printf( ch, FG_CYAN "Polityk± organizacji zajmuje siê " PLAIN "%s" FG_CYAN "." EOL,
			DIPLOMAT_NAME(clan) );

	ch_printf( ch,	FG_CYAN "%s posiada " PLAIN "%d" FG_CYAN " cz³onk%s", capitalize( CLANTYPE( clan, 0 ) ),
		clan->members, NUMBER_SUFF( clan->members, "a", "ów", "ów" ) );
	if( is_member )
		ch_printf( ch, FG_CYAN " a na koncie ma " PLAIN "%d" FG_CYAN " kredytek." EOL, clan->funds );
	else
		send_to_char( "." NL, ch );
	...
	{
		{
			...
			if( x )
				ch_printf( ch, FG_CYAN "Planety podlegaj±ce organizacji:" PLAIN "%s" FG_CYAN "." EOL, buf  );
		}
		...
		if( clan->loan )
			ch_printf( ch, FG_CYAN "Organizacja musi sp³aciæ po¿yczkê wysoko¶ci: " PLAIN "%ld"
		...
		{
			j++;
			ch_printf( ch, FG_CYAN "%d." FG_YELLOW " %s" FG_CYAN ", Lider:" PLAIN " %s" FG_CYAN ", Mo¿ni:" PLAIN
				" %s" FG_CYAN " i" PLAIN " %s" FG_CYAN "." NL,
				j, CLANNAME(suborg), suborg->leader, suborg->first, suborg->second );
		}
		ch_printf( ch, FG_CYAN "Ilo¶æ organizacji podlegaj±cych:" PLAIN " %d" FG_CYAN "." NL, j );

		ch_printf( ch, FG_CYAN "Bilans zgonów:" PLAIN " %d " FG_CYAN "zabójstw," PLAIN " %d " FG_CYAN "zgonów." NL, clan->pkills, clan->pdeaths);
		...
		FOREACH( ship, first_ship )
			...
			{
				i++;
				ch_printf( ch, FG_CYAN "%d." PLAIN " %s" FG_CYAN ", Pilot:" PLAIN " %s" FG_CYAN ", Drugi pilot:" PLAIN
					" %s" FG_CYAN "." NL, i, ship->name, ship->pilot, ship->copilot );
			}
		ch_printf( ch, FG_CYAN "Ilo¶æ statków kosmicznych:" PLAIN " %d" FG_CYAN "." EOL, i );
	}
}
...
void clan_members( CHAR_DATA *ch )
{
	...
	if( !clan )
	{
		send_to_char( "Nie nale¿ysz do ¿adnej organizacji, gildii ani ¿adnego zakonu." NL, ch );
		return;
	}

	ch_printf( ch, FG_CYAN "%s: " PLAIN "%s" FG_CYAN ". Pomagaj± mu: %s: " PLAIN "%s" FG_CYAN
		" i %s: " PLAIN "%s" FG_CYAN "." EOL, CRANK( clan, CLAN_LEADER ), LEADER_NAME(clan),
		CRANK( clan, CLAN_FIRST ), FIRST_NAME(clan), CRANK( clan, CLAN_SECOND ), SECOND_NAME(clan) );

	if( IS_ORG( clan ) && clan->diplomat )
		ch_printf( ch, FG_CYAN "Polityk± organizacji zajmuje siê: " PLAIN "%s" FG_CYAN "." EOL,
			DIPLOMAT_NAME(clan) );

	ch_printf( ch, FG_CYAN "Oto lista cz³onków twoje%s %s:" EOL, CLANSUFFIX( clan, "go", "j" ),
		CLANTYPE( clan, 1 ) );
	i = 0;
	FOREACH( member, clan->first_member )
		if( member->status > CLAN_WAITING )
	...
	ch_printf( ch, NL NL FG_CYAN "%s posiada " PLAIN "%d" FG_CYAN " cz³onk%s." EOL,
		capitalize( CLANTYPE( clan, 0 ) ), clan->members, (clan->members==1)
		? "a" : "ów" );
...
void clan_switch( CHAR_DATA *ch, char *argument )
{
	...
	if( !(clan = get_clan( argument )) || (IS_SET(clan->flags, CLAN_NOINFO)
	&& (!(member = get_member( clan, ch->name )) || member->status < CLAN_MEMBER)
	&& !IS_IMMORTAL( ch )) )
		send_to_char( "Nie ma takiej organizacji, gildii ani zakonu." NL, ch );
	else
	if( (member = get_member( clan, ch->name )) && member->status >= CLAN_MEMBER )
	{
		ch->pcdata->clan = clan;
		save_char_obj( ch );
		ch_printf( ch, "Zaczynasz zajmowaæ siê %s %s." NL, CLANTYPE( clan, 5 ), clan->name );
	}
	else
		ch_printf( ch, "Nie nale¿ysz do te%s %s." NL, CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
}
...
DEF_DO_FUN( resign )
{
	...
	{
		ch_printf( ch, "Nie mo¿esz zrezygnowaæ z cz³onkostwa w %s." NL, CLANTYPE( clan, 4 ) );
		return;
	}
	...
	if( IS_SET(clan->flags, CLAN_PENALTYQUIT) )
	{
		...
		{
			ch_printf( ch, "Nie masz tyle kredytek aby zap³aciæ karê za rezygnacjê z cz³onkostwa w %s." NL,
				CLANTYPE( clan, 4 ) );
			return;
		}
		else
		{
			...
			ch_printf( ch, "P³acisz %ld kredytek kary za rezygnacjê z cz³onkostwa w %s." NL,
				clan->penalty, CLANTYPE( clan, 4 ) );
			(ch->gold >= penalty) ? (ch->gold -= penalty) : (ch->pcdata->bank -= penalty);
		}
	}


	outcast_member( ch->pcdata->clan, ch );
	act( COL_FORCE, "Rezygnujesz z cz³onkostwa w $t", ch, clan->name, NULL, TO_CHAR );
	loose_exp = UMAX( ch->experience[DIPLOMACY_ABILITY] - exp_level( ch->skill_level[DIPLOMACY_ABILITY] ), 0 );
	ch->experience[DIPLOMACY_ABILITY] -= loose_exp;
	ch->experience[DIPLOMACY_ABILITY] = URANGE( 0, ch->experience[DIPLOMACY_ABILITY], ch->experience[DIPLOMACY_ABILITY] );
	ch_printf( ch, "Tracisz %ld punktów do¶wiadczenia w dyplomacji." NL, loose_exp );
}
...
DEF_DO_FUN( clan_withdraw )
{
	...
	if( !clan )
	{
		send_to_char( "Musisz najpierw nale¿eæ do jakiej¶ organizacji, gildii, lub zakonu." NL, ch );
		return;
	}
	...
	{
		send_to_char( "Aby wyp³acaæ gotówkê z konta organizacji trzeba najpierw sp³aciæ po¿yczkê." NL, ch );
		return;
	}
...
DEF_DO_FUN( appoint )
{
	...
	if( !*argument )
	{
		send_to_char( "Sk³adnia: appoint <imiê> stopieñ | pierwszy | drugi | dyplomata" NL, ch );
		return;
	}

	if( !(member = get_member( clan, arg )) )
	{
		ch_printf( ch, "Nikt taki nie nale¿y do twoje%s %s." NL, CLANSUFFIX( clan, "go", "j" ),
			CLANTYPE( clan, 1 ) );
		return;
	}
	...
	{
		ch_printf( ch, "Aby awansowac cz³onka %s, koniecznie jest spotkanie." NL, CLANTYPE( clan, 1 ) );
		return;
	}

	if( victim == ch )
	{
		ch_printf( ch, "Jeste¶ liderem - to najwy¿sze stanowisko w %s." NL, CLANTYPE( clan, 4 ) );
		return;
	}
	...
	{
		if( member->status > 9 )
			ch_printf( ch, "%s posiada ju¿ najwy¿szy stopieñ w %s." NL, victim->przypadki[0],
				CLANTYPE( clan, 4 ) );
		else
		{
			member->status++;
			act( COL_FORCE, "Zwiêkszasz $N$3 stopieñ w $t.",  ch, clan->name, victim, TO_CHAR );
			act( COL_FORCE, "$n zwiêksza $N$3 stopieñ w $t.", ch, clan->name, victim, TO_NOTVICT );
			act( COL_FORCE, "$n zwiêksza ci stopieñ w $t.", ch, clan->name, victim, TO_VICT );
		}
	}
	else
	if( is_name( argument, "first pierwszy" ) )
	{
		if( member->status == CLAN_FIRST )
			ch_printf( ch, "%s jest ju¿ pierwsz%s twoj%s %s." NL, victim->przypadki[0],
				SEX_STR( victim, "±", "ym", "ym" ), CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
		else
		{
			act( COL_FORCE, "Czynisz $N$3 pierwszym w $t.",  ch, clan->name, victim, TO_CHAR );
			act( COL_FORCE, "$n czyni $N$3 pierwszym w $t.", ch, clan->name, victim, TO_NOTVICT );
			act( COL_FORCE, "$n czyni ciê pierwszym w $t.", ch, clan->name, victim, TO_VICT );
			clan->first = member;
			member->status = CLAN_FIRST;
		}
	}
	else
	if( is_name( argument, "second drugi" ) )
	{
		if( member->status == CLAN_SECOND )
			ch_printf( ch, "%s jest ju¿ drug%s twoj%s %s." NL, victim->przypadki[0],
				SEX_STR( victim, "±", "im", "im" ), CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
		else
		{
			act( COL_FORCE, "Czynisz $N$3 drugim w $t.",  ch, clan->name, victim, TO_CHAR );
			act( COL_FORCE, "$n czyni $N$3 drugim w $t.", ch, clan->name, victim, TO_NOTVICT );
			act( COL_FORCE, "$n czyni ciê drugim w $t.", ch, clan->name, victim, TO_VICT );
			clan->second = member;
			member->status = CLAN_SECOND;
		}
	}
	else
	if( is_name( argument, "diplomat dyplomata" ) && clan->type <= CLAN_SUBORG )
	{
		if( member->status == CLAN_DIPLOMAT )
			ch_printf( ch, "%s jest ju¿ dyplomat± twoj%s %s." NL, victim->przypadki[0],
				CLANSUFFIX( clan, "go", "j" ), CLANTYPE( clan, 1 ) );
		else
		{
			...
			{
				send_to_char( "Ta osoba nie jest odpowiednim kandydatem na dyplomatê Twojej organizacji." NL, ch );
				return;
			}

			act( COL_FORCE, "Czynisz $N$3 dyplomat± w $t.",  ch, clan->name, victim, TO_CHAR );
			act( COL_FORCE, "$n czyni $N$3 dyplomat± w $t.", ch, clan->name, victim, TO_NOTVICT );
			act( COL_FORCE, "$n czyni ciê dyplomat± w $t.", ch, clan->name, victim, TO_VICT );
			clan->diplomat = member;
			member->status = CLAN_DIPLOMAT;
		}
	}
...
DEF_DO_FUN( demote )
{
	...
	if( !*argument )
	{
		send_to_char( "Sk³adnia: demote <imiê> stopieñ | pierwszy | drugi | dyplomata" NL, ch );
		return;
	}
	...
	if( is_name( argument, "rank stopien" ) )
	{
		if( member->status <= CLAN_MEMBER )
			ch_printf( ch, "%s posiada ju¿ najni¿szy stopieñ w %s." NL, member->name, CLANTYPE( clan, 4 ) );
		else
		{
			member->status--;
			act( COL_FORCE, "$T uzyskuje ni¿szy stopieñ w $t.",  ch, clan->name, member->name, TO_CHAR );
		}
	}
	else
	if( is_name( argument, "first pierwszy" ) )
	{
		if( member->status != CLAN_FIRST )
			ch_printf( ch, "%s nie jest pierwszym twoj%s %s." NL, member->name, CLANSUFFIX( clan, "go", "j" ),
				CLANTYPE( clan, 1 ) );
		else
		{
			act( COL_FORCE, "$T traci status pierwszego w $t.",  ch, clan->name, member->name, TO_CHAR );
			clan->first = NULL;
			member->status = CLAN_MEMBER;
		}
	}
	...
	{
		if( member->status != CLAN_SECOND )
			ch_printf( ch, "%s nie jest drugim twoj%s %s." NL, member->name, CLANSUFFIX( clan, "go", "j" ),
				CLANTYPE( clan, 1 ) );
		else
		{
			act( COL_FORCE, "$T traci status drugiego w $t.",  ch, clan->name, member->name, TO_CHAR );
			clan->second = NULL;
			member->status = CLAN_MEMBER;
		}
	}
	...
	if( is_name( argument, "diplomat dyplomata" ) && clan->type <= CLAN_SUBORG )
	{
		...
		{
			act( COL_FORCE, "$T traci status dyplomaty $t.",  ch, clan->name, member->name, TO_CHAR );
			clan->diplomat = NULL;
			member->status = CLAN_MEMBER;
		}
	}
...
DEF_DO_FUN( capture )
{
	...
	{
		send_to_char ( "Tylko organizacje mog± przejmowaæ w³adze na planetach." NL , ch );
		return;
	}
	...
	if( planet->starsystem )
	{
		...
		for( ship = planet->starsystem->first_ship; ship; ship = ship->next_in_starsystem )
			...
			{
				send_to_char ( "Planeta nie mo¿e byæ przejêta, gdy¿ jest chroniona przez orbituj±cy statek." NL , ch );
				return;
			}
	}
	...
	{
		send_to_char ( "Mieszkañcy planety nie zgadzaj± siê na t± zmianê w tej chwili." NL , ch );
		return;
	}
	...
	if( support < 0 )
	{
		send_to_char ( "Poparcie dla twojej organizacji jest niewystarczaj±ce!" NL
			"Spróbuj polepszyæ lojalno¶æ na planetach, które ju¿ kontrolujesz." NL, ch );
		return;
	}
	...
	sprintf( buf , "%s przejmuje kontrolê nad planet± %s!", clan->name, planet->name );
	echo_to_all( buf , 0 );
	save_planet( planet );
}
...
DEF_DO_FUN( empower )
{
	...
	if( !(member = get_member( clan, arg )) )
	{
		ch_printf( ch, "Ta osoba nie nale¿y do twoje%s %s." NL, CLANSUFFIX( clan, "go", "j" ),
			CLANTYPE( clan, 1 ) );
		return;
	}
	...
	{
		ch_printf( ch, "Po co? Przecie¿ i tak bêdziesz mia³%s wszystkie uprawnienia." NL,
			SEX_SUFFIX__AO( ch ) );
		return;
	}
	...
	{
		ch_printf( ch, "%s jest upowa¿nion%s do: %s." NL, victim->przypadki[0], SEX_SUFFIX_YAE( victim ),
			member->bestowments );
		return;
	}
	else
	if( is_name( arg2, member->bestowments ) )
	{
		ch_printf( ch, "%s jest ju¿ upowa¿nion%s do: %s." NL, victim->przypadki[0], SEX_SUFFIX_YAE( victim ),
			arg2 );
		return;
	}
	else
	if( !str_cmp( arg2, "pilot" ) && (IS_ORG( clan ) || IS_SUBORG( clan )) )
	{
		sprintf( buf, "%s %s", member->bestowments, "pilot" );
		STRDUP( member->bestowments, buf );
		ch_printf( victim, "%s upowa¿nia ciê do latania statkami organiacji." NL, PERS( ch, victim, 0 ) );
		send_to_char( "Ok. Upowa¿aniasz do latania statkami organizacji." NL, ch );
	}
	else
	if( !str_cmp( arg2, "withdraw" ) )
	{
		sprintf( buf, "%s %s", member->bestowments, "withdraw" );
		STRDUP( member->bestowments, buf );
		ch_printf( victim, "%s upowa¿nia ciê do wyp³acania funduszy %s." NL, PERS( ch, victim, 0 ),
			CLANTYPE( clan, 1 ) );
		ch_printf( ch, "Ok. Upowa¿aniasz %s do wyp³acania funduszy %s." NL, victim->przypadki[1],
			CLANTYPE( clan, 1 ) );
	}
	else
	if( !str_cmp( arg2, "clanbuyship" ) && (IS_ORG( clan ) || IS_SUBORG( clan )) )
	{
		sprintf( buf, "%s %s", member->bestowments, "clanbuyship" );
		STRDUP( member->bestowments, buf );
		ch_printf( victim, "%s przydziela ci prawo do kupowania statków dla organizacji." NL,
			PERS( ch, victim, 0 ) );
		send_to_char( "Ok. Upowa¿aniasz do kupowania statków na rzecz organizacji." NL, ch );
	}
	else
	if( !str_cmp( arg2, "clansellship" ) && (IS_ORG( clan ) || IS_SUBORG( clan )) )
	{
		sprintf( buf, "%s %s", member->bestowments, "clansellship" );
		STRDUP( member->bestowments, buf );
		ch_printf( victim, "%s przydziela ci prawo do sprzedawania statków nale¿±cych do organizacji." NL,
			PERS( ch, victim, 0 ) );
		send_to_char( "Ok. Upowa¿aniasz do sprzedawania statków nale¿±cych do organizacji." NL, ch );
	}
	else
	if( !str_cmp( arg2, "induct" ) )
	{
		...
		STRDUP( member->bestowments, buf );
		ch_printf( victim, "%s upowa¿nia ciê do przyjmowania nowych cz³onków w szeregi %s." NL,
			PERS( ch, victim, 0 ), CLANTYPE( clan, 2 ) );
		send_to_char( "Ok. Upowa¿aniasz do przyjmowania nowych cz³onków." NL, ch );
	}
	else
	...
	if( !str_cmp( arg2, "loan" ) && (IS_ORG( clan ) || IS_SUBORG( clan )) )
	{
		sprintf( buf, "%s %s", member->bestowments, "loan" );
		STRDUP( member->bestowments, buf );
		ch_printf( victim, "%s upowa¿nia ciê do pobierania po¿yczek na rzecz organizacji." NL,
			PERS( ch, victim, 0 ) );
		send_to_char( "Ok. Upowa¿aniasz do brania po¿yczek na rzecz organizacji." NL, ch );
	}
	else
...
bool clan_bestow( CHAR_DATA *ch, const char *cmd )
{
	...
	&& (is_name( cmd, member->bestowments ) || member->status == CLAN_LEADER ) )
		return true;
	return false;
}
...
void clan_loan( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "Nie zajmujesz siê ¿adn± organizacj± w tej chwili." NL, ch );
		return;
	}
	...
	if( !*arg )
	{
		send_to_char( "Chcesz wzi±æ po¿yczkê (borrow), czy j± sp³aciæ (repay)?" NL, ch );
		return;
	}
	...
	if( !str_cmp( arg, "borrow" ) )
	{
		...
		{
			ch_printf( ch, "Nie op³aca siê braæ mniejszej po¿yczki ni¿ %ld." NL, CLAN_MINLOAN );
			return;
		}

		if( (ammount + clan->loan) > CLAN_MAXLOAN( clan ) )
		{
			ch_printf( ch, "Nie mo¿esz wzi±æ wiêkszej po¿yczki ni¿ %ld." NL, CLAN_MAXLOAN( clan ) );
			return;
		}
	...
	if( !str_cmp( arg, "repay" ) )
	{
		if( clan->loan > clan->funds )
		{
			ch_printf( ch, "Organizacja nie posiada wystarczaj±cych funduszy aby sp³aciæ po¿yczkê." NL );
			return;
		}
