...
DEF_DO_FUN( gold )
{
    ...
    "Masz przy sobie " FB_RED "%d" FG_RED " kredyt%s." EOL,
	vch->gold, NUMBER_SUFF( vch->gold, "kê", "ki", "ek" ) );

    if( vch->pcdata && vch->pcdata->bank )
    	ch_printf( ch,  PLAIN FG_RED
    			"Na twoim koncie jest obecnie " FB_RED "%d" FG_RED " kredyt%s." EOL,
    			vch->pcdata->bank, NUMBER_SUFF( vch->pcdata->bank, "kê", "ki", "ek" ) );
...
char *ac_info( CHAR_DATA *ch )
{
    ...
    GET_AC(ch) >    50 ?
	sprintf( b, "GORZEJ ni¿ nag%s!", 	SEX_SUFFIX_IAE(ch) ) :
    GET_AC(ch) >=   40 ?
	sprintf( b, "Nag%s.",			SEX_SUFFIX_IAE(ch) ) :
    GET_AC(ch) >=   30 ?
	sprintf( b, "Ubran%s.",			SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >=   20 ?
	sprintf( b, "Nieco uzbrojon%s.",	SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >=   10 ?
	sprintf( b, "Trochê uzbrojon%s.",	SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >=    0 ?
	sprintf( b, "Uzbrojon%s.",		SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >= - 40 ?
	sprintf( b, "Dobrze uzbrojon%s.",	SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >= - 80 ?
	sprintf( b, "Mocno uzbrojon%s.",	SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >= -120 ?
	sprintf( b, "Ciê¿ko uzbrojon%s.",	SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >= -160 ?
	sprintf( b, "Niesamowicie uzbrojon%s.",	SEX_SUFFIX_YAE(ch) ) :
    GET_AC(ch) >= -200 ?
	sprintf( b, "Nie do zdarcia!" ) 			     :
	sprintf( b, "NiEzNiSzCzAlN%s!",		SEX_SUFFIX_YAE(ch) );
...
/*
    ...
    return b;
 */
    return (char *)(	ch->alignment >  900 ? "Mistrza Jedi!"		:
		ch->alignment >  700 ? "Rycerza Jedi."		:
		ch->alignment >  350 ? "Senatora Republiki."	:
		ch->alignment >  100 ? "Rebelianta."		:
		ch->alignment > -100 ? "Neutralny."		:
		ch->alignment > -350 ? "Szturmowca Imperium."	:
		ch->alignment > -700 ? "Moffa Imperium."	:
		ch->alignment > -900 ? "Mrocznego Jedi."	:
				       "Lorda Sith!"		);
}
...
void do_clonescore( CHAR_DATA *ch, char *argument )	//Trog
{
	...
	ch_printf( ch, COL_SCORE "*-----Informacja-o-klonie-----*" NL
				   ...
				   "| Data: %22s|" NL
				   "*-----------------------------*" NL RESET,
				   buf, defectprcnt, static_cast<SWString>(SWTimeStamp(fst.st_mtime)).c_str());
	return;
}
...
DEF_DO_FUN( oldscore )
{
	...
	if (ch->attribute1>=0 || ch->attribute2>=0)
		ch_printf( ch,"\n\rJeste¶ postrzegan%s jako : %s" EOL, ch->sex == SEX_MALE ? "y" : "a",
                                                                     format_char_attribute(ch,0));

    if ( get_trust( ch ) != ch->top_level )
	ch_printf( ch, "Jeste¶ zaufany na poziom %d.\n\r", get_trust( ch ) );

    send_to_char("----------------------------------------------------------------------------\n\r", ch);
    ch_printf(ch,   "Zapisywa³e¶:  %s" NL, ch->save_time ?
    		static_cast<SWString>(SWTimeStamp(ch->save_time)).c_str() : "nie." NL );
...

    {
       ...
       for ( ability = 0 ; ability < MAX_ABILITY ; ability++ )
          if ( ability != FORCE_ABILITY || ch->skill_level[FORCE_ABILITY] > 1 )
            ...
            class_table[ability].przypadki[0], ch->skill_level[ability], max_level(ch, ability), ch->experience[ability],
            exp_level( ch->skill_level[ability]+1 ) );
          else
            ch_printf( ch, "%-15s Poziom: %-3d   Max: ???   Exp: ???  Nastêpny: ???\n\r",
            class_table[ability].przypadki[0], ch->skill_level[ability], ch->experience[ability]);
    }
	...
	{
	    ch_printf( ch, "%s(%d/%d) ", spice_types_list[drug],
    ...
    {
      send_to_char( "----------------------------------------------------------------------------\n\r", ch);
      ch_printf(ch, "ORGANIZACJA: %-35s Pkills/Deaths: %3.3d/%3.3d",
...
DEF_DO_FUN( score )
{
    ...
    "| Status: %3d letn%s %-15s (%4d godz. gry)/         "         "   |" NL,
		IS_NPC(ch) ? (char *)strip_colors( ch->przypadki[0], 44 ) :
			     (char *)strip_colors( titbuf, 44 ),
		get_age(ch),
                ch->sex == SEX_MALE ? "i " : "ia",
		( IS_NPC( ch ) && IS_SET( ch->act, ACT_DROID ) ) ? "Droid" :
		CH_RACE( ch, 0),                   // Pixel: przypadkiF
		( (int)SEC_PLAYED( ch )/3600 ) );
...
/*    ch_printf(ch, " |                                 Praktyki:         %-6d       |" NL,
		ch->practice );*/
    ch_printf(ch, " |   Punkty questowe:  %-6d      Charyzma:         %-6d       |" NL,
    	!IS_NPC( ch ) ? ch->pcdata->quest_points : 0, get_curr_cha( ch ) );
    for ( ability = 0 ; ability < MAX_ABILITY ; ability++ )
    {
	if ( ability != FORCE_ABILITY || ch->skill_level[FORCE_ABILITY] > 1 )
	    ch_printf(ch, " |   %-15s %3d/%-3d ",
		class_table[ability].przypadki[0], ch->skill_level[ability], max_level(ch, ability));
	else
	    ch_printf(ch, " |                           ");
	...
	{
	    default:
		ch_printf(ch, "      Charakter %s|" NL, (char *)strip_colors( align_info( ch ), 21 ) );	break;
	    case 0:
		ch_printf(ch, "      Si³a:             %-2d           |" NL, get_curr_str(ch) );	break;
	    case 1:
	    	ch_printf(ch, "      M±dro¶æ:          %-2d           |" NL, get_curr_wis(ch) );	break;
	    case 2:
		ch_printf(ch, "      Inteligencja:     %-2d           |" NL, get_curr_int(ch) );	break;
	    case 3:
		ch_printf(ch, "      Zrêczno¶æ:        %-2d           |" NL, get_curr_dex(ch) );	break;
	    case 4:
		ch_printf(ch, "      Kondycja:         %-2d           |" NL, get_curr_con(ch) );	break;
	    case 5:
		IS_IMMORTAL( ch ) ?
    ...
    if ( ch->top_level >= 15 )
    ch_printf(ch, " |   Hitroll:            %-6d    Damroll:           %-6d      |" NL,
	    GET_HITROLL(ch), GET_DAMROLL(ch) );
    ...
    {
    case POS_DEAD:
	ch_printf(  ch,   "MARTW%s !!!  ",
	      capitalize( SEX_SUFFIX_YAE( ch ) ) ); 		break;
    case POS_MORTAL:
	ch_printf( ch,   "Umieraj±c%s  ",
	      SEX_SUFFIX_YAE( ch ) ); 				break;
    case POS_INCAP:
	ch_printf( ch,   "Nieprzytomn%s",
	      SEX_SUFFIX_YAE( ch ) );				break;
    case POS_STUNNED:
	ch_printf( ch,   "Og³uszon%s   ",
	      SEX_SUFFIX_YAE( ch ) );				break;
    case POS_SLEEPING:
	ch_printf( ch,   "Sen           ");			break;
    case POS_RESTING:
	ch_printf( ch,   "Odpoczynek    ");			break;
    case POS_STANDING:
	ch_printf( ch,   "Stoj±ca       ");			break;
    case POS_SITTING:
	ch_printf( ch,   "Siedz±ca      ");			break;
    case POS_FIGHTING:
	ch_printf( ch,   "Walcz±ca      ");			break;
    case POS_MOUNTED:
	ch_printf( ch,   "Dosiadan%s   ",
	      SEX_SUFFIX_YAE( ch ) );				break;
    case POS_SHOVE:
	ch_printf( ch,   "Pchan%s      ",
	      SEX_SUFFIX_YAE( ch ) );				break;
    case POS_DRAG:
	ch_printf( ch,   "Ci±gniêt%s   ",
	      SEX_SUFFIX_YAE( ch ) );				break;
    }
	if( IS_IMMORTAL( ch ) )
		ch_printf(ch, " Ekwipunek: %10d  %10.1f kg   |" NL , ch->carry_number, ((float)ch->carry_weight)/10 );
	else
	    ch_printf(ch, " Ekwipunek: %5d/%-5d  %5.1f/%-5.1f kg |" NL,
	ch->carry_number, can_carry_n(ch), ((float)ch->carry_weight)/10, ((float)can_carry_w(ch))/10);

    sprintf( buf, "%-4d", GET_AC(ch) );
    ch_printf( ch, " |   Uzbrojenie:    %s           ",
		( ch->top_level >= 25 ) ?  buf : "    " );
...
DEF_DO_FUN( experience )
{
       ...
       for ( ability = 0 ; ability < MAX_ABILITY ; ability++ )
          if ( ability != FORCE_ABILITY || ch->skill_level[FORCE_ABILITY] > 1 )
        	ch_printf( ch, "%-15s   Poziom: %3d/%-3d       Postêp: %s" NL,
        	    capitalize(class_table[ability].przypadki[0]),
		    ch->skill_level[ability],
		    max_level( ch, ability ),
		    show_exp_stars( ch, ability) );
          else
...
DEF_DO_FUN( affected )
{
	...
	ch_printf( ch, COL_SCORE "%s" EOL,
	    flag_string( aff_flags_list, ch->affected_by )  );
        if ( ch->top_level >= 20 )
        {
            if ( ch->resistant > 0 )
	    {
                ...
                ch_printf( ch, COL_SCORE "%s" EOL,
		    flag_string(ris_flags_list,ch->resistant) );
	    }
            if ( ch->immune > 0 )
	    {
                send_to_char( FB_BLUE "Nietykalno¶æ:" EOL, ch);
                ch_printf( ch, COL_SCORE "%s" EOL,
		    flag_string(ris_flags_list,ch->immune) );
	    }
            if ( ch->susceptible > 0 )
	    {
                send_to_char( FB_BLUE "Podatno¶æ:" EOL, ch );
                ch_printf( ch, COL_SCORE "%s" EOL,
		    flag_string(ris_flags_list,ch->susceptible) );
	    }
        }
	return;
    }
	...
	for (paf = ch->first_affect; paf; paf = paf->next)
	    if ( (skill=get_skilltype(paf->type)) != NULL )
    	    {
    		    ...
    		    ch_printf( ch, COL_SCORE "%s: " FB_YELLOW "%s" COL_SCORE,
		        skill_tname[skill->type],
		        skill->name );

        	if ( ch->top_level >= 30 )
            	    ch_printf( ch, " zmienia %s",
		...
		if ( ch->top_level >= 20 )
            	    ch_printf( ch, " przez oko³o %d godzi%s",
			paf->duration/35,
			NUMBER_SUFF( paf->duration/35, "nê", "ny", "n" ) );
	   ...
	   if ( obj->wear_loc == iWear )
	   {
		...
		if ( can_see_obj( ch, obj ) )
		{
		    ...
		    switch ( obj->item_type )
			    ...
			    if (dam >= 10)      strcat( buf,
			            obj->gender==GENDER_MALE   ?" (nowiutki) " :
				    obj->gender==GENDER_FEMALE ?" (nowiutka) " :
								" (nowiutkie) " );
			    else if (dam >=  7) strcat( buf,
			            obj->gender==GENDER_MALE   ?" (dobry) " :
				    obj->gender==GENDER_FEMALE ?" (dobra) " :
								" (dobre) ");
 		            else if (dam >=  5) strcat( buf,
			            obj->gender==GENDER_MALE   ?" (u¿ywany) " :
				    obj->gender==GENDER_FEMALE ?" (u¿ywana) " :
								" (u¿ywane) ");
			    else if (dam >=  3) strcat( buf,
			            obj->gender==GENDER_MALE   ?" (zu¿yty) " :
				    obj->gender==GENDER_FEMALE ?" (zu¿yta) " :
								" (zu¿yte) ");
			    else if (dam >=  1) strcat( buf,
			            obj->gender==GENDER_MALE   ?" (okropny) " :
				    obj->gender==GENDER_FEMALE ?" (okropna) " :
								" (okropne) ");
			    else if (dam ==  0) strcat( buf, " (nieu¿ytek) ");
			    send_to_char( buf, ch );
	                    break;
				    ...
				    obj->gender==GENDER_FEMALE ?" (u¿ywana) " :
								" (u¿ywane) ");
                           else if (dam < 10)   strcat( buf,
			            obj->gender==GENDER_MALE   ?" (zu¿yty) " :
				    obj->gender==GENDER_FEMALE ?" (zu¿yta) " :
								" (zu¿yte) ");
                           else if (dam < 12)   strcat( buf,
				    obj->gender==GENDER_MALE   ?" (okropny) " :
				    obj->gender==GENDER_FEMALE ?" (okropna) " :
								" (okropne) ");
                           else if (dam ==  12) strcat( buf, " (nieu¿ytek) ");
			   send_to_char( buf, ch );
...
DEF_DO_FUN( title )
{
    ...
    ch_printf( ch, "Ok." NL "Od teraz jestes znan%s jako '%s %s" PLAIN "'." NL,
	SEX_SUFFIX_YAE( ch ), ch->name, ch->pcdata->title );
}
...
DEF_DO_FUN( report )
{
    ...
    ch_printf( ch,
	"Podajesz: %d/%d hp %d/%d mv." NL,
	ch->hit,  ch->max_hit,
	ch->move, ch->max_move   );


    sprintf( buf, "$n podaje: %d/%d hp %d/%d.",
	ch->hit,  ch->max_hit,
	ch->move, ch->max_move   );
