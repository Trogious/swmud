...
ROOM_INDEX_DATA		* room_index_hash		[MAX_KEY_HASH] = { 0 };
...
void load_helps_files()
{
	...
	sprintf( found_str, "...%d helps found in %d files.", top_help, top_helps_file );
...
void area_update( void )
{
	...
	reset_age = pArea->reset_frequency ? pArea->reset_frequency : DEF_RESETFREQ;
	...
	*/
	if ( pArea->nplayer > 0 && pArea->age == (reset_age-1) )
	{
		...
		strcpy( buf, NL "S³yszysz jakie¶ dziwne ³omotanie w oddali..." NL );
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	mob->max_hit = mob->top_level * 10 + number_range( mob->top_level ,
		mob->top_level * 10 );
...
DEF_DO_FUN( memory )
{
	...
	*/
	...
	if( !IS_CODER( ch->name ) )
	{
		send_to_pager( "You cannot use this command with 'proc' option." NL, ch );
		return;
	}
	...
	ch_printf( ch, "Helps(%2d) %5d    Resets    %5d" NL, top_helps_file, top_help, top_reset );
	ch_printf( ch, "IdxMobs   %5d    Mobs      %5d" NL, 	top_mob_index, nummobsloaded );
	ch_printf( ch, "IdxObjs   %5d    Objs      %5d (%d)" NL, 	top_obj_index, numobjsloaded, physicalobjects );
	ch_printf( ch, "IdxQuests %5d    RunQuests %5d" NL, 	top_quest_index, top_quest );
	...
	ch_printf( ch, "Players   %5d    Maxplrs   %5d" NL, 	num_descriptors, sysdata.maxplayers );
	ch_printf( ch, "MaxEver   %5d    Topsn     %5d (%d)" NL, 	sysdata.alltimemax, top_sn, MAX_SKILL );
...
char *str_str( const char *astr, char *bstr )
{
	...
	{
	if( c0 == LOWER(change_to_nopol( *ichar ))
	&& !str_prefix( astr, ichar ) )
		return ichar;
	}
...
if ( !str_cmp( name, "extradesc_prog"      ) )	return EXTRADESC_PROG;
	/* UWAGA !!! Dodane ze wzglêdu na kompatybilno¶æ z Envy !!! */
if ( !str_cmp( name, "singlehitprcnt_prog"	) )	return SPEECH_PROG;
	/* Thanos */
return( ERROR_PROG );
}
...
bool is_room_unlinked(RID *room)
{
	...
	IF_BUG( room->next!=NULL, "room probably not unlinked %d, %s", room->vnum, room->name)
		;
...
void fread_quest_header( QUEST_INDEX_DATA *quest, FILE *fp )
{
	...
	switch ( UPPER(word[0]) )
	{
		...
		if( !str_cmp( word, "Action" ) )
		{
			...
			LINK( action_cmd, quest->first_action, quest->last_action,next, prev );
			fMatch = true;
			break;
		}
		break;
		...
		if( !str_cmp( word, "Init" ) )
		{
			...
			LINK( init_cmd, quest->first_init_cmd, quest->last_init_cmd,next, prev );
			fMatch = true;
			break;
		}
		case 'M':
...
void fread_quest_chapter( QUEST_INDEX_DATA *quest, FILE *fp )
{
	...
	switch ( UPPER(word[0]) )
	{
		...
		if( !str_cmp( word, "Action" ) )
		{
			...
			CREATE( action_cmd, QUEST_ACTION_DATA, 1 );
			fread_action_cmd( action_cmd, fp );
			LINK( action_cmd, chapter->first_action, chapter->last_action,next, prev );
			fMatch = true;
			break;
		}
		case 'E':
			if ( !str_cmp( word, "End" ) )
		{
			LINK( chapter, quest->first_chapter, quest->last_chapter, next, prev );
			return;
		}
		if( !str_cmp( word, "Event" ) )
		{
			...
			LINK( event_cmd, chapter->first_event, chapter->last_event,next, prev );
			fMatch = true;
			break;
		}
		break;
		...
		if( !str_cmp( word, "Init" ) )
		{
			...
			CREATE( init_cmd, QUEST_CMND_DATA, 1 );
			fread_init_cmd( init_cmd, fp );
			LINK( init_cmd, chapter->first_init_cmd, chapter->last_init_cmd,next, prev );
			fMatch = true;
			break;
		}
		case 'T':
...
bool load_quest_file( char *filename )
{
	...
	if( !str_cmp( word, "CHAPTER" ) )
	{
		fread_quest_chapter( quest, fp );
		fMatch = true;
	}
	else
	if( !str_cmp( word, "ENDING" ) )
	{
		fread_quest_chapter( quest, fp );
		fMatch = true;
	}
...
DEF_DO_FUN( alist )
{
	...
	if (*arg1)
	{
		...
		if (cmd == "sort" || (cmd == "range" && (low = atoi(arg2)) > 0 && (high = atoi(argument)) > low))
		{
			sort = true;
		}
		else
	...
	for( i=1,area = sort ? first_asort : first_area; area; i++, area = sort ? area->next_sort : area->next )
	{
		if (area->lvnum < low || area->uvnum > high
				|| (name && SWString(area->name).toLower().find(arg2str) == SWString::npos)
				|| (author && SWString(area->author).toLower().find(arg2str) == SWString::npos)
				|| (builder && SWString(area->builder).toLower().find(arg2str) == SWString::npos)
				|| (file && SWString(area->filename).find(arg2str) == SWString::npos))
			continue;

		pager_printf( ch, FB_CYAN "[" PLAIN "%3d" FB_CYAN "][" PLAIN "%-20s" FB_CYAN "][" PLAIN "%-5d"
			FB_CYAN "-" PLAIN "%5d" FB_CYAN "][" PLAIN "%3d" FB_CYAN "]",
			i, strip_colors( area->name, 20 ),
			area->lvnum, area->uvnum, area->security );
		pager_printf( ch, "[" PLAIN "%-16s" FB_CYAN "]", strip_colors( area->builder, 16 ) );
		pager_printf( ch, "[" PLAIN "%-14s" FB_CYAN "]" EOL, strip_colors( area->filename, 14 ) );
	}
	...
	FOREACH( area, first_uarea )
	{
		...
		pager_printf( ch, "[%3d][%-20s][%-5d-%5d][%3d]",
			i++, strip_colors( area->name, 20 ),
			area->lvnum, area->uvnum, area->security );
...
DEF_DO_FUN( check_vnums )
{
	...
	for ( pArea = first_asort; pArea; pArea = pArea->next_sort )
	{
		...
		if ( ( low_range >= pArea->lvnum )
		&& ( low_range <= pArea->uvnum ) )
			area_conflict = true;

		if ( ( high_range <= pArea->uvnum )
		&& ( high_range >= pArea->lvnum ) )
			area_conflict = true;

		if(area_conflict)
		{
			if( !str_cmp( argument, "prototype" )
			&& !IS_SET( pArea->flags, AFLAG_PROTOTYPE ) )
				continue;

			sprintf(buf, "Conflict:%-15s| ",
				(pArea->filename ? pArea->filename : "(invalid)"));
			sprintf( buf2, "Vnums: %5d - %-5d" NL, pArea->lvnum,
				pArea->uvnum);
			strcat( buf, buf2 );
			send_to_pager(buf, ch);
		}
...
void copyover ( void )
{
	...
	*/
	...
	else
	{
		...
		d->descriptor, d->port, och->name, d->host, d->ipv6 ? 1 : 0);
		...
		sprintf( buf, "SYSTEM: authorized %s Bcoz of copyover.", och->name );
...
void copyover_recover( void )
{
	...
	for (;;)
	{
		...
		if (!fOld)
		{
			write_to_descriptor(desc, "Jakim¶ cudem twoja postaæ uleg³a zniszczeniu podczas rebootu." NL
			"Skontaktuj siê z w³adcami muda by j± odzyskaæ: mud@swmud.pl." NL, 0);
			close_socket(d, true);
		}
		else
		{
			...
			act(COL_FORCE, "$n materializuje siê tutaj!", d->character, NULL, NULL, TO_ROOM);
			...
			if (CH( d )->inquest)
			{
				...
				if ((questor = get_mob_index(CH( d )->inquest->pIndexData->questor)) != NULL)
					ch_printf(CH( d ), NL "                "
...
void reset_planet( PLANET_DATA *planet )
{
	...
	hours 	= planet->curr_season->day_length
		+ planet->curr_season->night_length;
	...
	if ( planet->hour <  5 ) 	planet->sunlight = SUN_DARK;
	else if ( planet->hour <  6 ) 	planet->sunlight = SUN_RISE;
	else if ( planet->hour < 19 ) 	planet->sunlight = SUN_LIGHT;
	else if ( planet->hour < 20 ) 	planet->sunlight = SUN_SET;
	else 				planet->sunlight = SUN_DARK;


	planet->windspeed 		= planet->curr_season->windspeed;
	planet->change		= 0;
	planet->mmhg		= planet->curr_season->mmhg;
	planet->mmhg  	       += ( number_range( 1,
					planet->curr_season->diff_mmhg * 2 ) -
	...
	planet->temperature        += ( number_range( 1,
		...
		if ( planet->mmhg <=  980 ) planet->sky = SKY_LIGHTNING;
	else if ( planet->mmhg <= 1000 ) planet->sky = SKY_RAINING;
	else if ( planet->mmhg <= 1020 ) planet->sky = SKY_CLOUDY;
	else                             planet->sky = SKY_CLOUDLESS;
...
void save_race2( RACE_DATA *race )
{
	...
	swNewProp( root, "xsi:schemaLocation", "http://swmud.pl/ns/swmud/1.0/race race.xsd" );
...
RACE_DATA *load_race( const char *filename )
{
	...
	FOREACH( node, root->children )
	{
		...
		if( !swXmlStrcmp( node->name, "short" ) )
		{
			FOREACH( child, node->children )
			{
				...
				if( !swXmlStrcmp( child->name, "inflect1" ) )
					swGetContent( &race->przypadki[1], child );
				else
				if( !swXmlStrcmp( child->name, "inflect2" ) )
					swGetContent( &race->przypadki[2], child );
				else
				if( !swXmlStrcmp( child->name, "inflect3" ) )
					swGetContent( &race->przypadki[3], child );
				else
				if( !swXmlStrcmp( child->name, "inflect4" ) )
					swGetContent( &race->przypadki[4], child );
				else
				if( !swXmlStrcmp( child->name, "inflect5" ) )
		...
		if( !swXmlStrcmp( node->name, "affected" ) )
			swGetContentInt64( &race->affected, node );
		else
		if( !swXmlStrcmp( node->name, "str_plus" ) )
			swGetContentInt( &race->str_plus, node );
		else
		if( !swXmlStrcmp( node->name, "int_plus" ) )
			swGetContentInt( &race->int_plus, node );
		else
		if( !swXmlStrcmp( node->name, "wis_plus" ) )
			swGetContentInt( &race->wis_plus, node );
		else
		if( !swXmlStrcmp( node->name, "dex_plus" ) )
			swGetContentInt( &race->dex_plus, node );
		else
		if( !swXmlStrcmp( node->name, "con_plus" ) )
			swGetContentInt( &race->con_plus, node );
		else
		if( !swXmlStrcmp( node->name, "cha_plus" ) )
			swGetContentInt( &race->cha_plus, node );
		else
		if( !swXmlStrcmp( node->name, "lck_plus" ) )
			swGetContentInt( &race->lck_plus, node );
		else
		if( !swXmlStrcmp( node->name, "frc_plus" ) )
			swGetContentInt( &race->frc_plus, node );
		else
		if( !swXmlStrcmp( node->name, "hp_plus" ) )
			swGetContentInt( &race->hp_plus, node );
		else
		if( !swXmlStrcmp( node->name, "force_plus" ) )
			swGetContentInt( &race->force_plus, node );
		else
		if( !swXmlStrcmp( node->name, "susceptible" ) )
			swGetContentInt64( &race->susceptible, node );
		else
		if( !swXmlStrcmp( node->name, "resistant" ) )
			swGetContentInt64( &race->resistant, node );
		else
		if( !swXmlStrcmp( node->name, "immune" ) )
			swGetContentInt64( &race->immune, node );
		else
		if( !swXmlStrcmp( node->name, "languages" ) )
		{
			FOREACH( child, node->children )
			{
				EONLY( child );
...
void save_languages()
{
	...
	xmlNewNs( root, BC"http://www.w3.org/2001/XMLSchema-instance", BC"xsi" );
	swNewProp( root, "xsi:schemaLocation", "http://swmud.pl/ns/swmud/1.0/languages languages.xsd" );
	xmlDocSetRootElement( doc, root );
...
void load_languages()
{
	...
	if( !(doc = xsd_validate( "languages", SCHEMA_LANGUAGES, RACE_DIR, "languages.xml" )) )
		return;
	...
	FOREACH( node, root->children )
	{
		...
		FOREACH( child, node->children )
		{
			...
			if( !swXmlStrcmp( child->name, "alphabet" ) )
				swGetContent( &lang->alphabet, child );
			else
			if( !swXmlStrcmp( child->name, "deny_text" ) )
				swGetContent( &lang->deny_text, child );
			else
			if( !swXmlStrcmp( child->name, "max_learn" ) )
				swGetContentInt( &lang->max_learn, child );
			else
			if( !swXmlStrcmp( child->name, "max_practice" ) )
				swGetContentInt( &lang->max_practice, child );
			else
			if( !swXmlStrcmp( child->name, "min_int" ) )
				swGetContentInt( &lang->min_int, child );
			else
			if( !swXmlStrcmp( child->name, "difficulty" ) )
