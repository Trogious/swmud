...
DEF_DO_FUN( track )
{
   ...
   switch(dir) {
	 ...
	 learn_from_failure( ch, gsn_track );
         break;
      default:
         ch_printf(ch, "Znajdujesz trop na %s st±d..." NL,
	    dir_where_name[dir] );
...
void found_prey( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	switch( number_bits( 2 ) )
 	{
	case 0: sprintf( buf, "Nie chcij bym ciê znalaz³%s %s!",
		    SEX_SUFFIX__AO( ch ), victname );
		do_say( ch, buf );
	        break;
	case 1: act( COL_ACTION, "$n wêszy po doko³a szukaj±c $N$1.", ch, NULL, victim, TO_NOTVICT );
		act( COL_ACTION, "Wêszysz doko³a szukaj±c $N$1.", ch, NULL, victim, TO_CHAR );
		act( COL_ACTION, "$n wêszy doko³a szukaj±c ciebie.", ch, NULL, victim, TO_VICT );
		sprintf( buf, "Czujê ju¿ zapach twojej krwi!!!" );
		do_say( ch, buf );
		break;
	case 2: sprintf( buf, "Rozszarpiê ciê na kawa³ki %s!", victname );
		do_yell( ch, buf );
		break;
	case 3: do_say( ch, (char *)"Czekaj! Niech tylko ciê znajdê...");
		break;
        }
	return;
     }
	...
	switch( number_bits( 2 ) )
	{
	case 0:	do_say( ch, (char *)"Wy³a¼ tchórzu!" );
		sprintf( buf, "%s to zakichany tchórz !!!", victname );
		do_yell( ch, buf );
		break;
	case 1: sprintf( buf, "Za³atwmy to na zewn±trz %s", victname );
		do_say( ch, buf );
		break;
	case 2: sprintf( buf, "%s to wymoczek i tchórz!", victname );
		do_yell( ch, buf );
		break;
	case 3: act( COL_ACTION, "$n popycha $N$3 prowokuj±co.", ch, NULL, victim, TO_NOTVICT );
		act( COL_ACTION, "Popychasz $N$3 prowokuj±co.", ch, NULL, victim, TO_CHAR );
		act( COL_ACTION, "$n popycha ciê prowokuj±co.", ch, NULL, victim, TO_VICT );
		break;
	}
	return;
     }
     ...
     switch( number_bits( 2 ) )
     {
     case 0: sprintf( buf, "Twoja krew jest ju¿ moja %s!", victname );
	     do_yell( ch, buf);
	     break;
     case 1: sprintf( buf, "Ha, wiêc znów siê spotykamy %s!", victname );
     	     do_say( ch, buf );
     	     break;
     case 2: sprintf( buf, "Jaki chcesz mieæ nagrobek, %s?", victname );
     	     do_say( ch, buf );
     	     break;
     case 3: act( COL_ACTION, "$n wyskakuje na $N$3 dos³ownie znik±d!", ch, NULL, victim, TO_NOTVICT );
	     act( COL_ACTION, "Wyskakujesz na $N$3 zaskakuj±c $I go zupe³nie.", ch, NULL, victim, TO_CHAR );
	     act( COL_ACTION, "$n wyskakuje na ciebie dos³ownie znik±d!", ch, NULL, victim, TO_VICT );
     }
     stop_hunting( ch );
...
/* hunting with snipe */
   ...
   else
   {
      ...
      if ( !ch->hunting )
      {
	IF_BUG( ch->in_room==NULL, "Mob #%d, name: %s. Placing mob in limbo.",
