...
DROP SEQUENCE IF EXISTS pdata_seq CASCADE;
CREATE SEQUENCE pdata_seq;
DROP TABLE IF EXISTS pdata CASCADE;
CREATE TABLE pdata (
    id int DEFAULT nextval('pdata_seq') PRIMARY KEY,
    ...
    st_bar smallint[3] DEFAULT '{0,0,0}' NOT NULL,
    outcast_time bigint DEFAULT 0 NOT NULL,
    restore_time bigint DEFAULT 0 NOT NULL,
    password varchar(89) NOT NULL, -- output of: encode(digest('a_password','sha512'),'base64')
...
DROP SEQUENCE IF EXISTS pchar_seq CASCADE;
CREATE SEQUENCE pchar_seq;
DROP TABLE IF EXISTS pchar CASCADE;
CREATE TABLE pchar (
    id int DEFAULT nextval('pchar_seq') UNIQUE,
    pdata_id int NOT NULL,
    primary key(id,pdata_id),
    foreign key(pdata_id) references pdata(id) on delete cascade,
    inflect varchar(16)[6] NOT NULL,
    ...
    mental_state smallint DEFAULT '-10' NOT NULL,
    bio text DEFAULT ''::text NOT NULL,
    addiction smallint[9] DEFAULT '{0,0,0,0,0,0,0,0,0}' NOT NULL,
    drug_level smallint[9] DEFAULT '{0,0,0,0,0,0,0,0,0}' NOT NULL,
    ...
    condition smallint[4] DEFAULT '{0,0,0,0}' NOT NULL,
    cloned timestamp
);


DROP SEQUENCE IF EXISTS ability_pchar_seq CASCADE; 
CREATE SEQUENCE ability_pchar_seq;
DROP TABLE IF EXISTS ability_pchar CASCADE;
...
CREATE TABLE fevent_pchar (
    ...
    attributes bigint[5] DEFAULT '{0,0,0,0,0}' NOT NULL,
    pchar_id int
);
