...
void violence_update(void)
{
	...
	for (ch = last_char; ch; lst_ch = ch, ch = gch_prev)
	{
		...
		if (!ch->in_room || !ch->name)
		{
			...
			sprintf(buf, "ch: %p  ch->in_room: %p  ch->prev: %p  ch->next: %p", ch, ch->in_room, ch->prev, ch->next);
			log_string( buf );

			log_string( lastplayercmd );
			if (lst_ch)
				sprintf(buf, "lst_ch: %p  lst_ch->prev: %p  lst_ch->next: %p", lst_ch, lst_ch->prev, lst_ch->next);
			else
    ...
    /*
     ...
     if ( dt == (TYPE_HIT + WEAPON_BLASTER ) && wield && wield->item_type == ITEM_WEAPON )
     {
		...
		if ( !IS_AFFECTED( victim, AFF_PARALYSIS ) )
		{
            ...
            {
               act( FB_BLUE, "Niebieskie pier¶cienie energii z blastera $N$1 trafiaj± ciê ale z mizernym efektem.", victim, NULL, ch, TO_CHAR );
	       act( FB_BLUE, "Niebieskie pier¶cienie energii z wtojego blastera trafiaj± $N$3, jednak nic siê nie dzieje!", ch, NULL, victim, TO_CHAR );
	       act( FB_BLUE, "Niebieskie pier¶cienie energii z blastera $N$1 trafiaj± $N$3, jednak nic siê nie dzieje!", ch, NULL, victim, TO_NOTVICT );
            ...
            )
     {
     	if ( wield->value[4] < 1  )
     	{
            act( FG_YELLOW, "Twoja force-pike wymaga na³adowania...", ch, NULL, victim, TO_CHAR    );
            dam /= 2;
     	}
     	else
     	    wield->value[4]--;
            ...
            )
     {
     	if ( wield->value[4] < 1  )
     	{
            act( FG_YELLOW, "$n wykonuje rêk± jakie¶ dziwne wymachy.",  ch, NULL, victim, TO_VICT    );
            act( FG_YELLOW, "Musisz na³adowaæ swój miecz ¶wietlny... brakuje mu ostrza.", ch, NULL, victim, TO_CHAR    );
     	    if ( IS_NPC(ch) )
     	    {
     	        do_remove( ch, wield->name );
     	    }
     ...
     else if ( dt == (TYPE_HIT + WEAPON_BOWCASTER ) && wield && wield->item_type == ITEM_WEAPON )
     {
     	if ( wield->value[4] < 1  )
     	{
            act( FG_YELLOW, "$n celuje swoj± kusz± w ciebie, ale nic siê nie dzieje.",  ch, NULL, victim, TO_VICT    );
            act( FG_YELLOW, "*CLICK* ... Twoja kusza nie ma amunicji!", ch, NULL, victim, TO_CHAR    );
            if ( IS_NPC(ch) )
     	    {
     	        do_remove( ch, wield->name );
     	    }
...
//byTrog
	...
	&& !IS_NPC( victim )
	&& ((lightsaber = get_eq_char( ch, WEAR_WIELD )) != NULL)
        && lightsaber->value[3] == WEAPON_BLASTER )
    {
	if( (int) victim->pcdata->learned[gsn_lightsabers] == 100 )
	{
	    act( PLAIN, "$n odbija mieczem strza³ z blastera.", victim, NULL, ch, TO_ROOM );
            act( PLAIN, "Odbijasz mieczem strza³ z blastera.", ch, NULL, victim, TO_VICT );
            act( PLAIN, "Odbity strza³ z blastera trafia ciê.", victim, NULL, ch, TO_VICT );
            act( PLAIN, "Odbity strza³ z blastera trafia $n$1.", ch, NULL, victim, TO_ROOM );
...
/* weapon spells	-Thoric */
    if ( wield
    &&  !IS_SET(victim->immune, RIS_FORCE)
    &&  !IS_SET(victim->in_room->room_flags, ROOM_NO_FORCE) )
    {
	AFFECT_DATA *aff;
...
//Trog: chwilowo?
	...
	if ( IS_AFFECTED(ch, AFF_INVISIBLE) || IS_AFFECTED(ch, AFF_FORCEINVIS) )
	{
	    ...
	    act( PLAIN, "$n pojawia siê jakby znik±d!", ch, NULL, NULL, TO_ROOM );
	}
	...
	if ( dt >= TYPE_HIT && ch->in_room == victim->in_room )
	{
	    ...
	    if ( IS_NPC(ch)
	    &&   ( IS_SET( ch->attacks, DFND_DISARM )|| number_percent()<30 )
	    &&   number_percent( ) <
		( leveldiff < -5 ? fight_lev(ch) / 2 : UMAX( 10, leveldiff ) )
		&& dam == 0)
	/*    &&   number_percent( ) < ch->skill_level[COMBAT_ABILITY] / 2 )*/
		disarm( ch, victim );

	    if ( (IS_NPC(ch) && number_percent( )
		< ( leveldiff < -5 ? fight_lev(ch) / 2 : UMAX( 20, leveldiff ) )
		&&   ( IS_SET( ch->attacks, ATCK_TRIP ) || number_percent()<30 )
		&& dam == 0)
	    && ( dt != ( TYPE_HIT + WEAPON_BLASTER ) ) )
...
*/
    ...
    if( victim->hit < 1 && IS_SET(victim->in_room->room_flags, ROOM_SAFEARENA) )//Trog
    {
    	...
    	act( FG_GREEN, "Twój pojedynek z $N$4 dobieg³ koñca. Jeste¶ zwyciêzc±.", ch, NULL, victim, TO_CHAR );
    	act( FG_RED, "Twój pojedynek z $n$4 dobieg³ koñca. Jeste¶ przegranym.", ch, NULL, victim, TO_VICT );
    	act( FG_YELLOW, "Pojedynek $n$1 z $N$4 zosta³ zakoñczony. $n jest zwyciêzc±.", ch, NULL, victim, TO_NOTVICT );
    	return rNONE;
    }
    ...
    case POS_INCAP:
	act( COL_DYING, "$n jest nieprzytomn$y. Umrze bez pomocy.",
	    victim, NULL, NULL, TO_ROOM );
	ch_printf( victim, FB_RED
    ...
    case POS_STUNNED:
        if ( !IS_AFFECTED( victim, AFF_PARALYSIS ) )
        {
	  act( COL_ACTION, "$n jest og³uszon$y, ale prawdopodobnie z tego wyjdzie.",
	    victim, NULL, NULL, TO_ROOM );
	  ch_printf( victim, FB_RED
	...
	if ( IS_NPC(victim) && IS_SET( victim->act, ACT_NOKILL )  )
	   act( PLAIN, "$n ucieka w pop³ochu cudem unikaj±c ¶mierci!", victim, 0, 0, TO_ROOM );
	else if ( IS_NPC(victim) && IS_SET( victim->act, ACT_DROID )  )
	   act( COL_DEAD, "$n EXPLODUJE na ma³e kawa³eczki!", victim, 0, 0, TO_ROOM );
	else
	   act( COL_DEAD, "$n UMIERA!!!", victim, 0, 0, TO_ROOM );
        ch_printf( victim, NL FB_WHITE "W£A¦NIE ZGIN%s¦ !!!" EOL NL,
		MALE( victim )   ? "¡£E" :
		FEMALE( victim ) ? "Ê£A" : "Ê£O" );
	break;

    default:
	if ( dam > victim->max_hit / 4 )
	{
	   act( COL_HURT, "Ough!!! To naprawdê BOLA£O !!!", victim, 0, 0, TO_CHAR );
	   if ( number_bits(3) == 0 )
	...
	{
	   act( COL_DANGER, "KRWAWISZ POTÊ¯NIE !!!",
		victim, 0, 0, TO_CHAR );
	   if ( number_bits(2) == 0 )
		worsen_mental_state( ch, 1 );
	}
        ...
        for ( obj = victim->first_carrying; obj; obj = obj_next )
	{
		...
		act( COL_ACTION, "$n odrzuca $p$3.", victim, obj, NULL, TO_ROOM );
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(victim) && victim->pcdata && victim->pcdata->hotel_safe_time > 0)
    {
...
void update_pos( CHAR_DATA *victim )
{
    ...
    if ( victim->mount )
    {
	act( COL_ACTION, "$n spada bezw³adnie z $N$1.",
		victim, NULL, victim->mount, TO_ROOM );
	REMOVE_BIT( victim->mount->act, ACT_MOUNTED );
	victim->mount = NULL;
    }
...
void set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    IF_BUG( ch->fighting, "%s -> %s (already fighting %s)",
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
	   sprintf( buf, "Zdobywasz %d punkt%s do¶wiadczenia w walce." NL, xp, NUMBER_SUFF(xp, "", "y", "ów") );
	   send_to_char( buf, gch );
	}
        ...
        {
           xp = URANGE( members, xp*members, (exp_level( gch->skill_level[LEADERSHIP_ABILITY]+1) - exp_level(gch->skill_level[LEADERSHIP_ABILITY] )/30) );
           sprintf( buf, "Zdobywasz %d punktów do¶wiadczenia przywódczego za poprowadzenie grupy do zwyciêstwa." NL, xp );
	   send_to_char( buf, gch );
	...
	for ( obj = ch->first_carrying; obj; obj = obj_next )
	{
	    ...
	    {
		act( COL_FORCE, "Zosta³$a¶ oparzon$y przez $p$3.", ch, obj, NULL, TO_CHAR );
		act( COL_FORCE, "$n zosta³$o oparzon$y przez $p$3.",   ch, obj, NULL, TO_ROOM );
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    exp = URANGE(1, xp, ( exp_level(  gch->skill_level[COMBAT_ABILITY]+1 ) - exp_level( gch->skill_level[COMBAT_ABILITY]) )   );
#if defined (ARMAGEDDON)
    return exp/5;
#else
    return exp/2;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    if ( victim->hit > 0 )
	 ...
	 if ( dam   <     0 )  { v = "leczy%s";			}
    else if ( dam   ==    0 )  { v = "chybia%s";		}
    else if ( dampc <=    1 )  { v = "muska%s";			}
//    else if ( dampc <=    2 )  { v = "ledwie drapie%s";		}
    else if ( dampc <=    2 )  { v = "drapie%s";		}
    else if ( dampc <=    4 )  { v = "trafia%s";		}
    else if ( dampc <=    8 )  { v = "lekko rani%s";		}
    else if ( dampc <=   15 )  { v = "kaleczy%s";		}
    else if ( dampc <=   30 )  { v = "rani%s";			}
    else if ( dampc <=   50 )  { v = "dotkliwie rani%s";	}
    else if ( dampc <=   70 )  { v = "powa¿nie rani%s";		}
    else if ( dampc <=   80 )  { v = "¶miertelnie rani%s";	}
    else                       { v = "ZABIJA%s";		}
    ...
    if ( dt == TYPE_HIT )
    {
	sprintf( buf1, "$n %s $N$3%c",  vp, punct );
	sprintf( buf2, "%s $N$3%c", 	vs, punct );
	sprintf( buf3, "$n %s ciê%c", 	vp, punct );
    }
    else//Added by Onyx
    if( dam == 0 )
    {
	...
	case 2:
	    ...
	    break;
	case 3:
	    sprintf( buf1, "$n o ma³o siê nie przewróci³$o próbuj±c trafiæ $N$3." );
	...
	sprintf( buf1, "Zatrute %s $n$1 %s $N$3%c", attack, vp, punct );
	sprintf( buf2, "Twoje zatrute %s %s $N$3%c", attack, vp, punct );
	sprintf( buf3, "Zatrute %s $n$1 %s ciê%c", attack, vp, punct );
    }
    else
    {
	if ( skill )
	{
		...
		{
		   act( COL_HITME, skill->miss_vict, ch, NULL, victim, TO_VICT );
		   found = true;
		}
		if ( skill->miss_room && skill->miss_room[0] != '\0' )
		{
		   act( COL_ACTION, skill->miss_room, ch, NULL, victim, TO_NOTVICT );
		   found = true;
		}
		...
		if ( skill->hit_room && skill->hit_room[0] != '\0' )
		  act( COL_ACTION, skill->hit_room, ch, NULL, victim, TO_NOTVICT );
	    }
	}
	else if ( dt >= TYPE_HIT
	...
	if( dt > TYPE_HIT )
	{
	    if( ch == victim )
	    {
		sprintf( buf1, "Lamerskie %s $n$1 %s $i%c", 	 attack, vp, punct );
		sprintf( buf2, "Twoje lamerskie %s %s Ciebie%c", attack, vp, punct );
	    }
	    else
	    {
		sprintf( buf1, "%s $n$1 %s $N$3%c",	 	attack, vp, punct );
		sprintf( buf2, "Twoje %s %s $N$3%c",  		attack, vp, punct );
		sprintf( buf3, "%s $n$1 %s Ciebie%c", 		attack, vp, punct );
	    }
	}
	else
	{
	    if( ch == victim )
	    {
		sprintf( buf1, "$n %s sam%s siebie %s%c",
			 vp, SEX_SUFFIX__AO( ch ), attack, punct );
		sprintf( buf2, "%s sam%s siebie %s%c",
			 vs, SEX_SUFFIX__AO( ch ), attack, punct );
	    }
	    else
	    {
		sprintf( buf1, "$n %s $N$3 %s%c",  vp, attack, punct );
		sprintf( buf2, "%s $N$3 %s%c", 	   vs, attack, punct );
		sprintf( buf3, "$n %s ciê %s%c",   vp, attack, punct );
	    }
	}
    }

    if ( !IS_NPC(ch) && IS_SET(ch->act, PLR_HOLYLIGHT) )
       sprintf( buf2, "%s Zadajesz cios o sile %d punktów.", buf2, dam);
...
DEF_DO_FUN( kill )
{
    ...
    {
	act( PLAIN, "Przecie¿ $N to twój uwielbiany mistrz.", ch, NULL, victim, TO_CHAR );
	return;
    }
...
DEF_DO_FUN( flee )
{
	...
	ch->in_room = now_in;
	act( COL_ACTION, "$n rozgl±da siê szukaj±c kryjówki.", ch, NULL, NULL, TO_ROOM );
...
bool get_cover( CHAR_DATA *ch )
{
	...
	ch->in_room = now_in;
	act( PLAIN, "$n rozgl±da siê szukaj±c kryjówki.", ch, NULL, NULL, TO_ROOM );
...
DEF_DO_FUN( slay )
{
    ...
    {
	  act( FG_YELLOW, "Tworzysz niewyobra¿alnie du¿± kulê ognia i ciskasz ni± w $N$3!",  ch, NULL, victim, TO_CHAR    );
      act( FG_YELLOW, "$n ciska w ciebie kulê ognia!", ch, NULL, victim, TO_VICT    );
      act( FG_YELLOW, "$n ciska w stronê $N$3 kulê ognia i spala go ¿ywcem.",  ch, NULL, victim, TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "shatter" ) )
    {
	  act( FB_CYAN, "Zamra¿asz wzrokiem $N$3 po czym jednym szybkim ciosem rozpierdalasz go na kawa³eczki!",  ch, NULL, victim, TO_CHAR    );
      act( FB_CYAN, "$n zamra¿a Ciê wzorkiem, po czym jednym szybkim ciosem rozpierdala Ciê na kawa³eczki!", ch, NULL, victim, TO_VICT    );
      act( FB_CYAN, "$n zamra¿a wzrokiem $N$3 po czym jednym szybkim ciosem rozpierdala go na kawa³eczki!!",  ch, NULL, victim, TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "demon" ) )
    {
      act( FB_WHITE, "You gesture, and a slavering demon appears.  With a horrible grin, the",  ch, NULL, victim, TO_CHAR );
      act( FB_WHITE, "foul creature turns on $N, who screams in panic before being eaten alive.",  ch, NULL, victim, TO_CHAR );
      act( FB_WHITE, "$n gestures, and a slavering demon appears.  The foul creature turns on",  ch, NULL, victim, TO_VICT );
      act( FB_WHITE, "you with a horrible grin.   You scream in panic before being eaten alive.",  ch, NULL, victim, TO_VICT );
      act( FB_WHITE, "$n gestures, and a slavering demon appears.  With a horrible grin, the",  ch, NULL, victim, TO_NOTVICT );
      act( FB_WHITE, "foul creature turns on $N, who screams in panic before being eaten alive.",  ch, NULL, victim, TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "pounce" ) && get_trust(ch) >= LEVEL_ASCENDANT )
    {
      act( FG_RED, "Leaping upon $N with bared fangs, you tear open $S throat and toss the corpse to the ground...",  ch, NULL, victim, TO_CHAR );
      act( FG_RED, "In a heartbeat, $n rips $s fangs through your throat!  Your blood sprays and pours to the ground as your life ends...", ch, NULL, victim, TO_VICT );
      act( FG_RED, "Leaping suddenly, $n sinks $s fangs into $N's throat.  As blood sprays and gushes to the ground, $n tosses $N's dying body away.",  ch, NULL, victim, TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "slit" ) && get_trust(ch) >= LEVEL_ASCENDANT )
    {
      act( FG_RED, "You calmly slit $N's throat.", ch, NULL, victim, TO_CHAR );
      act( FG_RED, "$n reaches out with a clawed finger and calmly slits your throat.", ch, NULL, victim, TO_VICT );
      act( FG_RED, "$n calmly slits $N's throat.", ch, NULL, victim, TO_NOTVICT );
    }

	else if ( !str_cmp( arg2, "decapit" ) )
	{
	  act( FG_RED, "Z przyjemno¶ci± ogl±dasz, jak rêce $N$3 odrywaj± siê od jego cia³a i ³ami± $N$2 kark jak zapa³kê.",  ch, NULL, victim, TO_CHAR    );
      act( FG_RED, "Twoje w³asne rêce odrywaj± siê od cia³a i ³ami± Ci kark jak zapa³kê. Silny by³e¶...", ch, NULL, victim, TO_VICT    );
      act( FG_RED, "Od cia³a $N$3 nagle odrywaj± siê rêce a nastêpnie ³ami± $N$2 kark jak zapa³kê...",  ch, NULL, victim, TO_NOTVICT );
	}

    else
    {
      act( FB_WHITE, "Za¿ynasz $N$3 z zimn± krwi±!",  ch, NULL, victim, TO_CHAR    );
      act( FB_WHITE, "$n za¿yna ciê z zimn± krwi±!", ch, NULL, victim, TO_VICT    );
      act( FB_WHITE, "$n b³yskawicznie dobywa swego miecza swietlnego i rozcina $N$3 na dwie czêsci!",  ch, NULL, victim, TO_NOTVICT );
    }
