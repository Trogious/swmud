...
const char * 	name_expand    		args( (CHAR_DATA *ch  ) );
...
void	add_material_warehouse ( PLANET_DATA * planet,
...
const char * const corpse_descs[] =
	{
	"Martwe cia³o %s rozk³ada siê tutaj.",
...
const char * const d_corpse_descs[] =
	{
	"Zmasakrowane czê¶ci %s rdzewiej± tutaj.",
...
int max_level( CHAR_DATA *ch, int ability)
{
	...
	if ( ability == COMBAT_ABILITY )
	{
		...
		if ( ch->main_ability == HUNTING_ABILITY )	level = 20;
		if ( IS_RACE( ch, "Wookiee" ) )			level += 20;
		if ( IS_RACE( ch, "Noghri" ) )			level += 15;
		if ( IS_RACE( ch, "Gamorrean" ) )		level += 10;
		if ( IS_RACE( ch, "Defel" ) )			level += 15;
		if ( IS_RACE( ch, "Trandoshan" ) )		level += 15;
		if ( IS_RACE( ch, "Duinuogwuin" ) )		level += 20;
		if ( IS_RACE( ch, "Yuuzhan Vong" ) )		level += 50;
		if ( IS_RACE( ch, "Aqualish" ) )		level +=  5;
		if ( IS_RACE( ch, "Barabel" ) )			level += 20;
		if ( IS_RACE( ch, "Iktotchi" ) )		level += 10;
		level += ch->perm_con + ch->perm_dex + ch->perm_str;
	}

	if ( ability == PILOTING_ABILITY )
	{
		if ( ch->main_ability == ENGINEERING_ABILITY )  level = 30;
		if ( ch->main_ability == HUNTING_ABILITY )      level = 20;
		if ( ch->main_ability == SMUGGLING_ABILITY )    level = 40;
		if ( ch->main_ability == FORCE_ABILITY )        level = 25;
		if ( IS_RACE( ch, "Hutt" ) )                    level -= 50;
		if ( IS_RACE( ch, "Noghri" ) )                  level -= 15;
		if ( IS_RACE( ch, "Gamorrean" ) )               level -= 10;
		if ( IS_RACE( ch, "Quarren" ) )                 level += 10;
		if ( IS_RACE( ch, "Duinuogwuin" ) )             level -= 30;
		if ( IS_RACE( ch, "Yuuzhan Vong" ) )            level += 50;
		if ( IS_RACE( ch, "Barabel" ) )                 level -= 10;
		if ( IS_RACE( ch, "Devaronian" ) )              level += 10;
		if ( IS_RACE( ch, "Duros" ) )                   level += 25;
		if ( IS_RACE( ch, "Sullustan" ) )               level += 20;
		if ( IS_RACE( ch, "Iktotchi" ) )                level += 15;
		level += ch->perm_dex*2;
	}
	...
	if ( ability == ENGINEERING_ABILITY )
	{
		...
		if ( IS_RACE( ch, "Wookiee" ) )                 level += 20;
		if ( IS_RACE( ch, "Rodian" ) )                  level -= 10;
		if ( IS_RACE( ch, "Hutt" ) )                    level -= 20;
		if ( IS_RACE( ch, "Mon Calamari" ) )            level += 10;
		if ( IS_RACE( ch, "Gamorrean" ) )               level -= 20;
		if ( IS_RACE( ch, "Verpine" ) )                 level += 50;
		if ( IS_RACE( ch, "Trandoshan" ) )              level -= 10;
		if ( IS_RACE( ch, "Chadra-Fan" ) )              level += 20;
		if ( IS_RACE( ch, "Yuuzhan Vong" ) )            level -= 50;
		if ( IS_RACE( ch, "Bothan" ) )                  level += 10;
		if ( IS_RACE( ch, "Duros" ) )                   level += 15;
		if ( IS_RACE( ch, "Sullustan" ) )               level += 20;
		if ( IS_RACE( ch, "Iktotchi" ) )		level -= 15;
		level += ch->perm_int * 2;
	}
	...
	{
		if ( IS_RACE( ch, "Wookiee" ) )                 level += 10;
		if ( IS_RACE( ch, "Rodian" ) )                  level += 40;
		if ( IS_RACE( ch, "Twi'lek" ) )                 level += 10;
		if ( IS_RACE( ch, "Hutt" ) )                    level -= 20;
		if ( IS_RACE( ch, "Jawa" ) )                    level -= 10;
		if ( IS_RACE( ch, "Adarian" ) )                 level -= 30;
		if ( IS_RACE( ch, "Trandoshan" ) )              level += 20;
		if ( IS_RACE( ch, "Chadra-Fan" ) )              level -= 25;
		if ( IS_RACE( ch, "Yuuzhan Vong" ) )            level += 50;
		if ( IS_RACE( ch, "Barabel" ) )                 level += 10;
		if ( IS_RACE( ch, "Bith" ) )                    level -= 15;
		if ( IS_RACE( ch, "Duros" ) )                   level -= 15;
		if ( IS_RACE( ch, "Gran" ) )                    level += 10;
		if ( IS_RACE( ch, "Ithorian" ) )                level -= 10;
		if ( IS_RACE( ch,  "Sullustan" ) )              level -= 10;
		level += ch->perm_wis;
	}
	...
	if ( ability == SMUGGLING_ABILITY )
	{
		...
		if ( IS_RACE( ch, "Twi'lek" ) )                 level += 30;
		if ( IS_RACE( ch, "Mon Calamari" ) )            level -= 20;
		if ( IS_RACE( ch, "Jawa" ) )                    level += 30;
		if ( IS_RACE( ch, "Adarian" ) )                 level -= 20;
		if ( IS_RACE( ch, "Verpine" ) )                 level -= 20;
		if ( IS_RACE( ch, "Defel" ) )                   level += 25;
		if ( IS_RACE( ch, "Quarren" ) )                 level += 20;
		if ( IS_RACE( ch, "Chadra-Fan" ) )              level += 10;
		if ( IS_RACE( ch, "Duinuogwuin" ) )             level -= 10;
		if ( IS_RACE( ch, "Aqualish" ) )                level += 10;
		if ( IS_RACE( ch, "Bith" ) )                    level += 10;
		if ( IS_RACE( ch, "Bothan" ) )                  level += 15;
		if ( IS_RACE( ch, "Devaronian" ) )              level += 15;
	...
	if ( ability == LEADERSHIP_ABILITY )
	{
		...
		if ( IS_RACE( ch, "Twi'lek" ) )                 level -= 25;
		if ( IS_RACE( ch, "Rodian" ) )                  level -= 20;
		if ( IS_RACE( ch, "Hutt" ) )                    level += 50;
		if ( IS_RACE( ch, "Mon Calamari" ) )            level += 30;
		if ( IS_RACE( ch, "Jawa" ) )                    level -= 10;
		if ( IS_RACE( ch, "Adarian" ) )                 level += 10;
		if ( IS_RACE( ch, "Trandoshan" ) )              level -= 10;
		if ( IS_RACE( ch, "Quarren" ) )                 level -= 10;
		if ( IS_RACE( ch, "Yuuzhan Vong" ) )            level += 50;
		if ( IS_RACE( ch, "Cerean" ) )                  level += 20;
		if ( IS_RACE( ch, "Iktotchi" ) )                 level += 10;
	...
	if ( ability == DIPLOMACY_ABILITY )
	{
		...
		if ( ch->main_ability == LEADERSHIP_ABILITY )   level = 35;
		if ( IS_RACE( ch, "Wookiee" ) )                 level -= 50;
		if ( IS_RACE( ch, "Twi'lek" ) )                 level -= 25;
		if ( IS_RACE( ch, "Rodian" ) )                  level -= 20;
		if ( IS_RACE( ch, "Hutt" ) )                    level -= 10;
		if ( IS_RACE( ch, "Mon Calamari" ) )            level += 10;
		if ( IS_RACE( ch, "Noghri" ) )                  level -= 50;
		if ( IS_RACE( ch, "Gamorrean" ) )               level -= 10;
		if ( IS_RACE( ch, "Jawa" ) )                    level -= 10;
		if ( IS_RACE( ch, "Adarian" ) )                 level += 40;
		if ( IS_RACE( ch, "Verpine" ) )                 level -= 10;
		if ( IS_RACE( ch, "Defel" ) )                   level -= 30;
		if ( IS_RACE( ch, "Trandoshan" ) )              level -= 30;
		if ( IS_RACE( ch, "Quarren" ) )                 level -= 20;
		if ( IS_RACE( ch, "Duinuogwuin" ) )             level += 20;
		if ( IS_RACE( ch, "Yuuzhan Vong" ) )            level -= 10;
		if ( IS_RACE( ch, "Aqualish" ) )                level -= 10;
		if ( IS_RACE( ch, "Arcona" ) )                  level += 10;
		if ( IS_RACE( ch, "Bith" ) )                    level += 25;
		if ( IS_RACE( ch, "Cerean" ) )                  level += 10;
		if ( IS_RACE( ch, "Gotal" ) )                   level += 30;
		if ( IS_RACE( ch, "Zabrak" ) )                  level -= 10;
		level += ch->perm_cha*3;
	}
...
int hit_gain( CHAR_DATA *ch )
{
	...
	if ( (get_age( ch ) > 800) && !IS_IMMORTAL( ch ) )
	{
		ch_printf( ch, "Jeste¶ ju¿ star%s." NL
				...
				"Nied³ugo umrzesz." NL,
				SEX_SUFFIX_YAE( ch ), SEX_SUFFIX_YAE( ch ) );
		return -10;
	}
...
void gain_addiction( CHAR_DATA *ch )
{
	...
	for ( drug=0 ; drug <= 9 ; drug ++ )
	{
		...
		{
			ch_printf( ch,
				"Czujesz, ¿e za chwilê umrzesz. POTRZEBUJESZ %s." NL,
		...
		{
			ch_printf( ch,
				"Potrzebujesz %s." NL,spice_types_list[drug] );
			worsen_mental_state( ch, 2 );
		}
	...
	if( ch->pcdata->condition[iCond] == 0 )
	{
		...
		case COND_THIRST:
			if ( ch->top_level <= LEVEL_AVATAR )
			{
				...
				act( COL_THIRSTY, "$n umiera z pragnienia!", ch, NULL, NULL, TO_ROOM);
				worsen_mental_state( ch, 2 );
				retcode = damage(ch, ch, 5, TYPE_UNDEFINED);
			}
			break;
	...
	if( ch->pcdata->condition[iCond] == COND_MAX/16 )
	{
		...
		case COND_THIRST:
			if ( ch->top_level <= LEVEL_AVATAR  )
			{
				...
				worsen_mental_state( ch, 1 );
				act( COL_THIRSTY, "$n wygl±da jakby potrzebowa³$o czego¶ do picia.", ch,
					 NULL, NULL, TO_ROOM);
			}
			break;
...
void mobile_update( void )
{
	...
	for ( ch = last_char; ch; ch = gch_prev )
	{
		...
		if ( ch->pIndexData->vnum == 5 && !IS_AFFECTED(ch, AFF_CHARM) )
		{
			...
			act(COL_FORCE, "$n obraca siê w proch i wraca tam, sk±d przyby³$o.",
		...
		&&   number_bits( 2 ) == 0 )
		{
			...
			if ( obj_best )
			{
				...
				act( COL_ACTION, "$n podnosi $p$3.", ch, obj_best, NULL, TO_ROOM );
			}
		}
	...
	FOREACH( pPlanet, first_planet )
	{
		if ( pPlanet->first_deposit)
		{
			FOREACH( pDeposit, pPlanet->first_deposit )
			{
				...
				add_material_warehouse( pPlanet,
					pMat, pDeposit->daily_mining/24);
			}
			save_planet( pPlanet );
		}
	}
...
void weather_update( void )
{
	...
	for( pPlanet = first_planet; pPlanet; pPlanet = pPlanet->next )
	{
		...
		*/
		...
		pPlanet->mmhg = UMAX( pPlanet->mmhg, pPlanet->curr_season->mmhg
					- pPlanet->curr_season->diff_mmhg );
		pPlanet->mmhg = UMIN( pPlanet->mmhg, pPlanet->curr_season->mmhg
					+ pPlanet->curr_season->diff_mmhg );
		...
		if( IS_SET(ch->in_room->room_flags, ROOM_BACTA) )
		{
			if( ch->hit < ch->max_hit )
    	    {
				...
				if (ch->hit > ch->max_hit)
					ch->hit = ch->max_hit;
				ch_printf( ch,
					"Czujesz, ¿e twoje rany goj± siê du¿o "
			...
			if( ch->move < ch->max_move )
			{
				...
				if (ch->move > ch->max_move)
					ch->move = ch->max_move;
				ch_printf( ch,
					"Aromat bacty dzia³a na ciebie koj±co,"
		...
		*/
		...
		if ( !char_died(ch) )
		{
			...
			*/
			if ( IS_AFFECTED(ch, AFF_POISON) )
			{
				act( COL_POISON, "$n trzêsie siê i cierpi.",
					 ch, NULL, NULL, TO_ROOM );
				act( COL_POISON, "Trzêsiesz siê i cierpisz.",
			...
			if ( ch->mental_state >= 30 )
				switch( (ch->mental_state+5) / 10 )
				{
					case  3:
						...
						act( COL_ACTION, "$n nie wygl±da najlepiej.",
						ch, NULL, NULL, TO_ROOM );
						break;
					case  4:
						send_to_char( "¬le siê czujesz." NL, ch );
						act( COL_ACTION, "$n ¼le siê czuje.",
						ch, NULL, NULL, TO_ROOM );
						break;
					case  5:
					...
					case  7:
						...
						act( COL_ACTION, "$n wygl±da jakby traci³$o poczucie rzeczywisto¶ci.",
						ch, NULL, NULL, TO_ROOM );
						break;
					case  8:
					...
					case  9:
						...
						act( COL_ACTION, "$n be³kocze co¶ o 'odpowiedzi', 'JEDYNYM' i"
								" jakie¶ tam mumbo-jumbo...",
						ch, NULL, NULL, TO_ROOM );
						break;
					case 10:
						send_to_char( "Czujesz, ¿e koniec jest ju¿ bliski." NL, ch );
						act( COL_ACTION, "$n poci siê strasznie i wije w konwulsjach...",
						ch, NULL, NULL, TO_ROOM );
						break;
				}
...
void halucinations( CHAR_DATA *ch )
{
	...
	{
	    default:
		case  1: t = "Energia ciê rozpiera. Nie mo¿esz usiedzieæ w jednym miejscu." NL;		break;
		case  2: t = "£askoczesz wszystkich dooko³a." NL;			break;
		case  3: t = "Czujesz jak dreszcze przechodz± twoje cia³o." NL;	break;
		case  4: t = "Nagle zdajesz sobie sprawê, ze jednak wcale nie jest tak weso³o..." NL;	break;
		case  5: t = "Hej! Ci tam ¶miej± siê z ciebie" NL;			break;
		case  6: t = "S³yszysz p³acz. To twoja mama..." NL;			break;
		case  7: t = "By³$a¶ ju¿ tutaj?" NL " Nie." NL "   Tak!" NL " Nie..." NL;	break;
		case  8: t = "Bolesne wspomnienia z dzieciñstwa migaj± w twojej g³owie" NL;	break;
		case  9: t = "S³yszysz jakby kto¶ wo³a³ twoje imiê z daleka..." NL;	break;
		case 10: t = "G³owa ci pêka, nie mo¿esz siê na niczym skupiæ.";	break;
		case 11: t = "Ziemia... Ona chyba siê rozstêpuje!" NL;		break;
		case 12: t = "Nie bardzo wiesz co jest prawd±, a co nie." NL;	break;
		case 13: t = "To wszystko jest snem. A mo¿e nie jest?" NL;		break;
		case 14: t = "Oni nadchodz±. Oni s± coraz bli¿ej. Widz± ciê." NL;	break;
		case 15: t = "MOC wype³nia ca³e twoje cia³o!" NL;			break;
		case 16: t = "Jeste¶ lekk$z jak piórko... niebo stoi dla ciebie otworem." NL;	break;
		case 17: t = "Ca³e twoje ¿ycie mignê³o ci przed oczyma. O, a teraz twoja przysz³o¶æ." NL;	break;
		case 18: t = "Jeste¶ wszêdzie i wszystkim. Znasz wszystko i wszystkich." NL;	break;
		case 19: t = "Czy jeste¶ nie¶mierteln$y?" NL;			break;
		case 20: t = "Ahhh... W³adza Absolutna... Co zrobiæ... Co zrobiæ?" NL;break;
		}
...
void auth_update( void )
{
	...
	if ( (victim = d->character) && IS_WAITING_FOR_AUTH(victim) )
	{
		...
		sprintf( buf, "\r %s@%s new %s", victim->name,
		victim->desc->host, victim->race->name);
		to_channel( buf, CHANNEL_MONITOR, "Monitor", 101);
	}
	}
...
void update_handler( void )
{
	...
	if( timechar )
	{
		...
		ch_printf( timechar, "Timing took %d.%06d seconds." NL,
...
void reboot_check( time_t reset )
{
	...
	if ( new_boot_time_t <= current_time )
	{
		...
		/* Added by Thanos */
		sysdata.downtype =  sysdata.reboot_type == REB_COPYOVER ? DOWN_COPYOVER :
		...
		if( (IS_ORG( clan ) || IS_SUBORG( clan )) && clan->repay_date != 0 )
		{
			...
			if( clan->repay_date < current_time )
			{
				...
				FOREACH( d, first_descriptor )
					if( (ch = d->original) && ch->pcdata
						&& ch->pcdata->clan && ch->pcdata->clan == clan )
					{
						ch->pcdata->clan = NULL;
						save_char_obj( ch );
					}
...
void update_rat()
{
	...
	{
		exec_command_in_loop( rat_loop.rOwner,
			owner->in_room,
			rat_loop.command,
			itoa(rat_loop.rRoom) );
		rat_loop.rRoom++;
