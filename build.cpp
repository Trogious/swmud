...
bool can_rmodify( CHAR_DATA *ch, ROOM_INDEX_DATA *room )
{
	...
	{
	  send_to_char( "You must have an assigned area to modify this room." NL, ch );
	  return false;
	}
	if ( vnum >= pArea->lvnum
	&&   vnum <= pArea->uvnum )
...
bool can_omodify( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if ( !ch->pcdata || !(pArea=ch->pcdata->area) )
    {
      send_to_char( "You must have an assigned area to modify this object." NL, ch );
      return false;
    }
...
bool can_mmodify( CHAR_DATA *ch, CHAR_DATA *mob )
{
	...
	if ( !ch->pcdata || !(pArea=ch->pcdata->area) )
	{
	  send_to_char( "You must have an assigned area to modify this mobile." NL, ch );
	  return false;
	}
	if ( vnum >= pArea->lvnum
	&&   vnum <= pArea->uvnum )
...
DEF_DO_FUN( goto )
{
	...
	if ( get_trust( ch ) < sysdata.level_modify_proto )
	{
	    if ( !ch->pcdata || !(pArea=ch->pcdata->area) )
	    {
		send_to_char( "You must have an assigned area to create rooms." NL, ch );
		return;
	    }
	    if ( vnum < pArea->lvnum
	    ||   vnum > pArea->uvnum )
	...
	||   ch->in_room->vnum > pArea->uvnum ) && !IS_SET(ch->in_room->room_flags , ROOM_HOTEL) )
	{
	      send_to_char( "Builders can only use goto from a hotel or in their zone." NL, ch );
	      return;
	}
    ...
    if ( !IS_SET(ch->act, PLR_WIZINVIS) )
    {
         if (ch->pcdata && ch->pcdata->bamfout[0] != '\0')
               ...
               act( FB_WHITE, "$n $T", ch, NULL, "znika w zawirowaniu MOCY.",  TO_ROOM );
    }
   ...
   if ( !IS_SET(ch->act, PLR_WIZINVIS) )
	...
	if (ch->pcdata && ch->pcdata->bamfin[0] != '\0')
             ...
             act( COL_IMMORT, "$n $T", ch, NULL, "pojawia siê tu a Moc wiruje doko³a.",  TO_ROOM );
    }
...
DEF_DO_FUN( mset )
{
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Intelligence range is %d to %d." NL, minattr, maxattr );
	    return;
	}
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Constitution range is %d to %d." NL, minattr, maxattr );
	    return;
	}
	...
	if ( value < -30 || value > 30 )
	{
	    send_to_char( "Saving throw range vs bad breath is -30 to 30." NL, ch );
	    return;
	}
	...
	if ( value < -30 || value > 30 )
	{
	    send_to_char( "Saving throw range vs force powers is -30 to 30." NL, ch );
	    return;
	}
	...
	return;
      ...
      {
	send_to_char(
	    "New password must be at least five characters long." NL, ch );
	return;
      }
	...
	{
	    send_to_char(
		"New password not acceptable, try again." NL, ch );
	    return;
	}
      }
    ...
    if ( !str_cmp( arg2, "clan" ) )
    {
		...
		if( !get_member( clan, victim->name ) )
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "sentinal, scavenger, aggressive, stayarea, wimpy, practice, immortal," NL, ch );
	   send_to_char( "deadly, mountable, guardian, nokill, scholar, noassist, droid, nocorpse," NL, ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
	   ...
	   else
	   {
	     if ( IS_NPC(victim) && value == ACT_IS_NPC )
	       send_to_char( "If that could be changed, it would cause many problems." NL, ch );
	     else
	     if ( IS_NPC(victim) && value == ACT_POLYMORPHED )
	       send_to_char( "Changing that would be a _bad_ thing." NL, ch);
	     else
	     {
		if ( pcflag )
	...
	{
	    send_to_char( "You can't set security greater than You have!" NL, ch );
	    return;
	}
	...
	while ( argument[0] != '\0' )
	{
	   ...
	   if( !get_planet( arg3 ) )
		ch_printf( ch, "!!! Warning !!! There is no such planet!\n\rLeave it if you're sure about that." NL);
	...
	if ( !argument || argument[0] == '\0' )
	{
	   send_to_char( "Usage: mset <victim> affected <flag> [flag]..." NL, ch);
	   return;
	}
	while ( argument[0] != '\0' )
	{
	...
	if ( !argument || argument[0] == '\0' )
	{
	   send_to_char( "Usage: mset <victim> resistant <flag> [flag]..." NL, ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
	...
	if ( !IS_NPC( victim ) && get_trust( ch ) < LEVEL_LESSER )
	{
	    send_to_char( "You can only modify a mobile's susceptibilities." NL, ch );
	    return;
	}
	if ( !can_mmodify( ch, victim ) )
	    return;
	if ( !argument || argument[0] == '\0' )
	{
	    send_to_char( "Usage: mset <victim> susceptible <flag> [flag]..." NL, ch );
	    return;
	}
	while ( argument[0] != '\0' )
	{
	...
	if ( !argument || argument[0] == '\0' )
	{
	    ...
	    send_to_char( "bite          claws        tail        sting      punch        kick" NL, ch );
	    send_to_char( "trip          bash         stun        gouge      backstab" NL, ch );
	    return;
	}
	while ( argument[0] != '\0' )
	{
	...
	if ( !argument || argument[0] == '\0' )
	{
	    send_to_char( "Usage: mset <victim> defense <flag> [flag]..." NL, ch );
...
*/
	...
	if ( !str_cmp( arg2,"kin") && !str_cmp( arg3, "clear") )
	{
		...
		while( (_friend = victim->last_known) != NULL)
		{
			...
			UNLINK(_friend,victim->first_known,victim->last_known,next,prev);
			DISPOSE(_friend);
		}
...
DEF_DO_FUN( oset )
{
	...
	{
	   send_to_char( "There is nothing like that in all the realms." NL, ch );
	   return;
 	}
    }
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Possible Types:" NL, ch );
           ...
           send_to_char( "Treasure    Armor      Comlink    Fabric      Grenade" NL, ch );
           send_to_char( "Furniture   Trash      Container  Drink_con   Landmine" NL, ch );
           send_to_char( "Key         Food       Money      Pen         Fuel" NL, ch );
           send_to_char( "Fountain    Pill       Weapon     Medpac      Missile" NL, ch );
           send_to_char( "Fire        Book       Superconductor         Rare_metal" NL, ch );
           send_to_char( "Switch      Lever      Button     Dial        Government" NL, ch );
           send_to_char( "Trap        Map        Portal     Paper       Magnet" NL, ch );
           send_to_char( "Lockpick    Shovel     Thread     Smut        Ammo" NL, ch );
           send_to_char( "Rawspice    Lens       Crystal    Duraplast   Battery" NL, ch );
           send_to_char( "Toolkit     Durasteel  Oven       Mirror      Circuit" NL, ch );
           send_to_char( "Potion      Salve      Pill       Device      Shipmodule" NL, ch );
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "glow, dark, force, bless, antievil, noremove, antisith, antisoldier," NL, ch );
           send_to_char( "donation, covering, hum, invis, nodrop, antigood, antipilot, anticitizen" NL, ch );
           send_to_char( "antineutral, inventory, antithief, antijedi, clanobject, antihunter" NL, ch );
	   send_to_char( "small_size, human_size, large_size, hutt_size, contraband, personal" NL, ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
	...
	if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
	  ...
	  send_to_char( "Item must have prototype flag to set this value." NL, ch );
	return;
    }
	...
	if ( !*arg2 || !*argument )
	{
	   send_to_char( "Usage: oset <object> affect <field> <value>" NL, ch );
           ...
           send_to_char( "none        strength    dexterity   intelligence  wisdom       constitution" NL, ch );
           send_to_char( "sex         level       age         height        weight       force" NL, ch );
           send_to_char( "hit         move        credits     experience    armor        hitroll" NL, ch );
           send_to_char( "damroll     save_para   save_rod    save_poison   save_breath  save_power" NL, ch );
           send_to_char( "charisma    resistant   immune      susceptible   affected     luck" NL, ch );
           send_to_char( "backstab    pick        track       steal         sneak        hide" NL, ch );
           send_to_char( "detrap      dodge       peek        scan          gouge        search" NL, ch );
           send_to_char( "mount       disarm      kick        parry         bash         stun" NL, ch );
           send_to_char( "punch       climb       grip        scribe        brew" NL, ch );
	   return;
	}
	...
	if ( !*arg3  )
	{
	    send_to_char( "Syntax: oset <object> ed <keywords>" NL,
		ch );
	    return;
	}
	CHECK_SUBRESTRICTED( ch );
	if ( obj->timer )
	{
	   send_to_char("It's not safe to edit an extra description on an object with a timer.\n\rTurn it off first." NL, ch );
	   return;
	}
	if ( obj->item_type == ITEM_PAPER )
	{
	   send_to_char("You can not add an extra description to a note paper at the moment." NL, ch);
	   return;
	}
	if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
	...
	if ( obj->item_type == ITEM_PAPER )
	{
	   send_to_char("You can not add a description to a note paper at the moment." NL, ch);
	   return;
	}
	if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
	...
	case ITEM_WEAPON:
		...
		if ( value < 0 )
		{
		    ...
		    send_to_char( "\n\rChoices:" NL, ch );
		    send_to_char( "   none, lightsaber, vibro-blade, blaster, force pike, bowcaster, bludgeon" NL, ch );
		    return;
		}
		tmp = 3;
		break;
	...
	case ITEM_CRYSTAL:
		...
		if ( value < 0 )
		{
		    ...
		    send_to_char( "\n\rChoices:" NL, ch );
		    send_to_char( "   non-adegan, kathracite, relacite, danite, mephite, ponite, illum, corusca" NL, ch );
		    return;
		}
		tmp = 0;
		break;
...
char *sprint_reset( CHAR_DATA *ch, RESET_DATA *pReset, int num, bool rlist )
{
    ...
    {
	default:
	  sprintf( buf, "%2d) *** BAD RESET: %c %d %lld %lld %lld ***" NL,
	  			num,
	  			pReset->command,
	  			pReset->extra,
	  			pReset->arg1,
	...
	case 'M':
	  ...
	  sprintf( buf, "%2d) %s (%lld) -> %s (%lld) [%lld]" NL,
	  			num,
	  			mobname,
	  			pReset->arg1,
	  			roomname,
	...
	case 'E':
	  ...
	  sprintf( buf, "%2d) %s (%lld) -> %s (%s) [%lld]" NL,
	  			num,
	  			objname,
	  			pReset->arg1,
	  			mobname,
	...
	case 'G':
	  ...
	  if ( (obj = get_obj_index( pReset->arg1 )) == NULL )
	      ...
	      strcpy( objname, obj->name );
	  sprintf( buf, "%2d) %s (%lld) -> %s (carry) [%lld]" NL,
	  			num,
	  			objname,
	  			pReset->arg1,
	  			mobname,
	...
	case 'O':
	  ...
	  if ( !room )
	      ...
	      strcpy( roomname, room->name );
	  sprintf( buf, "%2d) (object) %s (%lld) -> %s (%lld) [%lld]" NL,
	  			num,
	  			objname,
	  			pReset->arg1,
	  			roomname,
	...
	case 'P':
	  ...
	  sprintf( buf, "%2d) (Put) %s (%lld) -> %s (%lld) [%lld]" NL,
	  			num,
	  			objname,
	  			pReset->arg1,
	  			roomname,
	...
	case 'D':
	  ...
	  sprintf( buf, "%2d) %s [%lld] the %s [%lld] door %s (%lld)" NL,
	  			num,
	  			mobname,
	  			pReset->arg3,
	  			objname,
	...
	case 'R':
	  if ( (room = get_room_index( pReset->arg1 )) == NULL )
		...
		strcpy( roomname, room->name );
	  sprintf( buf, "%2d) Randomize exits 0 to %lld -> %s (%lld)" NL,
	  			num,
	  			pReset->arg2,
	  			roomname,
	  			pReset->arg1 );
	  break;
	case 'T':
	  sprintf( buf, "%2d) TRAP: %d %lld %lld %lld (%s)" NL,
	  		num,
			pReset->extra,
			pReset->arg1,
			pReset->arg2,
...
/*Trog*/send_to_char( "  rlist exdistance nightdesc rmnightdesc" NL, 	ch );
	...
	if ( argument[0] == '\0' )
	{
	   send_to_char( "Set the room name.  A very brief single line room description." NL, ch );
	...
	if ( !argument || argument[0] == '\0' )
	{
	   send_to_char( "Set the maximum characters allowed in the room at one time. (0 = unlimited)." NL, ch );
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: rset flags <flag> [flag]..." NL, ch );
           ...
           send_to_char( "private, safe, petshop, norecall, donation, nodropall, silence," NL, ch );
           send_to_char( "logspeach, nodrop, clanstoreroom, plr_home, empty_home, teleport" NL, ch );
           send_to_char( "nofloor, prototype, refinery, factory, republic_recruit, empire_recruit" NL, ch );
           send_to_char( "spacecraft, auction, no_drive, can_land, can_fly, hotel, shipdevice" NL, ch );
           return;
	}
	while ( argument[0] != '\0' )
	{
	   ...
	   if ( value < 0 )
	     ch_printf( ch, "Unknown flag: %s" NL, arg2 );
	       ...
	       send_to_char( "If you want to build a player home use the 'empty_home' flag instead." NL, ch );
	   else
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: rset sector <value>" NL, ch );
           ...
           send_to_char( "0:dark, 1:city, 2:field, 3:forest, 4:hills, 5:mountain, 6:water_swim" NL, ch );
           send_to_char( "7:water_noswim, 8:underwater, 9:air, 10:desert, 11:unkown, 12:oceanfloor, 13:underground" NL, ch );
	...
	if ( !xit )
	{
	   send_to_char( "No exit in that direction.  Use 'rset exit ...' first." NL, ch );
	   return;
	}
	xit->key = value;
	send_to_char( "Done." NL, ch );
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: rset exflags <dir> <flag> [flag]..." NL, ch );
           send_to_char( "\n\rExit flags:" NL, ch );
           send_to_char( "isdoor, closed, locked, can_look, searchable, can_leave, can_climb," NL, ch );
           send_to_char( "nopassdoor, secret, pickproof, fly, climb, dig, window, auto, can_enter" NL, ch );
	...
	if ( !*arg2 )
	{
	    ...
	    send_to_char( "Usage: rset exit <dir> [room] [flags] [key] [keywords]" NL, ch );
	    return;
	}
	...
	if ( addexit || !xit )
	{
	    ...
	    {
		send_to_char( "There is already an exit in that direction leading to that location." NL, ch );
		return;
	    }
	    ...
	    xit->key			= -1;
	    xit->orig_flags		= 0;
	    act( COL_IMMORT, "$n reveals a hidden passage!", ch, NULL, NULL, TO_ROOM );
	}
	else
	    act( COL_IMMORT, "Something is different...", ch, NULL, NULL, TO_ROOM );
	if ( xit->to_room != tmp )
	{
	    xit->to_room = tmp;
	    xit->vnum = evnum;
...
//	    xit->orig_flags = atoll( arg3 ); To jakies zjebane bylo, kto by chcial ustawiac flagi jako liczbe o.O
        send_to_char( "Sorry, orig_flags weren't changed. Use redit like a normal builder." NL, ch );
	if ( argument && argument[0] != '\0' )
	{
	...
	if ( !*arg2 )
	{
	    ...
	    send_to_char( "Usage: rset bexit <dir> [room] [flags] [key] [keywords]" NL, ch );
	    return;
	}
	numnotdir = false;
	switch( arg2[0] )
	...
	if ( !*arg2 )
	{
	   send_to_char( "Set the distance (in rooms) between this room, and the destination room." NL, ch );
...
void adv_save_mobiles( AREA_DATA *tarea, FILE *fpout, bool install )
{
	...
	fprintf( fpout,	"%s@%s@%s@%s@%s@%s~\n",
	    pMobIndex->przypadki[0], pMobIndex->przypadki[1],
	    pMobIndex->przypadki[2], pMobIndex->przypadki[3],
	    pMobIndex->przypadki[4], pMobIndex->przypadki[5]		);
	...
	||  pMobIndex->resistant || pMobIndex->immune || pMobIndex->susceptible
	...
	|| pMobIndex->hitsizedice||pMobIndex->hitplus || pMobIndex->damnodice
	|| pMobIndex->damsizedice||pMobIndex->damplus )
	{
	    fprintf( fpout, "T %d %d ",	pMobIndex->mobthac0,
					pMobIndex->ac			);
...
void vsave_resets( FILE * fp, AREA_DATA * pArea )
{
	...
	if ( ( pRoomIndex = get_room_index( vnum ) ) )
	{
	    ...
	    {
		for ( pReset = pRoomIndex->first_reset; pReset; pReset = pReset->next )
		{
...
*/
	...
	fprintf( fpout, "%s~\n",	pObjIndex->name			);
	fprintf( fpout,	"%s@%s@%s@%s@%s@%s~\n",
	    pObjIndex->przypadki[0], pObjIndex->przypadki[1],
	    pObjIndex->przypadki[2], pObjIndex->przypadki[3],
	    pObjIndex->przypadki[4], pObjIndex->przypadki[5]		);
	...
	switch ( pObjIndex->item_type )
	{
		...
		fprintf( fpout, "%d~ %d~ %d~ %s~ %d~ %d~\n",
		    ...
		    val1,
		    val2,
		    IS_VALID_SN(val3) ? skill_table[val3]->name : " ",
		    val4,
		    val5 );
	    break;
	case ITEM_SALVE:
	...
	fprintf( fpout, "        %2d %2d %5lld  ; %s\n",
		 ...
		 pShop->close_hour,
		 pShop->flags,
		 pMobIndex->przypadki[0] );//changed by Thanos (short_descr/przypadki[0])
    }
    fprintf( fpout, "0\n\n\n" );
...
DEF_DO_FUN( foldarea )
{
	...
	if( ch->desc && (ed=ch->desc->olc_editing) )
	{
		...
		default:
		    break;
		case CON_MEDITOR: case CON_MPEDITOR:
		    tarea = get_vnum_area( ((MOB_INDEX_DATA *) ed)->vnum);
		    break;
		case CON_OEDITOR: case CON_OPEDITOR:
		    tarea = get_vnum_area( ((OBJ_INDEX_DATA *) ed)->vnum);
		    break;
		case CON_REDITOR: case CON_RPEDITOR:
...
RESET_DATA *parse_reset( ROOM_INDEX_DATA *pRoom, char *argument, CHAR_DATA *ch )
{
	...
	if ( arg1[0] == '\0' )
	{
	   send_to_char( "Reset commands: mob obj give equip door rand trap hide." NL, ch );
	   return NULL;
	}
	...
	if ( !str_cmp( arg1, "door" ) )
	{
	    ...
	    if ( val3 < 0 || val3 > 2 )
	    {
		send_to_char( "Reset: DOOR: invalid door state (0 = open, 1 = close, 2 = lock)" NL, ch );
		return NULL;
	    }
	    letter = 'D';
	    value = val3;
	...
	if ( !str_cmp( arg1, "trap" ) )
	{
	    ...
	    {
		send_to_char( "Reset: TRAP: Must specify room OR object, not both!" NL, ch );
		return NULL;
	    }
...
DEF_DO_FUN( astat )
{
	...
	if ( tarea->high_economy )
	   ...
	   ch_printf( ch, "Area economy: %d credits." NL,
			tarea->low_economy );
        if ( tarea->planet )
	   ch_printf( ch, "Planet: %s." NL,
			tarea->planet->name );
	ch_printf( ch,
		FG_CYAN "Mdeaths:" PLAIN " %d  "
		FG_CYAN "Mkills:" PLAIN " %d  "
		FG_CYAN "Pdeaths:" PLAIN " %d  "
		FG_CYAN "Pkills: " PLAIN "%d" NL,
			tarea->mdeaths,
			tarea->mkills,
			tarea->pdeaths,
			tarea->pkills );
    ...
    ch_printf( ch, FG_CYAN "Vnums: " PLAIN "%5d " FG_CYAN "-" PLAIN " %d" NL,
			tarea->lvnum,
			tarea->uvnum );
    ch_printf( ch, FG_CYAN "Level range: " PLAIN "%d " FG_CYAN "-" PLAIN " %d" NL,
			tarea->low_range,
			tarea->high_range );
    ch_printf( ch, FG_CYAN "Resetmsg:" PLAIN " %s" EOL,
	    *tarea->resetmsg ? tarea->resetmsg
						: "(default)" ); /* Rennard */
    ch_printf( ch, FG_CYAN "Reset frequency: " PLAIN "%d" FG_CYAN " minutes." EOL,
