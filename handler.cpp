			...
			if (!IS_NPC( xch ) && nifty_is_name(obj->armed_by, xch->name))
			{
				if (obj->carried_by)
				{
					...
					buf << SWInt::toString(obj->pIndexData->vnum) << " (" << nocolor(obj->przypadki[0]) << ") armed by "
							<< obj->armed_by << ", carried by " << obj->carried_by->name << " exploded in room "
							<< SWInt::toString(room->vnum) << " (" << nocolor(room->name) << ").";
				}
				else if (obj->in_room)
				{
					room = obj->in_room;
					...
					buf << SWInt::toString(obj->pIndexData->vnum) << " (" << nocolor(obj->przypadki[0]) << ") armed by "
							<< obj->armed_by << " exploded in room " << SWInt::toString(room->vnum)
							<< " (" << nocolor(room->name) << ").";
				}
				else
				{
					...
					buf << SWInt::toString(obj->pIndexData->vnum) << " (" << nocolor(obj->przypadki[0]) << ") armed by "
							<< obj->armed_by << " exploded in NULL room.";
				}
				logExplosion(buf);
...
void room_explode_1( OBJ_DATA *obj , CHAR_DATA *xch, ROOM_INDEX_DATA *room , int blast )
{
	...
	if ( IS_SET( room->room_flags, BFS_MARK ) || IS_SET( room->room_flags, ROOM_SAFE ) )
		return;

	buf << "shockwave of " << SWInt::toString(obj->pIndexData->vnum) << " (" << nocolor(obj->przypadki[0]) << ") armed by "
			<< obj->armed_by << " passes through room " << SWInt::toString(room->vnum)
			<< " (" << nocolor(room->name) << ").";
	logExplosion(buf);
	...
	for ( rch = room->first_person ; rch ;  rch = rnext )
	{
		...
		switch( obj->value[3] )
		{
			default:
				act( FB_WHITE,
					"Fala uderzeniowa olbrzymiej eksplozji przechodzi przez twoje cia³o!",
			...
			case GRANADE_POISON:
			{
				...
				act( FB_WHITE,
					"Fala uderzeniowa olbrzymiej eksplozji truj±cego gazu"
					" przechodzi przez twoje cia³o!", rch, obj, NULL, TO_CHAR );
				if (blast >1)
					dam = number_range ( obj->value[0]/blast , obj->value[0]/(blast-1) );
				else
					dam = obj->value[0];
				...
				rch->mental_state = URANGE( 20, rch->mental_state
								+ (first ? 5 : 0), 100 );
			}
				break;
			case GRANADE_STUN:
				act( FB_WHITE,
					"Huk o niezwykle wysokiej czêstotliwo¶ci wwierca siê w twój mózg!",
					rch, obj, NULL, TO_CHAR );
				if (blast >1)
					dam = number_range ( obj->value[0]/blast , obj->value[0]/(blast-1) );
				else
				...
				if ( !IS_AFFECTED( rch, AFF_PARALYSIS )
				|| rch->top_level > obj->level )
				{
					af.type			= gsn_stun;
			...
			case GRANADE_BLIND:
				act( FB_WHITE,
				"B³ysk jasny jak wybuch supernovej o¶lepia ciê!", rch, obj, NULL, TO_CHAR );
				if (blast >1)
					dam = number_range ( obj->value[0]/blast , obj->value[0]/(blast-1) );
				else
					dam = obj->value[0];
	...
	for ( robj = room->first_content; robj; robj = robj_next )
	{
		...
		if( robj != obj
			&&  robj->item_type != ITEM_SHIPMODULE
		...
		// tylko dynamit odpala sie rekursywnie -- Thanos:
			if( ( robj->item_type == ITEM_GRENADE
			||  robj->item_type == ITEM_LANDMINE )
			&& obj->value[3] == GRANADE_EXPLOSIVE )
			{
			...
			else
			{
				...
				{
				if( robj->item_type == ITEM_DRINK_CON
					||  robj->item_type == ITEM_FOOD
					||  robj->item_type == ITEM_FOUNTAIN )
					robj->value[3] = 1;
				}
			}
		}
		...
		for ( pexit = room->first_exit; pexit; pexit = pexit->next )
		{
				...
				&&  pexit->to_room != room )
			{
				if ( blast <= obj->value[1] )
				{
					...
					if (!IS_SET( pexit->to_room->room_flags, BFS_MARK))
...
if ( IS_NPC(ch) && IS_SET(ch->act, ACT_QUEST) )
{
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	*/
	...
	if ( !IS_NPC(ch) )
		...
		URANGE( 0, ch->pcdata->condition[COND_FULL] + mod, COND_MAX );
	break;

	case APPLY_THIRST:
	if ( !IS_NPC(ch) )
		ch->pcdata->condition[COND_THIRST] =
		URANGE( 0, ch->pcdata->condition[COND_THIRST] + mod, COND_MAX );
	break;
...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	&&   skill->type == SKILL_SPELL )
		if( (skill->spell_fun
		&& (retcode=(*skill->spell_fun) ( mod, ch->skill_level[FORCE_ABILITY], ch, ch )) == rCHAR_DIED)
		|| char_died(ch) )
			return;
	break;
...
/* skill apply types	-Thoric */
	...
	ch->pcdata->learned[gsn_snipe] =
		UMAX( 1, ch->pcdata->learned[gsn_snipe] + mod );
	break;
	case APPLY_PICK:
	...
	*/
	...
	{
		depth++;
		act( COL_ACTION, "Nie masz ju¿ si³y by d¼wigaæ $p$3.",
		ch, wield, NULL, TO_CHAR );
		act( COL_ACTION, "$n przestaje u¿ywaæ $p$3.", ch, wield, NULL, TO_ROOM );
...
void obj_personalize( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	if ( loading_char != ch )
		ch_printf( ch,
	"Czujesz, ¿e pokocha³%s¶ %s, ju¿ nigdy siê nie rozstaniecie!" NL,
		SEX_SUFFIX_EAE( ch ),
		obj->przypadki[3] );
...
OBJ_DATA *obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch )
{
	...
	if( str_cmp( obj->owner_name, ch->name ) )
	{
		...
		sprintf( buf, "%s" PLAIN " upad%s na ziemiê.",
		obj->przypadki[0], obj->gender > GENDER_FEMALE ? "aj±" : "a" );
	...
	if ( !IS_SET(extra_flags, ITEM_FORCE) )
	ch->carry_weight	+= oweight;
	/* Thanos: questy */
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
	...
	{
	bug( "already equipped (%d) (ch=%s)(obj=%s)(inRoom=%d).",
		iWear, ch->name, obj->name, ch->in_room->vnum );
	return;
	}
	...
	*/
	if ( loading_char != ch )
	{
	act( COL_FORCE, "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( COL_FORCE, "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	}
	if ( obj->carried_by )
	obj_from_char( obj );
...
void extract_obj( OBJ_DATA *obj )
{
	...
	if( obj_content->inquest )
	{
		...
		if( room  )	obj_to_room( obj_content, room );
		else
		if( carry )	obj_to_char( obj_content, carry );
		else
		if( inobj )	obj_to_obj ( obj_content, inobj );
		else /* na wszelki... */
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
	...
	act( COL_FORCE, "$n wy³ania siê z wiruj±cej mg³y!", ch, NULL, NULL, TO_ROOM );
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	for ( rch = ch->in_room->first_person; rch; rch = rch->next_in_room )
  {
    if (IS_NPC(ch) || IS_NPC(rch) || does_knows(ch, rch) )
      ...
      name = format_char_attribute(rch,0);
    if ( can_see( ch, rch )&&
         ( (nifty_is_name( arg, name) ||
             (IS_NPC(rch) && vnum == rch->pIndexData->vnum)
           ) ||
           ( rch->pcdata && rch != ch && IS_AFFECTED(rch,AFF_DISGUISE) &&
        		 nifty_is_name_prefix( arg, rch->pcdata->fake_name )
           )
         )
       )
	  {
...
CHAR_DATA *get_char_world( CHAR_DATA *ch, char *argument )
{
	...
	for ( wch = ch->in_room->first_person; wch; wch = wch->next_in_room )
  {
    if (IS_NPC(ch) || IS_NPC(wch) || does_knows(ch, wch) )
      ...
      name = format_char_attribute(wch,0);
    if (  (  nifty_is_name( arg, name) ||
...
CHAR_DATA *get_player_world( CHAR_DATA *ch, char* argument )
{
	...
	if( number == 1 )
	{
		FOREACH( d, first_descriptor )
		{
			...
			if( ( ch  )
			&& !str_cmp(arg,
				does_knows( ch, wch ) ? wch->name :
				format_char_attribute( wch, 0 )
			) )
				return wch;
		}
	}
...
bool ms_find_obj( CHAR_DATA *ch )
{
	...
	*/
	...
	{
		default:
		case  1: t="Zaraz zaraz. Ale co to mia³o byæ?" NL;				break;
		case  2: t="Ju¿ ju¿ to masz, ale... co¶ ciê powstrzyma³o." NL;		break;
		case  3: t="Ju¿ siêgasz, ale co to? To siê rusza!" NL;			break;
		case  4: t="Hmmm... jako¶ nie mo¿esz tego utrzymaæ w rêkach.NL ";		break;
		case  5: t="Ju¿ to masz, a potem nie masz..." NL;				break;
		case  6: t="Nie mo¿esz jako¶ staæ prosto by to zrobiæ!" NL;			break;
		case  7: t="£aaa! To jest ca³e we krwi! Pfuj! Bleh!" NL;			break;
		case  8: t="Wow... Ale masz jazdê!" NL;					break;
		case  9: t="W miarê jak po to siêgasz, dostrzegasz, ¿e na twojej d³oni co¶ wyrasta! Co to?" NL;break;
		case 10: t="Obserwujesz jak wszystko doko³a ciebie zamienia siê w olbrzymie stado maleñkich robaków!" NL;break;
		case 11: t="A co powiesz temu wielkiemu droidowi stoj±cemu za twoimi plecami?" NL;break;
		case 12: t="Bu! Co to by³o ???" NL;						break;
		case 13: t="Ale przecie¿ teraz trzymasz w d³oni losy wszech¶wiata. Od³ó¿ je najpierw gdzie¶." NL;break;
		case 14: t="Za bardzo siê boisz..." NL;					break;
		case 15: t="Twoja mama karci ciê mówi±c 'Nie ma mowy!'" NL;			break;
		case 16: t="Co? Ale o co chodzi?" NL;					break;
		case 17: t="Rezygnujesz, gdy tylko widzisz, ¿e to ¿yje! I b³aga ciê o lito¶æ!" NL;break;
		case 18: t="A co z tym milionem ma³ych robaków pa³aszuj±cych twoje ramiê?!?!" NL;break;
		case 19: t="Eee tam, to ju¿ niewa¿ne. Znasz odpowied¼ na ka¿de pytanie!" NL;	break;
		case 20: t="Ehh. Przecie¿ w³adca wszech¶wiata nie musi zajmowaæ siê takimi b³ahostkami." NL;break;
	}
	else
	{
	...
	{
		default:
		case  1: t="Tak Tak, ju¿ momencik..." NL;				break;
		case  2: t="Jako¶ nie mo¿esz tego znale¼æ, a mo¿e mo¿esz..." NL;	break;
		case  3: t="To poza twoimi aspiracjami..." NL;			break;
		case  4: t="...ale czym jest jaka¶ tam g³upia rzecz w porównaniu..." NL;break;
		case  5: t="Ju¿ to masz... Masz? Nie masz." NL;			break;
		case  6: t="Które?!? Tu s± dwa... nie, trzy..." NL;			break;
	}
	}
	send_to_char( t, ch );
	return true;
...
OBJ_DATA *find_obj( CHAR_DATA *ch, char *argument, bool carryonly )
{
	...
	&&    IS_SET(container->value[1], CONT_CLOSED) )
	{
		...
		act( PLAIN, "$d jest zamkniêta.", ch, NULL, container->name, TO_CHAR );
		else if( container->gender == GENDER_NEUTRAL )
		act( PLAIN, "$d jest zamkniête.", ch, NULL, container->name, TO_CHAR );
		else
		act( PLAIN, "$d s± zamkniête.", ch, NULL, container->name, TO_CHAR );
		return NULL;
	}

	obj = get_obj_list( ch, arg1, container->first_content );
	if ( !obj )
		act( PLAIN, IS_OBJ_STAT(container, ITEM_COVERING) ?
		"Nie widzê nic takiego pod $p$4." :
		"Nie widzê nic takiego w $p$5.",
...
bool room_is_dark( ROOM_INDEX_DATA *pRoomIndex )
{
	...
	&& (pRoomIndex->area->planet->sunlight == SUN_SET
	||  pRoomIndex->area->planet->sunlight == SUN_DARK) )
	return true;
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	if (!IS_NPC( ch ) && IS_NPC( victim ) && IS_SET( victim->act, ACT_PROTOTYPE ) && !IS_OLCMAN( ch ) && get_trust(ch)
			< 103)
		return false;
	...
	if (!IS_NPC( victim ) && IS_SET( victim->act, PLR_WIZINVIS ) && get_trust(ch) < get_trust(victim) && get_trust(ch)
	...
	if (!IS_NPC( ch ))
	{
		if (IS_NPC( victim ) && victim->inquest && victim->inquest != ch->inquest && get_trust(ch) < LEVEL_QUESTSEE) /* no tam jeden wyj±tek ;) */
			return false;
	}
	...
	if (!ch)
	{
		if (IS_AFFECTED(victim, AFF_INVISIBLE) || IS_AFFECTED(victim, AFF_HIDE) || IS_AFFECTED(victim, AFF_FORCEINVIS)
				|| IS_SET(victim->act, PLR_WIZINVIS))
			return false;
		else
			return true;
...
char *item_type_name( OBJ_DATA *obj )
{
	...
	{
	default:		sprintf( buf, "nieznany" 	); break;
	case ITEM_GRENADE:	sprintf( buf, "granat" 		); break;
	case ITEM_LIGHT:	sprintf( buf, "¶wiat³o" 	); break;
	case ITEM_WEAPON:	sprintf( buf, "broñ" 		); break;
	case ITEM_MISSILE:	sprintf( buf, "pocisk" 		); break;
	case ITEM_TREASURE:	sprintf( buf, "skarb" 		); break;
	case ITEM_ARMOR:    	sprintf( buf, "zbroja" 		); break;
	case ITEM_POTION:	sprintf( buf, "mikstura" 	); break;
	case ITEM_FURNITURE:	sprintf( buf, "mebel" 		); break;
	case ITEM_TRASH:	sprintf( buf, "¶mieæ" 		); break;
	case ITEM_CONTAINER:	sprintf( buf, "pojemnik"	); break;
	case ITEM_DRINK_CON:	sprintf( buf, "pojemnik na napój");break;
	case ITEM_KEY:		sprintf( buf, "klucz"		); break;
	case ITEM_FOOD:		sprintf( buf, "jedzenie"	); break;
	case ITEM_MONEY:	sprintf( buf, "pieni±dze"	); break;
	case ITEM_PEN:		sprintf( buf, "pisak"		); break;
	case ITEM_CORPSE_NPC:	sprintf( buf, "cia³o"		); break;
	case ITEM_CORPSE_PC:	sprintf( buf, "cia³o"		); break;
	case ITEM_FOUNTAIN:	sprintf( buf, "fontanna"	); break;
	case ITEM_PILL:		sprintf( buf, "pigu³ka"		); break;
	case ITEM_SCRAPS:	sprintf( buf, "resztki"		); break;
	case ITEM_SWITCH:	sprintf( buf, "prze³±cznik"	); break;
	case ITEM_LEVER:	sprintf( buf, "d¼wignia"	); break;
	case ITEM_BUTTON:	sprintf( buf, "przycisk"	); break;
	case ITEM_DIAL:		sprintf( buf, "komunikator"	); break;//?????
	case ITEM_TRAP:		sprintf( buf, "pu³apka"		); break;
	case ITEM_MAP:		sprintf( buf, "mapa"		); break;
	case ITEM_PAPER:	sprintf( buf, "papier"		); break;
	case ITEM_LOCKPICK:	sprintf( buf, "wytrych"		); break;
	case ITEM_FUEL:		sprintf( buf, "paliwo"		); break;
	case ITEM_AMMO:		sprintf( buf, "amunicja"	); break;
	case ITEM_SHOVEL:	sprintf( buf, "³opata"		); break;
	case ITEM_SALVE:	sprintf( buf, "salve"		); break;//?????
	case ITEM_RAWSPICE:	sprintf( buf, "ostrze"		); break;
	case ITEM_LENS:		sprintf( buf, "soczewka"	); break;
	case ITEM_CRYSTAL:	sprintf( buf, "kryszta³"	); break;
	case ITEM_DURAPLAST:	sprintf( buf, "duraplast"	); break;
	case ITEM_BATTERY:	sprintf( buf, "bateria"		); break;
	case ITEM_TOOLKIT:	sprintf( buf, "niezbêdnik"	); break;// ;)
	case ITEM_DURASTEEL:	sprintf( buf, "durastal"	); break;
	case ITEM_OVEN:		sprintf( buf, "oven"		); break;//?????
	case ITEM_MIRROR:	sprintf( buf, "lustro"		); break;
	case ITEM_CIRCUIT:	sprintf( buf, "obwód"		); break;
	case ITEM_SUPERCONDUCTOR:sprintf( buf,"nadprzewodnik"	); break;
	case ITEM_COMLINK:	sprintf( buf, "urz±dzenie komunikacyjne"); break;
	case ITEM_MEDPAC:	sprintf( buf, "medpak"		); break;
	case ITEM_FABRIC:	sprintf( buf, "fabrykat"	); break;
	case ITEM_RARE_METAL:	sprintf( buf, "czysty metal"	); break;
	case ITEM_MAGNET:	sprintf( buf, "magnet"		); break;//?????
	case ITEM_THREAD:	sprintf( buf, "ig³a z nitk±"	); break;
	case ITEM_PIECE:	sprintf( buf, "czesc"		); break;
	}
	return( buf );
}
...
const char *affect_loc_name_pl( int location )
{
	switch ( location )
	{
	case APPLY_NONE:		return "nic";
	case APPLY_STR:		return "si³ê";
	case APPLY_DEX:		return "zrêczno¶æ";
	case APPLY_INT:		return "inteligencjê";
	case APPLY_WIS:		return "m±dro¶æ";
	case APPLY_CON:		return "kondycjê";
	case APPLY_CHA:		return "charyzmê";
	case APPLY_LCK:		return "szczê¶cie";
	case APPLY_SEX:		return "p³eæ";
	case APPLY_LEVEL:		return "poziom";
	case APPLY_AGE:		return "wiek";
	case APPLY_MANA:		return "frc";
	case APPLY_HIT:		return "hp";
	case APPLY_MOVE:		return "mv";
	case APPLY_GOLD:		return "ilo¶æ kredytek";
	case APPLY_EXP:		return "do¶wiadczenie";
	case APPLY_AC:		return "klasê zbroi";
	case APPLY_HITROLL:		return "hitroll";
	case APPLY_DAMROLL:		return "damroll";
	case APPLY_SAVING_POISON:	return "odporno¶æ na trucizny";
	case APPLY_SAVING_ROD:	return "odporno¶æ na rod";
	case APPLY_SAVING_PARA:	return "odporno¶æ na parali¿";
	case APPLY_SAVING_BREATH:	return "odporno¶æ na gaz";
	case APPLY_SAVING_SPELL:	return "ochronê przed moc±";
	case APPLY_HEIGHT:		return "wysoko¶æ";
	case APPLY_WEIGHT:		return "wagê";
	case APPLY_AFFECT:		return "wp³ywy";
	case APPLY_RESISTANT:	return "odporno¶æ na moc";
	case APPLY_IMMUNE:		return "nietykalno¶æ";
	case APPLY_SUSCEPTIBLE:	return "podatno¶æ";
	case APPLY_WEAPONSPELL:	return "pos³ugiwanie siê broni±";
	case APPLY_BACKSTAB:	return "backstab";
	case APPLY_PICK:		return "pick";
	case APPLY_TRACK:		return "track";
	case APPLY_STEAL:		return "steal";
	...
	case APPLY_SCRIBE:		return "scribe";
	case APPLY_BREW:		return "brew";
	case APPLY_WEARSPELL:	return "moc ubrania";
	case APPLY_REMOVESPELL:	return "usuniêcie mocy";
	case APPLY_EMOTION:		return "stan emocjonalny";
	case APPLY_MENTALSTATE:	return "stan psychiczny";
	case APPLY_REMOVE:		return "remove";
	case APPLY_DIG:		return "dig";
	case APPLY_FULL:		return "g³ód";
	case APPLY_THIRST:		return "pragnienie";
	case APPLY_DRUNK:		return "upicie";
	case APPLY_BLOOD:		return "krew";
	}
...
char *weapon_type_pl( int value )
{
	...
	switch( value )
	{
	default:	      	sprintf( buf, "nieznany typ" 	);	break;
	case WEAPON_NONE: 	sprintf( buf, "zwyk³a broñ" 	);	break;
	case WEAPON_VIBRO_AXE:  sprintf( buf, "wibrotopór" 	);	break;
	case WEAPON_VIBRO_BLADE:sprintf( buf, "wibroostrze" 	);	break;
	case WEAPON_LIGHTSABER: sprintf( buf, "miecz ¶wietlny" 	);	break;
	case WEAPON_WHIP:	sprintf( buf, "bicz" 		);	break;
	case WEAPON_CLAW:	sprintf( buf, "pazur" 		);	break;
	case WEAPON_BLASTER:    sprintf( buf, "blaster" 	);	break;
	case WEAPON_BLUDGEON:   sprintf( buf, "bludgeon" 	);	break;
	case WEAPON_BOWCASTER:  sprintf( buf, "kusza energetyczna");	break;
	case WEAPON_FORCE_PIKE: sprintf( buf, "pika ¶wietlna" 	);	break;
	}
...
char *force_bit_name( int force_flags )
{
	...
	buf[0] = '\0';
	if ( force_flags & ITEM_RETURNING     ) strcat( buf, " returning"     );
...
ch_ret spring_trap( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	{
	default:
	txt = "zranion$y w py³apce";				break;
	case TRAP_TYPE_POISON_GAS:
	txt = "otoczon$y przez zielon± chmurê gazu";		break;
	case TRAP_TYPE_POISON_DART:
	txt = "trafion$y truj±c± strza³k±";			break;
	case TRAP_TYPE_POISON_NEEDLE:
	txt = "uk³ut$y truj±c± ig³±";				break;
	case TRAP_TYPE_POISON_DAGGER:
	txt = "zaatakowan$y sztyletem";			break;
	case TRAP_TYPE_POISON_ARROW:
	txt = "trafion$y strza³±";				break;
	case TRAP_TYPE_BLINDNESS_GAS:
	txt = "otoczon$y przez czerwon± chmurê gazu";		break;
	case TRAP_TYPE_SLEEPING_GAS:
	txt = "otoczon$y przez zó³t± chmurê gazu";		break;
	case TRAP_TYPE_FLAME:
	txt = "oparzon$y strumieniem p³omieni";		break;
	case TRAP_TYPE_EXPLOSION:
	txt = "uderzon$y eksplozj±";				break;
	case TRAP_TYPE_ACID_SPRAY:
	txt = "ochlapan$y strumieniem kwasu";			break;
	case TRAP_TYPE_ELECTRIC_SHOCK:
	txt = "pora¿on$y pr±dem";				break;
	case TRAP_TYPE_BLADE:
	txt = "okaleczon$y ostrzem";				break;
	case TRAP_TYPE_SEX_CHANGE:
	txt = "otoczon$y dziwn± mgie³k±";			break;
	}
...
void showaffect( CHAR_DATA *ch, AFFECT_DATA *paf )
{
	...
	if ( paf->location != APPLY_NONE && paf->modifier != 0 )
	{
		...
		default:
		sprintf( buf, FG_CYAN
		"Wp³ywa na " FB_CYAN "%s" FG_CYAN " o " FB_CYAN "%d" FG_CYAN "." EOL,
		affect_loc_name_pl( paf->location ), paf->modifier );
		break;
		case APPLY_AFFECT:
		sprintf( buf, "Wp³ywa na " FB_CYAN "%s" FG_CYAN " o " FB_CYAN,
		affect_loc_name_pl( paf->location ) );
...
if ( ch->top_level >= tarea->low_range || ch->top_level <= tarea->high_range )
	return true;
else
	return false;
}
...
int req_query( CHAR_DATA *ch, REQUIREMENT_DATA *req )
{
	...
	{
	default:		return 	0;
	case REQ_STR:		return	get_curr_str(ch);
	case REQ_DEX:		return	get_curr_dex(ch);
	case REQ_INT:		return	get_curr_int(ch);
	case REQ_WIS:		return	get_curr_wis(ch);
	case REQ_CON:		return	get_curr_con(ch);
	case REQ_CHA:		return	get_curr_cha(ch);
	case REQ_LCK:		return	get_curr_lck(ch);
	case REQ_FRC:		return	get_curr_frc(ch);
	case REQ_CLASS:		return  ch->main_ability;
	case REQ_SEX:		return	ch->sex;
	case REQ_GOLD:		return	ch->gold;
	case REQ_AGE:		return	get_age(ch);
	case REQ_HIT:		return	ch->max_hit;
	case REQ_FORCE:		return	ch->max_mana;
	case REQ_MOVE:		return	ch->max_move;
	case REQ_LEVEL:		return  ch->top_level;
	case REQ_AC:		return 	GET_AC(ch);
	case REQ_HITROLL:	return 	GET_HITROLL(ch);
	case REQ_DAMROLL:	return 	GET_DAMROLL(ch);
	case REQ_FULL:		return 	IS_NPC(ch) ? 0 :
					ch->pcdata->condition[COND_FULL];
	case REQ_THIRST:	return 	IS_NPC(ch) ? 0 :
					ch->pcdata->condition[COND_THIRST];
	case REQ_DRUNK:		return 	IS_NPC(ch) ? 0 :
					ch->pcdata->condition[COND_DRUNK];
	case REQ_RACE:		return	0; /* Trog: od mojej przerobki ras, to nie bedzie dzialac */
	case REQ_SKILL:		return	IS_NPC(ch) ? 0 :
					ch->pcdata->learned[req->type];
	case REQ_AFFECT:	return  IS_AFFECTED( ch, req->modifier );
	}
}
...
int req_eq( REQUIREMENT_DATA *req )
{
	switch( req->location )
	{
	case REQ_AC:		return REQ_LT;	/* musisz mieæ mniej */
	case REQ_SEX:
	case REQ_CLASS:
	case REQ_RACE:		return REQ_EQ;  /* musisz mieæ dok³adnie takie*/
	case REQ_AFFECT:	return REQ_BOOL;/* wystarczy, ¿e masz */
	default:		return REQ_GT;  /* dla reszty: wiecej */
	}
}
...
void explain( CHAR_DATA *ch, REQUIREMENT_DATA *req, OBJ_DATA *obj, bool iden )
{
	...
	{
	"",			"si³y", 		"zrêczno¶ci",
	"inteligencji",     "m±dro¶ci", 		"kondycji",
	"charyzmy",		"szczê¶cia", 		"mocy",
	"p³ci", 		"kredytek",     	"lat",
	"punktów uderzeniowych","punktów mocy",     "punktów ruchu",
	"poziomu", 		"klasy zbroi",     	"hitrolla",
	"damrolla", 	"poziomu g³odu",    	"poziomu pragnienia",
	"poziomu upicia", 	"rasy", 		"znajomo¶ci umiejêtno¶ci",
	"bycia pod wp³ywem","profesji podstawowej"
	};
	...
	default:
		if( iden )
		sprintf( expl, FB_CYAN "%s" FG_CYAN " (" FB_CYAN "%d lub wiêcej" FG_CYAN ")", expl_table[req->location], req->modifier );
		else
		sprintf( expl, "Masz za ma³o %s", expl_table[req->location] );
		break;
	case REQ_SEX: case REQ_RACE: case REQ_CLASS:
		if( iden )
		{
		sprintf( expl, FB_CYAN "%s" FG_CYAN " (" FB_CYAN "%s" FG_CYAN ")",
			expl_table[req->location],
//			req->location == REQ_RACE ? race_table[req->modifier].przypadki[0] :
			...
			(req->modifier == SEX_MALE ? "mêskiej" :
			req->modifier == SEX_FEMALE ? "¿eñskiej":"nijakiej") );
		}
		else
	...
	case REQ_LEVEL:
		...
		sprintf( expl, "Musisz byæ conajmniej %d %s",
		req->modifier,
		expl_table[req->location] );
		break;
	case REQ_SKILL:
		if( iden )
		sprintf( expl, FB_CYAN "%s " FG_CYAN "'" FB_CYAN "%s" FG_CYAN "' (" FB_CYAN "%d%%" FG_CYAN ")",
			expl_table[req->location],
			skill_table[req->type]->name,
			req->modifier );
		else
		sprintf( expl, "Za s³abo znasz umiejêtno¶æ '%s'",
		skill_table[req->type]->name );
		break;
	case REQ_AFFECT:
		if( iden )
		sprintf( expl, FB_CYAN "%s %s", expl_table[req->location],
			flag_string( aff_flags_list, req->modifier ) );
		else
		sprintf( expl, "Musisz byæ pod wp³ywem %s",
		flag_string( aff_flags_list, req->modifier ) );
		break;
	case REQ_AC:
		if( iden )
		sprintf( expl, "%s " FG_CYAN "(" FB_CYAN "%d lub mniej" FG_CYAN ")",
		expl_table[req->location], req->modifier );
		else
	...
	{
	ch_printf( ch, FB_CYAN "%s by u¿ywaæ %s %s." EOL,
		expl,
		obj->gender == GENDER_NEUTRAL 	? "tego" :
		obj->gender == GENDER_MALE 		? "tego" :
		obj->gender == GENDER_FEMALE 	? "tej"  : "tych",
		obj->przypadki[1] );
	}
	return;
}
...
void show_req( CHAR_DATA *ch, REQUIREMENT_DATA *req )
{
	...
	ch_printf( ch,
	"%-10s " FG_CYAN, bit_name( req_types_list,  req->location ) );
	if( req->location == REQ_SKILL )
	{
	char	buf[MSL];
	sprintf( buf, "'%s'",
		STRING_REQUIREMENT(req->location, req->modifier, req->type ) );
	ch_printf( ch, "%-30s %s", buf,
		req->location == REQ_SKILL ? itoa( req->modifier ) : "" );
	}
	else
		ch_printf( ch, "%-30s %s", " ",
			STRING_REQUIREMENT(req->location, req->modifier, req->type ) );
