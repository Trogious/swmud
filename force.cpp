...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    {
         if( rch != ch )
         act( COL_FORCE, "$n zatrzymuje siê i koncentruje na moment.",
         ch, NULL, rch, TO_VICT );
    }
...
DEF_DO_FUN( cast )
{
	...
	if( ch->position < skill->minimum_position )
	{
	    switch( ch->position )
	    {
	      default:
		send_to_char(
		"Nie mo¿esz siê wystarczaj±co skoncentrowaæ." NL, ch );
		break;
	      case POS_SITTING:
		send_to_char(
		"Siedz±c, nie zgromadzisz w sobie wystarczaj±cej ilo¶ci energii." NL, ch );
		break;
	      case POS_RESTING:
		ch_printf( ch, "Jeste¶ zbyt rozlu¼nion%s by u¿ywaæ mocy.." NL,
			SEX_SUFFIX_YAE( ch ) );
		break;
	      case POS_FIGHTING:
	...
	switch( number_bits(2) )
	{
		...
		if( number_bits(2) == 0 )
		{
		  switch( number_bits(2) )
		  {
			...
			break;
		     case 1:
		        ch_printf( ch,
			"Poczu³%s¶ mrowienie na nodze i straci³%s¶ koncentracjê." NL,
			    SEX_SUFFIX_EAE(ch),SEX_SUFFIX_EAE(ch)  );
			break;
		     case 2:
		...
		if( ch->fighting )
		  ch_printf( ch,
		    "Mia³%s¶ za ma³o czasu na skupienie. Nie uda³o siê." NL,
			SEX_SUFFIX_EAE(ch) );
		else
		  ch_printf( ch,
		    "Straci³%s¶ koncentracjê." NL, SEX_SUFFIX_EAE(ch) );
		break;
	    case 3:
	...
	&&    victim && IS_SET(victim->immune, RIS_FORCE) )
	{
    ...
    if( retcode != rSPELL_FAILED )
    {
		...
		if( IS_NPC( ch ) )
	        ...
	        force_exp = 100-ch->pcdata->learned[sn] + log(skill->min_level+ch->pcdata->learned[sn]+ch->skill_level[FORCE_ABILITY])/log(2);
...
ch_ret obj_cast_spell( int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj )
{
    ...
    IF_BUG( !skill, "bad sn %d (ch=%s) (vict=%s)",
		sn, ch->name, victim->name )
	return rERROR;

    IF_BUG( !skill->spell_fun, "bad spell %s (ch=%s) (vict=%s)",
		skill->name, ch->name, victim->name )
	return rERROR;
	...
	switch( number_bits(2) )
	{
	   ...
	   case 1:
		act( COL_FORCE,
		    "Lamersko u¿ywasz $p$3 i niechc±cy kierujesz wszystko na siebie!",
			ch, obj, victim, TO_CHAR );
		if( victim )
		  act( COL_FORCE,
		    "$n lamersko u¿ywa $p$3 i niechc±cy kieruje wszystko na siebie!",
...
ch_ret spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ch_printf( victim, "Zosta³%s¶ o¶lepion%s!" NL, SEX_SUFFIX_EAE( victim ),
...
ch_ret spell_dispel_force( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if( victim->affected_by == victim->race->affected
    ||   level < victim->top_level )
    {
	failed_casting( skill, ch, victim, NULL );
	return rSPELL_FAILED;
...
/* Modified by Scryn to work on mobs/players/objs */
    ...
    if( IS_NPC(victim) )
    {
      ch_printf(ch, "Ofiara wydaje siê mieæ poziom miêdzy %d a %d." NL,
...
ch_ret spell_know_alignment( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ap = victim->alignment;
	 ...
	 if( ap >  700 ) msg = "Aura bia³a jak ¶niegi Hoth otacza $N$3.";
    else if( ap >  350 ) msg = "$N jest wzorem moralno¶ci i dobra.";
    else if( ap >  100 ) msg = "$N jest mi³$Y i uczynn$Y.";
    else if( ap > -100 ) msg = "$N nie jest najszlachetniejsz$Y.";
    else if( ap > -350 ) msg = "$N czêsto ok³amuje przyjació³.";
    else if( ap > -700 ) msg = "$N wygl±da tak, jakby mia³ za chwilê ciê zabiæ!";
    else msg = "$N to z³o, które przechodzi ciê dreszczem.";
...
ch_ret spell_weaken( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
    send_to_char( COL_FORCE, victim );
    ch_printf( victim, "Czujesz siê s³absz%s." NL,
	SEX_SUFFIX_YAE(victim) );
    if( ch != victim )
	send_to_char( "Ok." NL, ch );
    return rNONE;
...
ch_ret spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( obj_lose = victim->first_carrying; obj_lose; obj_lose = obj_next )
	{
	    ...
	    case ITEM_CONTAINER:
		separate_obj( obj_lose );
		act( COL_ACTION, "$p syczy i rozsypuje siê na kawa³ki!",
		    victim, obj_lose, NULL, TO_CHAR );
		act( COL_OBJECT, "Zawarto¶æ $p$1 wypada na ziemiê.",
...
ch_ret spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	      ...
	      obj_lose = obj_next )
	{
	    ...
	    {
		act( COL_OBJECT, "The contents of $p spill out onto the ground.",
		   victim, obj_lose, NULL, TO_ROOM );
		act( COL_OBJECT, "The contents of $p spill out onto the ground.",
		   victim, obj_lose, NULL, TO_CHAR );
		empty_obj( obj_lose, NULL, victim->in_room );
...
ch_ret spell_farsight( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    */
    ...
    ||	 (!victim->in_room->area->planet && !ch->in_room->area->planet)
	||	 victim->in_room->area->planet != ch->in_room->area->planet
...
ch_ret spell_possess( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if(victim->desc)
	{
		ch_printf(ch, "Kto¶ ju¿ kontroluje zachowanie %s" PLAIN "." NL,
			victim->przypadki[1]);
		return rSPELL_FAILED;
	}
	...
	|| level < (victim->top_level + get_curr_int(victim))
	|| victim->desc
	|| !chance(ch, 25) )
	{
		failed_casting( skill, ch, victim, NULL );
...
ch_ret spell_dream( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ch_printf(victim, FB_WHITE "¦ni ci siê, ¿e %s" FB_WHITE " mówi ci '%s" FB_WHITE "'." NL,
	 PERS(ch, victim, 0), target_name);
...
ch_ret spell_scorching_surge( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( COL_FORCE, "Ognista furia w postaci czystej energii emanuje z $n$1!",
        ch, NULL, NULL, TO_ROOM );
    act( COL_FORCE, "Ognista furia w postaci czystej energii emanuje z ciebie!",
...
ch_ret spell_obj_inv( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	case SA_CREATE:
	  if( SPELL_FLAG(skill, SF_WATER) )	/* create water */
	  {
	    ...
	    &&  ch->in_room->area->planet->sky >= SKY_RAINING )
	        weather_mod = 2;
	    else
	        weather_mod = 1;
...
ch_ret spell_smaug( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	switch( skill->target )
	{
		...
		case TAR_CHAR_DEFENSIVE: case TAR_CHAR_SELF:
...
ch_ret spell_ethereal_fist( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( COL_FORCE, "A fist of black, otherworldly ether rams into $N, leaving $M looking stunned!"
                   , ch, NULL,
	 	   victim, TO_NOTVICT );
    return damage( ch, victim, dam, sn );
}
...
ch_ret spell_hand_of_darkness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if((int)ch->pcdata->learned[sn] < 75)
	dam /= 2;
    }
    else
    ...
    act( COL_FORCE, "$N is grasped by an incomprehensible hand of darkness!"
                   , ch, NULL,
	 	   victim, TO_NOTVICT );
    return damage( ch, victim, dam, sn );
}
...
ch_ret spell_mind_wrack( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( COL_FORCE, "$n stares intently at $N, causing $N to seem very lethargic."
                   , ch, NULL,
	 	  victim, TO_NOTVICT );
    return damage( ch, victim, dam, sn );
}
...
ch_ret spell_mind_wrench( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level	= UMAX(0, level);
    dam		= number_range( 0, 0 );
    act( COL_FORCE, "$n stares intently at $N, causing $N to seem very hyperactive."
                   , ch, NULL,
	 	   victim, TO_NOTVICT );
    return damage( ch, victim, dam, sn );
}
...
ch_ret spell_revive( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level	= UMAX(0, level);
    dam		= number_range( 0, 0 );
    act( COL_FORCE, "$n concentrates intently, and begins looking more centered."
                   , ch, NULL,
	 	  victim, TO_NOTVICT );
    return damage( ch, victim, dam, sn );
}
...
ch_ret spell_black_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	ch->alignment = URANGE( -1000, ch->alignment, 1000 );
	sith_penalty(ch);
	act( FB_BLUE, "Wi±zki elektryczno¶ci strzelaj± z palców $n$1, posy³aj±c $N$3 na ziemiê w bolesnych spazmach.",
		ch, NULL, victim, TO_NOTVICT );
    act( FB_BLUE, "Wi±zki elektryczno¶ci strzelaj± z Twoich palców, posy³aj±c $N$3 na ziemiê w bolesnych spazmach.",
		ch, NULL, victim, TO_CHAR );
    act( FB_BLUE, "Potê¿ny bó³ przechodzi przez Twoje cia³o podczas, gdy wi±zki elektryczno¶ci wydobywaj±ce siê z" NL
		"palców $N$1 anihiluj± Cie.",
...
void fevent_trigger( CHAR_DATA *ch, fe_trigger trigger, ... )
{
	...
	for( fevent = ch->pcdata->first_fevent; fevent; fevent = fevent_next,
		found = false )
	{
		...
		switch( fevent->trigger )
		{
			...
			case FE_FIND_PLAYER:
				{
					...
					FOREACH( player, ch->in_room->first_person )
						if( !IS_NPC( player )
						&& ((*fevent->sattr	&& !str_cmp( player->name, fevent->sattr ))
						|| !*fevent->sattr) )
						{
							found = true;
							break;
			...
			case FE_FIND_SHIP:
				{
					...
					FOREACH( ship, ch->in_room->first_ship )
						if( ((*fevent->sattr && !str_cmp( ship->name, fevent->sattr ))
						|| !*fevent->sattr) )
						{
							found = true;
							break;
			...
			case FE_KILL_MOB:
				{
					...
					if( (fevent->attr[0] && vnum == fevent->attr[0])
					|| !fevent->attr[0] )
						found = true;
				}
				break;
			...
			case FE_KILL_PLAYER:
				{
					...
					if( (*fevent->sattr && !str_cmp( playerName, fevent->sattr ) )
					|| !*fevent->sattr )
						found = true;
				}
				break;
			...
			case FE_KILL_SHIP:
				{
					...
					if( (*fevent->sattr && !str_cmp( shipName, fevent->sattr ) )
					|| !*fevent->sattr )
						found = true;
				}
				break;
			...
			case FE_GAIN_STAT:
				{
					...
					if( (fevent->attr[1] && amt >= fevent->attr[0])
					|| (!fevent->attr[1] && amt == fevent->attr[0]) )
						found = true;
				}
				break;
			...
			case FE_FINISH_QUEST:
				{
					...
					if( (fevent->attr[0] && quest->quest_id == fevent->attr[0])
					|| !fevent->attr[0] )
						found = true;
				}
				break;
			...
			case FE_ENTER_SHIP:
				if( (*fevent->sattr
				&& !str_cmp( ch->in_room->ship->name, fevent->sattr ) )
				|| !*fevent->sattr )
					found = true;
				break;
			...
			case FE_LEVEL:
				if( (fevent->attr[1] && ch->top_level >= fevent->attr[0])
				|| (!fevent->attr[1] && ch->top_level == fevent->attr[0]) )
					found = true;
				break;
			...
			case FE_MKILLS:
				if( (fevent->attr[1] && ch->pcdata->mkills >= fevent->attr[0])
				|| (!fevent->attr[1] && ch->pcdata->mkills == fevent->attr[0]) )
					found = true;
				break;

			case FE_PKILLS:
				if( (fevent->attr[1] && ch->pcdata->pkills >= fevent->attr[0])
				|| (!fevent->attr[1] && ch->pcdata->pkills == fevent->attr[0]) )
					found = true;
				break;
			...
			case FE_SKILL:
				{
					...
					{
						bug( "fevent_trigger: SKILL: no such skill: %s",
							fevent->sattr );
						break;
					}
			...
			default:
				bug( "fevent_trigger: player: %s - no such force event: %d",
					ch->name, fevent->trigger );
				break;
		}
		...
		if( found )
		{
			...
			UNLINK( fevent, ch->pcdata->first_fevent,
				ch->pcdata->last_fevent, next, prev );
			free_fevent( fevent );
		}
	...
	if( !ch->pcdata->first_fevent )
	{
		...
		sprintf( buf, "giving %d (+%d) frc to player %s due to force events completed",
			ch->pcdata->fevents, ch->race->frc_plus, ch->name );
		log_string( buf );
		ch->pcdata->fevents = 0;
	}
}
...
//	&& number_range( 0, 100 ) > 5 )
	...
	ch->pcdata->fevents = number_range( 1, number_range( 0, 100 ) < 90
		? number_range( 1, 5 ) : number_range( 1, 100 ) );
			...
			case FE_SKILL:
				{
					...
					if( !skill_table[no] || !skill_table[no]->name || !*skill_table[no]->name )
					{
						...
						free_fevent( fevent );
						link = false;
						bug( "fevent_trigger: player: %s - no such skill sn: %d",
							ch->name, no );
					}
					else
					{
