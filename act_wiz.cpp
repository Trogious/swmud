...
DEF_DO_FUN( wizinfo )
{
	...
	for ( cnt = hash = 0; hash < MAX_CMD_HASH; hash++ )
	{
		...
		if( cmd->level == level )
		{
		    ...
		    && !is_name( cmd->name, victim->pcdata->forbidden_cmd ) )
			sprintf(buf," " FB_WHITE "%-10.10s" PLAIN "%s",
			    all_capitalize(cmd->name),
			    ((++cnt)%6)? "" : NL "          ");
		    else
			sprintf(buf," " PLAIN "%-10.10s%s",
			    cmd->name,
			    ((++cnt)%6)? "" : NL "          ");
		    strcat( buf1, buf );
		}
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
	...
	to_channel( buf, CHANNEL_MONITOR, "Monitor", ch->top_level );
        ch_printf (victim,
          ...
          "Wymy¶l inne, bardziej odpowiednie i poinformuj o tym Administratorów." NL, victim->name);
...
DEF_DO_FUN( retire )
{
    ...
    else
    {
      ...
      ch_printf( victim, "Courtesy of %s, you are now a retired immortal." NL, ch->name );
    }
    return;
}
...
DEF_DO_FUN( pardon )
{
    ...
    send_to_char( "Syntax: pardon <character> <planet>.... But it doesn't work .... Tell Durga to hurry up and finish it :p" NL, ch );
    return;
}
...
DEF_DO_FUN( transfer )
{
    ...
    act( COL_FORCE, "$n znika rozpadaj±c siê na maleñkie cz±steczki.", victim, NULL, NULL, TO_ROOM );
    ...
    send_to_char("Warning: the player's level is not within the area's level range." NL, ch);
}
...
DEF_DO_FUN( rat )
{
    ...
    if ( argument[0] && !str_cmp( argument, "status" ) )
    {
	    ...
	    ch_printf( ch, "Range:     %d-%d" NL,  	rat_loop.Start,
							rat_loop.End );
	    ch_printf( ch, "Next number: %d" NL,
		rat_loop.rRoom );
	}
	return;
    }
    ...
    {
	send_to_char(
	"Sorry, Rat Loop is currently running. Stop it if you want." NL, ch );
	send_to_char(
	"Use 'rat stop' command, but check who is using it first.('rat status')." NL, ch );
	return;
    }
...
DEF_DO_FUN( rstat )
{
	...
	{
	    send_to_char( "Mo¿esz u¿ywaæ rstat tylko w przydzielonej ci krainie." NL, ch );
	    return;
	}
    }
	...
	ch_printf( ch, "&cExits for room '&w%s&c.' vnum&w %d" NL,
		location->name,
		location->vnum );
	...
	for ( cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next )
		...
		++cnt,
		dir_text[pexit->vdir],
		pexit->to_room ? VNUM(pexit->to_room) : 0,
		pexit->key,
		pexit->flags,
		pexit->orig_flags,
		pexit->keyword,
	...
	location->vnum,
	bit_name( sector_types_list, location->sector_type ),
	location->light,
	location->tele_delay,
	location->tele_vnum,
	location->tunnel );
...
DEF_DO_FUN( ostat )
{
    ...
    if ( ( obj = get_obj_world( ch, arg ) ) == NULL )
    {
	send_to_char( "Nie ma nic takiego w piekle na ziemi ani na niebie." NL, ch );
	return;
    }

    ch_printf( ch, "&cName:&w %s. &cGender:&w %d &c(&w%s&c)&w" NL,
	obj->name, obj->gender, bit_name( gender_types_list, obj->gender ) );
    ...
    ch_printf( ch, "&cVnum:&w %d&c.  Type:&w %s&c.  Count:&w %d &c Gcount:&w %d" NL,
	obj->pIndexData->vnum, bit_name( obj_types_list, obj->item_type ),
	obj->pIndexData->count,	obj->count );

    ch_printf( ch, "&cSerial#:&w %d&c  TopIdxSerial#:&w %d &c TopSerial#:&w %d" NL,
	obj->serial, obj->pIndexData->serial, cur_obj_serial );

    ch_printf( ch, "&cPrzypadki: 0.&w%-15s &c1.&w%-15s &c2.&w%-15s\n\r           &c3.&w%-15s &c4.&w%-15s &c5.&w%-15s" NL,
	obj->przypadki[0],	obj->przypadki[1],
	obj->przypadki[2],	obj->przypadki[3],
	obj->przypadki[4],	obj->przypadki[5] );
    ...
    ch_printf( ch, "&cNumber:&w %d&c/&w%d&c.  Weight:&w %d&c/&w%d&c.  Layers:&w %d",
    ...
    ch_printf( ch, "&cCost:&w %d&c.  Rent:&w %d&c.  Timer:&w %d&c.  Level:&w %d&c.&w" NL,
	...
	(obj->carried_by != NULL && IS_NPC(obj->carried_by)) ? obj->carried_by->pIndexData->vnum : 0,
	obj->wear_loc );

    show_obj_values( ch, obj->pIndexData );
    ch_printf( ch, "&cObject Values:&w %d %d %d %d %d %d&c.&w" NL,
...
DEF_DO_FUN( mstat )
{
    ...
    ch_printf( ch, "&cName:&w %s     &cOrganization:&w %s" NL,
	victim->name,
	( IS_NPC( victim ) || !victim->pcdata->clan ) ? "(none)"
			       : victim->pcdata->clan->name );

    if( get_trust(ch) >= LEVEL_GOD && !IS_NPC(victim) && victim->desc )
	ch_printf( ch, "&cUser:&w %s@%s   &cDescriptor:&w %d   &cTrust:&w %d   &cAuthedBy:&w %s" NL,
		victim->desc->user,
		victim->desc->host,
    ...
    if ( !IS_NPC( victim ) )
    {
       ...
       "Quest points: " PLAIN "%d" FG_CYAN ".  Quests made: " PLAIN "%s" FG_CYAN "." EOL,
                 ...
                 victim->pcdata->quest_done );
    }

    ch_printf( ch, "&cVnum:&w %d &c  Sex:&w %s  &c Room: &w%d &c  Count:&w %d &c Killed:&w %d" NL,
	IS_NPC(victim) ? victim->pIndexData->vnum : 0,
	MALE( victim )    ? "male"   :
	FEMALE( victim )  ? "female" : "neutral",
    ...
    ch_printf( ch, "&cStr:&w %d  &cInt:&w %d  &cWis:&w %d  &cDex:&w %d  &cCon:&w %d  &cCha:&w %d  &cLck:&w %d  &cFrc:&w %d" NL,
	get_curr_str(victim),
	get_curr_int(victim),
	get_curr_wis(victim),
	get_curr_dex(victim),
	...
	get_curr_lck(victim),
	get_curr_frc(victim) );
    ch_printf( ch, "&cHps:&w %d&c/&w%d  &cForce:&w %d&c/&w%d   &cMove:&w %d&c/&w%d" NL,
    ...
    if ( !IS_NPC( victim ) )
    {
       ...
       for ( ability = 0 ; ability < MAX_ABILITY ; ability++ )
            ch_printf( ch, "&c%-15s   Level:&w %-3d&c/&w%-3d&c   Exp:&w %-10ld   &cNext:&w %-10ld" NL,
            class_table[ability].name,
    ...
    ch_printf( ch, FG_CYAN "Security:" PLAIN " %d" FG_CYAN " Hitroll:&w %d&c   Damroll:&w %d&c   Position:&w %d  &c Wimpy:&w %d " NL,
	victim->pcdata ? victim->pcdata->security : 0,
    ...
    if ( !IS_NPC(victim) )
	ch_printf( ch,
	    "&cThirst:&w %d&c   Full: &w%d&c   Drunk:&w %d &c" NL,
	    victim->pcdata->condition[COND_THIRST],
	    victim->pcdata->condition[COND_FULL],
	    victim->pcdata->condition[COND_DRUNK] );
    else
	ch_printf( ch, FG_CYAN "Hit dice: " PLAIN "%d" FG_CYAN "d" PLAIN "%d" FG_CYAN "+" PLAIN "%d" FG_CYAN "  Damage dice: " PLAIN "%d" FG_CYAN "d" PLAIN "%d" FG_CYAN "+" PLAIN "%d  ",
    ...
    ch_printf( ch, "&cYears:&w %d &c  Seconds Played: &w%d &c  Timer:&w %d  &c Act: &w%d" NL,
	...
	if( victim->first_crime )
	{
	    ...
	    ch_printf( ch, "&cCrime data:&w Planet: %s  Lv: %d  for: %s (%d)" NL,
		crime->planet,
		crime->level,
    ...
    ch_printf( ch, "&cSpeaking:&w %s" NL,
    	victim->speaking->name );
    ...
    ch_printf( ch, "&cPrzypadki: 0.&w%-15s &c1.&w%-15s &c2.&w%-15s\n\r           &c3.&w%-15s &c4.&w%-15s &c5.&w%-15s" NL,
	victim->przypadki[0],	victim->przypadki[1],
	victim->przypadki[2],	victim->przypadki[3],
	victim->przypadki[4],	victim->przypadki[5] );
    ...
    if( IS_NPC( victim ) )
    {
	ch_printf( ch, FG_CYAN "Nameslist (" PLAIN "%d" FG_CYAN "):" PLAIN, victim->pIndexData->namescount );
	for( nameslist = victim->pIndexData->first_nameslist; nameslist; nameslist = nameslist->next )
	...
	if ( (skill=get_skilltype(paf->type)) != NULL )
	  ch_printf( ch,
	    ...
	    paf->modifier,
	    paf->duration,
	    flag_string( aff_flags_list, paf->bitvector )  );

	if( get_trust( ch ) > 103 && !IS_NPC( victim )
	&& victim->pcdata->first_fevent )
	{
		FEVENT_DATA	*fevent;

		ch_printf( ch, FG_CYAN "Force Events (" PLAIN "%d" FG_CYAN "):" EOL, victim->pcdata->fevents );
		FOREACH( fevent, victim->pcdata->first_fevent )
			ch_printf( ch, "%s" FG_CYAN ": (" PLAIN "%s" FG_CYAN ") (" PLAIN
				"%lld %lld %lld %lld %lld" FG_CYAN ")" EOL,
				bit_name( fevent_types_list, fevent->trigger ),
				fevent->sattr, fevent->attr[0], fevent->attr[1],
				fevent->attr[2], fevent->attr[3], fevent->attr[4] );
	}
    return;
}
...
/*  for ( vnum = 0; nMatch < top_mob_index; vnum++ )
	...
	for ( pMobIndex = mob_index_hash[hash];
	    ...
	    {
		nMatch++;
		pager_printf( ch, "[%5d] %s&z&w" NL,
		    pMobIndex->vnum, capitalize( pMobIndex->przypadki[0] ) );
	    }
...
DEF_DO_FUN( ofind )
{
	...
	for ( hash = 0; hash < MAX_KEY_HASH; hash++ )
		...
		{
		    nMatch++;
		    pager_printf( ch, "[%5d] %s" EOL,
			pObjIndex->vnum, capitalize( pObjIndex->przypadki[0] ) );
		}
    }
    else
    if( !str_cmp( arg, "type" ) )
	...
	for ( hash = 0; hash < MAX_KEY_HASH; hash++ )
    	for ( pObjIndex = obj_index_hash[hash];
		  pObjIndex; pObjIndex = pObjIndex->next )
			...
			if ( pObjIndex->item_type == type && pObjIndex->value[valnr] == value )
			{
			    nMatch++;
			    pager_printf( ch, "[%5d] %s" EOL,
				pObjIndex->vnum, capitalize( pObjIndex->przypadki[0] ) );
	...
	++icnt;
      ...
      if ( obj->carried_by )
        sprintf(buf+strlen(buf), "invent [%5d] %s" NL,
                ...
                : 0), PERS(obj->carried_by, ch, 0));
      else if ( obj->in_room )
        sprintf(buf+strlen(buf), "room   [%5d] %s" NL,
...
bool purge_room( CHAR_DATA *ch, ROOM_INDEX_DATA *room )
{
	...
	if( obj->item_type == ITEM_SHIPMODULE
	||  obj->inquest
...
//blee	act( COL_IMMORT, "$n dotyka $p$3 palcem, i $p znika!&z&w", ch, obj, NULL, TO_ROOM);
	act( COL_IMMORT, "You make $p disappear in a puff of smoke!", ch, obj, NULL, TO_CHAR);
	extract_obj( obj );
	return;
    }
...
//blee    act( COL_IMMORT, "$n dotyka d³oni± $N$3. $N rozpada siê na ma³e kawa³eczki!", ch, NULL, victim, TO_NOTVICT );
    act( COL_IMMORT, "You make $N disappear in a puff of smoke!", ch, NULL, victim, TO_CHAR);
    extract_char( victim, true );
    return;
}
...
DEF_DO_FUN( balzhur )
{
	...
	send_to_char( "Balzhur sneers at you evilly, then vanishes in a puff of smoke." NL, ch );
	send_to_char( COL_IMMORT, victim );
	send_to_char( "You hear an ungodly sound in the distance that makes your blood run cold!" NL, victim );
...
DEF_DO_FUN( immadvance )
{
    ...
    {
		send_to_char( "This player is not worthy of becoming immortal yet." NL, ch );
		return;
    }
    ...
    act( COL_IMMORT, "$n przygl±da ci siê z uwag±... potem wznosi swe rêce do góry...",
    ...
    do_help(victim, buf );
    send_to_char( FB_WHITE, victim );
    send_to_char( "Budzisz siê... a wszystkie niedoskona³o¶ci znikaj±." EOL, victim );
    while ( victim->first_carrying )
	extract_obj( victim->first_carrying );
...
DEF_DO_FUN( restore )
{
        ...
        if ( get_trust( ch ) < LEVEL_SUB_IMPLEM )
        {
 	    ...
 	    return;
          ...
          else
          {
            ...
            {
              send_to_char( "Sorry, you can't do a restore all yet." NL, ch );
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
	    ...
	    {
		restore_char( vch );
		ch_printf( vch, "%s przywraca ci pe³niê si³." EOL, PERS( ch, vch, 0 ) );
	    }
	}
    }
    else
    ...
    restore_char( victim );
    if ( ch != victim )
	ch_printf( victim, "%s przywraca ci pe³niê si³." EOL, PERS( ch, victim, 0 ) );
    send_to_char( "Ok." NL, ch );
    return;
    }
}
...
DEF_DO_FUN( restoretime )
{
  ...
  if ( !last_restore_all_time )
     ...
     ch_printf( ch, "The  last restore all was %d hours and %d minutes ago." NL,
                  hour, minute );
     }
...
DEF_DO_FUN( ban )
{
    ...
    if ( !str_cmp(arg, "help") )
    {
      ...
      send_to_char( "Syntax: ban <number> <level> | newban | mortal | total" NL, ch );
      return;
    }
...
DEF_DO_FUN( users )
{
	...
	||   (d->character && can_see( ch, d->character )) )
	{
		...
		(d->character && d->character->name) ? d->character->name : "(none)"
		);
...
DEF_DO_FUN( force )
{
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
	    ...
	    {
		act( COL_IMMORT, "Za spraw± $n$1 czujesz, ¿e dobrze by³oby zrobiæ: '$t'.", ch, argument, vch, TO_VICT );
    ...
    act( COL_IMMORT, "Za spraw± $n$1 czujesz, ¿e dobrze by³oby zrobiæ: '$t'.", ch, argument, victim, TO_VICT );
...
DEF_DO_FUN( invis )
{
    ...
    */
	...
	if( !str_cmp( arg, "on" ) )
	{
	    ...
	    act( COL_IMMORT, "$n powoli zlewa siê z otoczeniem.", ch, NULL, NULL, TO_ROOM );
	...
	if( !str_cmp( arg, "off" ) )
	{
	    ...
	    act( COL_IMMORT, "$n powoli materializuje siê.", ch, NULL, NULL, TO_ROOM );
	    send_to_char( "Wizinvis removed." NL, ch );
	}
	else
	if( !str_cmp( arg, "except" ) )
	{
	    if( IS_NPC( ch ) )
	    {
		...
		if( ch->pcdata->invis_except[0] != '\0' )
		    ch_printf( ch, "List of people who can see you in spite of your invis:" NL "%s" EOL,
		...
		STRDUP( ch->pcdata->invis_except, (char *)cut_from_string( buf, arg ) );
		...
		char	buf	[MSL];
    		...
    		ch_printf( ch, "'%s" PLAIN "' can see you in spite of your invis." NL, arg );
	    }
	    return;
	}
	else
...
DEF_DO_FUN( cmdtable )
{
    ...
    send_to_pager(FB_WHITE " Nr Command     Level    Log     Position  Used  Flags     Aliases" EOL, ch);
	...
	for ( cmd = command_hash[hash]; cmd; cmd = cmd->next )
	{
		...
		pager_printf(ch,"%-8.8s  %-20.20s" EOL,
		    flag_string( command_flags_list, cmd->flags ),
		    cmd->alias  );
	}
    ch_printf(ch,NL "You should remove commands shown in the yellow lines. (if there are some)" NL );
    return;
}
...
DEF_DO_FUN( loadup )
{
	...
	DISPOSE( d->outbuf );
	DISPOSE( d );
	ch_printf(ch, "Player %s loaded from room %d." NL,
...
/* Added the burlap sack to the newbieset.  The sack is part of sgate.are
   called Spectral Gate.  Brittany */
    ...
    act( COL_IMMORT, "$n wyposa¿y³$o ciê w zestaw m³odego gracza.", ch, NULL, victim, TO_VICT);
...
DEF_DO_FUN( bestow )
{
    ...
    {
        ch_printf( ch, "Current bestowed commands on %s: %s." NL,
...
DEF_DO_FUN( set_boot_time )
{
   ...
   argument = one_argument(argument, arg);
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Syntax: setboot time {hour minute <day> <month> <year>}" NL, ch);
	...
	send_to_char( "        setboot default" 			NL, ch);
		...
		sysdata.reboot_type == REB_COPYOVER ? "copyover" :
						       "reboot",
	...
	send_to_char("You must input a value for hour and minute." NL, ch);
 	return;
	  ...
	  return;
        ...
        if ( *arg != '\0' && is_number(arg) )
        {
          ...
          {
            send_to_char( "Valid range for year is 1900 to 2099." NL, ch );
            return;
          }
        }
      }
 	...
 	set_boot_time->manual = 1;
      ...
      ch_printf(ch, "Boot time set to %s" NL, SWTimeStamp(new_boot_time).getDescriptive().c_str());
      check = true;
    }
	...
	argument = one_argument(argument, arg1);
        ...
        if( !*arg || !*arg1
        ||( str_prefix(arg1, "minutes")
        && str_prefix(arg1, "hours") ) )
        {
	    if( ch )
	    send_to_char("Syntax: setboot after <how_many> "
	    "< " FB_WHITE "m" PLAIN "inutes | " FB_WHITE "h" PLAIN "ours >" NL, ch);
	    return;
        }
	...
	if( ch )
	ch_printf( ch, "Ok. Setboot set to %2d:%.2d." NL,
		now_time->tm_hour,
		now_time->tm_min );
...
void do_set_password( CHAR_DATA *ch, char *argument )//Trog
{
	...
	{
		ch_printf( ch, "Has³o musi mieæ co najmniej %d znaków." NL, MIN_PASS_LEN );
		return;
	}
...
DEF_DO_FUN( for )
{
    ...
    if ( argument[0] && !str_cmp( argument, "status" ) )
    {
		...
		ch_printf( ch, "Next room: %s {%d}" NL,
		    ...
		    get_room_index(for_loop.fRoom)->name : "no_room",
		    for_loop.fRoom );
	    else
		ch_printf( ch, "Next char: %s &z&w%s {%d}" NL,
		    for_loop.fVictim->przypadki[0],
		    IS_NPC( for_loop.fVictim ) ? "(NPC)" : "(PC)",
		    IS_NPC( for_loop.fVictim ) ? for_loop.fVictim->pIndexData->vnum : 0 );
	}
	return;
    }
    ...
    {
	send_to_char(
	"Sorry, For Loop is currently running. Stop it if you want." NL, ch );
	send_to_char(
	"Use 'for stop' command, but check who is using it first.('for status')." NL, ch );
	return;
    }

    if (!str_prefix("quit", argument )
    || 	is_name( "for", argument )
...
DEF_DO_FUN( cset )
{
  ...
  if (argument[0] == '\0')
  {
	...
	"  MaxClanId:                      %3d." NL
    "  Save flags: %s"	NL,
	    sysdata.level_forcepc,		sysdata.level_override_private,
	    sysdata.stun_regular, 		sysdata.stun_plr_vs_plr,
	    sysdata.dam_plr_vs_plr, 		sysdata.dam_plr_vs_mob,
	    sysdata.dam_mob_vs_plr, 		sysdata.dam_mob_vs_mob,
	    sysdata.level_getobjnotake,		sysdata.save_frequency,
		sysdata.max_clan_id, flag_string( save_flags_list,sysdata.save_flags ) );
    ...
    "  Council:       %s." NL
    "  Helpmasters:   %s." NL,
	sysdata.admins, sysdata.coders, sysdata.council, sysdata.helpmasters );
	...
	sysdata.tmpdowntype == DOWN_CRASHED	? FB_RED "Crashed" PLAIN : "*>HGW!<*",
	sysdata.log_all 	? "Yes" : "No",
	sysdata.lastcmd_log 	? "Yes" : "No",
	sysdata.crashlog 	? "Yes" : "No",
	sysdata.silent 		? "Yes" : "No",
...
/*Added by Thanos: czy mud ma logowaæ np. resety krain */
  ...
  ||  !str_cmp( arg, "denied" ) )
  {
	...
	if( !*arg2 )
	{
	    pager_printf( ch, "List of denied nicks:" NL "%s" EOL NL, sysdata.denied_nicks );
...
DEF_DO_FUN( isolate )
{
	...
	if( IS_ISOLATED( victim ) )
	{
		ch_printf(ch, "They are already isolated until %24.24s, by %s." NL,
			ctime(&victim->pcdata->release_date), victim->pcdata->isolated_by);
		return;
	}
	...
	ch_printf(ch, "%s will be unisolted at %24.24s." NL, victim->name,
		ctime(&victim->pcdata->release_date));
	act(COL_FORCE, "$n znika zabran$y za spraw± mocy.", victim, NULL, ch, TO_NOTVICT);
	char_from_room(victim);
	char_to_room(victim, get_room_index(6));
	act(COL_FORCE, "$n przylatuje tu wiezion$y przez moc.", victim, NULL, ch, TO_NOTVICT);
	do_look(victim, (char *)"auto");
	ch_printf(victim, "Mistrzowie Jedi nie s± zadowoleni z twojego postêpowania." NL
		"Zostaniesz w izolatce przez %d %s." NL,
		time, h_d ? ( time == 1 ? "godzinê" : "godzin" ) :
		( time == 1 ? "dzieñ"   : "dni"    ) );
...
DEF_DO_FUN( unisolate )
{
	...
	char_from_room(victim);
	char_to_room(victim, location);
	send_to_char( "Mistrzowie Jedi przebaczaj± Ci! Wyjdziesz z izolatki wcze¶niej." NL, victim );
	do_look(victim, (char *)"auto");
	...
	if( !*victim->pcdata->isolated_by )
	{
		if( str_cmp(ch->name, victim->pcdata->isolated_by) )
			ch_printf(ch, "(You should probably write an explanation note to %s.)" NL,
				victim->pcdata->isolated_by);
...
DEF_DO_FUN( silence )
{
	...
	if( IS_SILENCED( victim ) )
	{
		ch_printf(ch, "They are already silenced until %24.24s, by %s." NL,
			ctime(&victim->pcdata->unsilence_date), victim->pcdata->silenced_by);
		return;
	}
	...
	ch_printf(victim, "Mistrzowie Jedi nie s± zadowoleni z twoich wypowiedzi." NL
		"Tracisz prawo g³osu na %d %s." NL,
		time, h_d ? ( time == 1 ? "godzinê" : "godzin" ) :
		( time == 1 ? "dzieñ"   : "dni"    ) );
...
DEF_DO_FUN( unsilence )
{
	...
	send_to_char( "Mistrzowie Jedi przebaczaj± Ci! Pozwalaj± Ci przemówiæ." NL, victim );
...
DEF_DO_FUN( vsearch )
{
	...
	if ( in_obj->carried_by != NULL )
	  pager_printf( ch, "[%2d] Level %d %s carried by %s." NL,
		obj_counter,
...
DEF_DO_FUN( sedit )
{
	...
	{
	    send_to_char( "You cannot clear this field.  It must have a message." NL, ch );
	    return;
	}
...
DEF_DO_FUN( cedit )
{
	...
	if( !str_cmp( arg1, "create" ) )
	{
		...
		else
		{
			...
			if( (pCmd->do_fun = (DO_FUN *)skill_function( buf )) == skill_notfound )
	...
	if( !pCmd || ( (!str_prefix(pCmd->name, "admintalk")
...
void cedit( DESCRIPTOR_DATA *d, char *argument )
{
	...
	if( !str_prefix( arg1, "show" ) )
	{
		ch_printf( ch,
			...
			COL_SCORE "Flags:    " PLAIN "%s" COL_SCORE "." NL,
			pCmd->name,
			(pCmd->alias[0] != '\0') ? pCmd->alias : "none",
			pCmd->level,
			bit_name( position_types_list, pCmd->position ),
			bit_name( log_types_list, pCmd->log ),
			skill_name(pCmd->do_fun),
	...
	{
		send_to_char( "Commands:" NL "? create delete edit show save/fold raise lower list" NL
...
DEF_DO_FUN( fshow )
{
  ...
  if ( arg[0] == '\0' )
  {
    send_to_char( "Syntax:  fshow < pbug | ptypo | pidea | dmesg | qbug >" NL, ch );
    return;
  }
