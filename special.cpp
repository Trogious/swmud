...
bool spec_newbie_pilot( CHAR_DATA *ch )
{
	...
	v_next = victim->next_in_room;
	   ...
	   continue;
		...
		{
			home = 21100;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Mon-Calamari.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		if( IS_RACE( victim, "QUARREN" ) )
		{
			home = 6904;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Mon-Calamari.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		...
		if( IS_RACE( victim, "GAMORREAN" ) )
		{
			home = 28038;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Gamorr.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		if( IS_RACE( victim, "ADARIAN" ) )
		{
			home = 29006;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Adari.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		if( IS_RACE( victim, "JAWA" ) )
		{
			home = 31821;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Tatooine.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		if( IS_RACE( victim, "WOOKIEE" ) )
		{
			home = 28600;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Kashyyyk.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		if( IS_RACE( victim, "HUMAN" ) )
		{
			home = 201;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Coruscant do Portu Kosmicznego Menari.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		if( IS_RACE( victim, "NOGHRI" ) )
		{
			home = 1001;
			strcpy ( buf , COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na Honoghr do Portu Kosmicznego Nystao.\n\r\n\r" );
			echo_to_room( ch->in_room, buf );
		}
		else
		{
			sprintf ( buf , "Hmm, %s." , victim->sex == SEX_MALE ? victim->race->przypadki[0] :
                                                                               victim->race->inflectsFemale[0] );
			do_look ( ch, victim->name );
			do_say ( ch, buf );
			do_say ( ch, (char *)"Troszkê ciê¿ko bêdzie dolecieæ na twoj± ojczyst± planetê." );
			do_say ( ch, (char *)"Zabiorê ciê wiêc na Pluogusa." );
			echo_to_room( ch->in_room, COL_ACTION "Po d³ugim i wyczerpuj±cym locie docierasz na pok³ad statku 'Serin Pluogus'.\n\r\n\r" );
		}
        ...
        sprintf ( buf , COL_ACTION "%s wysiada, a kapsu³a momentalnie startuje by wrócic do akademi.\n\r" , victim->name );
        echo_to_room( ch->in_room, buf );
    }
	...
	if ( !can_see( ch, victim ) )
	   ...
	   continue;
        if ( !IS_NPC( victim ) && victim->pcdata && victim->pcdata->clan && IS_AWAKE(victim)
          ...
          {
	      do_yell( ch, (char *)"Hej Nie masz prawa by tu przebywaæ!" );
...
bool spec_customs_smut( CHAR_DATA *ch )
{
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_SMUT)
	    {
	       ...
	       {
	          sprintf( buf , "Posiadanie %s jest u nas nielegalne. Konfiskujê to w imieniu prawa.", obj->przypadki[1] );
                  do_say( ch , buf );
                  if ( obj->wear_loc != WEAR_NONE )
                     remove_obj( victim, obj->wear_loc, true );
                  separate_obj( obj );
    		  obj_from_char( obj );
    		  act( COL_ACTION, "$n konfiskuje $N$2 $p$3.", ch, obj, victim, TO_NOTVICT );
    		  act( COL_ACTION, "$n zabiera ci $p$3.",   ch, obj, victim, TO_VICT    );
    		  obj = obj_to_char( obj, ch );
                  ...
                  ch_printf( victim, "Tracisz %ld punktów do¶wiadczenia.\n\r " , ch_exp );
	       ...
	       {
                  ch_exp = UMIN( obj->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                  ch_printf( victim, "Otrzymujesz %ld punktów do¶wiadczenia za przemyt %s.\n\r" , ch_exp , obj->przypadki[1] );
                  gain_exp( victim, ch_exp , SMUGGLING_ABILITY );

	          act( COL_ACTION, "$n zerka na $N$3 podejrzliwie.", ch, NULL, victim, TO_NOTVICT );
    		  act( COL_ACTION, "$n zerka na ciebie podejrzliwie.",   ch, NULL, victim, TO_VICT  );
    		  SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
...
bool spec_customs_weapons( CHAR_DATA *ch )
{
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    ...
	    && obj->value[3] != WEAPON_LIGHTSABER )
	    {
	       if ( victim != ch && can_see( ch, victim ) && can_see_obj( ch,obj ) )
	       {
	          sprintf( buf , "Broñ jest u nas niedozwolona. Konfiskujê ten %s w imieniu prawa.", obj->przypadki[3] );
                  do_say( ch , buf );
                  if ( obj->wear_loc != WEAR_NONE )
                    remove_obj( victim, obj->wear_loc, true );
                  separate_obj( obj );
	       ...
	       else if ( can_see( ch, victim ) && !IS_SET( obj->extra_flags , ITEM_CONTRABAND)  )
	       {
	         ...
	         act( COL_ACTION, "$n zerka na $N$2 podejrzliwie.", ch, NULL, victim, TO_NOTVICT );
    		  act( COL_ACTION, "$n zerka na ciebie podejrzliwie.",   ch, NULL, victim, TO_VICT  );
    		  SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
	          return true;
	       }
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_DRINK_CON)
	    {
	     ...
	     if ( liq_table[ liquid ].liq_affect[COND_DRUNK] > 0 )
	     {
	       if ( victim != ch && can_see( ch, victim ) && can_see_obj( ch,obj ) )
	       {
	          sprintf( buf , "Posiadanie %s jest u nas nielegalne. Konfiskujê ci to w imieniu prawa", obj->przypadki[1] );
                  do_say( ch , buf );
                  if ( obj->wear_loc != WEAR_NONE )
                    remove_obj( victim, obj->wear_loc, true );
                  separate_obj( obj );
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_SPICE || obj->pIndexData->item_type  == ITEM_RAWSPICE)
	    {
	       if ( victim != ch && can_see( ch, victim ) && can_see_obj( ch,obj ) )
	       {
    		  ...
    		  obj_from_char( obj );
    		  act( COL_ACTION, "$n konfiskuje $N$2 $p$3.", ch, obj, victim, TO_NOTVICT );
    		  act( COL_ACTION, "$n zabiera ci $p$4.",   ch, obj, victim, TO_VICT    );
    		  obj = obj_to_char( obj, ch );
                  SET_BIT( obj->extra_flags , ITEM_CONTRABAND);
                  ch_exp = UMIN( obj->cost*10 , ( exp_level( victim->skill_level[SMUGGLING_ABILITY]+1) - exp_level( victim->skill_level[SMUGGLING_ABILITY])  )   );
                  ch_printf( victim, "Tracisz %ld punktów do¶wiadczenia. \n\r" , ch_exp );
...
void start_suspecting( CHAR_DATA *ch, CHAR_DATA *victim, int status, CRIME_DATA *crime )
{
    SUSPECT_DATA *	sus;
...
void police_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int amount )
{
    ...
    {
	ch_tell( mob, ch, "Co ??? Próbujesz mnie przekupiæ?" );
	ch_tell( mob, ch, "Ty MENDO !!!" );
	multi_hit( mob, ch, TYPE_UNDEFINED );
	return; /* Kasy oczywi¶cie nie oddajemy :P */
    }

    /* ale gdyby da³ siê przekupiæ, to oczywi¶cie za odpowiedni± kwotê */
    if( amount < sus->age*ch->top_level )
    {
	int cash = sus->age*ch->top_level+sus->age*ch->top_level/10;

	sprintf( buf, "Wiesz co? Daj mi %d kredyt%s i zapomnijmy o tym...",
	    cash, NUMBER_SUFF( cash, "kê", "ki", "ek" ) );
	ch_tell( mob, ch, buf );
    ...
    sprintf( buf, "No. Od razu wiedzia³%sm, ¿e siê dogadamy, %s.",
...
/*
    Hmmm Trochê dziwne, ¿e ten spec_fun nie ró¿ni siê niczym od
...
bool spec_police( CHAR_DATA *ch )
{
	...
	if( !sus->age )
	{
		...
		/* farciarz - wyszed³ z gry (to na d³u¿sz± metê nic nie da) */
		if( (victim = get_char_world(ch, sus->name)) )
		{
		    ...
		    sprintf( buf, "%s do wszystkich jednostek!" NL
				  "Cel akcji %s!" NL
			          "Uwaga! Przeciwnik mo¿e byæ uzbrojony!" NL,
			NAME(ch,0),
			sus->name );
			...
			||  !hch->in_room->area
			||  (hch->in_room->area != victim->in_room->area)
			||  (ch->pIndexData->vnum != hch->pIndexData->vnum ) )
			    continue;

			if( get_suspect( victim, hch ) )
			    continue;

			start_suspecting( hch, victim, SUS_CRITICAL, sus->crime );

			sprintf( buf,
			"Przyj%sm. Obiekt %s" COL_SAY ". Wzmamagam czujno¶æ.",
			    FEMALE( hch ) ? "ê³a" : "±³e",
			    NAME(victim,0) );
			do_say( hch, buf );
		    }
	    ...
	    {
		ch_tell( ch, victim,
		    "W imieniu prawa aresztujê ciê. Masz prawo nic nie mówiæ." );
		end_hunting( victim, ch, sus->crime );
	    }
	}
	else
	if( sus->age == 1 )
	    do_emote( ch, (char *)"wyra¼nie siê niecierpliwi." );
    }
	...
	for( planet = first_planet; planet; planet = planet->next )
	{
		...
		{
        	    sprintf( buf , "Hej %s! Poszukujemy ciê na %s! Rêce do góry!",
	    		victim->name, capitalize( planet->name ) );
		    do_say( ch , buf );
		    act( PLAIN, "$n momentalnie wyci±ga blaster i mierzy w ciebie.", ch, NULL, victim, TO_VICT );
		    act( PLAIN, "$n momentalnie wyci±ga blaster i mierzy w $N$3.",  ch, NULL, victim, TO_NOTVICT );
		    act( PLAIN, "Mierzysz w $N$3 blasterem.", 	ch, NULL, victim, TO_CHAR );
		    start_suspecting( ch, victim, SUS_NEW, crime );
            	    break;
		}
		else/* lub stary wyga co ma ju¿ ob³awê na planecie */
		if( sus->status == SUS_CRITICAL )
		{
		    if( victim->hit > 0 )
		    {
			ch_tell( ch, victim, "I na co ci to by³o?" );
			ch_tell( ch, victim, "Nie ma lito¶ci dla z³oczyñców!" );
			multi_hit( ch, victim, TYPE_UNDEFINED );
		    }
		    else/* jak ledwie zipie to do celi z nim! */
		    {
			do_say( ch, (char *)"Mam ciê ptaszku. Teraz nie uciekniesz..." );
			end_hunting( victim, ch, sus->crime );
		    }
		}
	    }
...
bool spec_police_attack( CHAR_DATA *ch )
{
	...
	for( planet = first_planet; planet; planet = planet->next )
	{
	    ...
	    &&( crime = find_crime( victim, planet ) ) != NULL )
            {
		sprintf( buf , "Hej! Poszukujemy ciê na %s! O ¯ESZ TY !!!",
...
bool spec_police_fine( CHAR_DATA *ch )
{
	...
	for (victim = ch->in_room->first_person; victim; victim = victim->next_in_room)
	{
		...
		sprintf(buf,"Hej Ty!!! Widzia³%sm Ciê na jednym z listów goñczych!", SEX_SUFFIX_EAE(ch));
		do_say(ch, buf);
		sprintf(buf,"Je¶li chcesz, to mogê, za drobn± op³at±, sprawiæ, ¿e ¶wiat"
						" zapomni o Twojej kryminalnej przesz³o¶ci...");
		do_say(ch, buf);
		sprintf(buf,"Wymazanie Twoich przestêpstw bêdzie kosztowa³o %d kredytek." NL,
...
bool spec_police_jail( CHAR_DATA *ch )
{
	...
	for( planet = first_planet; planet; planet = planet->next )
	{
	    ...
	    &&( crime = find_crime( victim, planet ) ) != NULL )
            {
        	sprintf( buf , "Hej! Poszukujemy ciê na %s!",
	    	    capitalize( planet->name ) );
		do_say( ch , buf );
...
bool spec_jedi_healer( CHAR_DATA *ch )
{
    ...
    {
    case 0:
    act( COL_FORCE, "$n zatrzymuje siê i koncentruje na moment.", ch, NULL, NULL, TO_ROOM );
...
bool spec_fido( CHAR_DATA *ch )
{
    ...
    act( COL_ACTION, "$n ³apczywie rozszarpuje cia³o.", ch, NULL, NULL, TO_ROOM );
...
bool spec_stormtrooper( CHAR_DATA *ch )
{
	...
	if ( !can_see( ch, victim ) )
	   continue;
        if ( get_timer(victim, TIMER_RECENTFIGHT) > 0 )
	   continue;
        if ( ( IS_NPC( victim ) && nifty_is_name( (char *)"republic" , victim->name )
               && victim->fighting && who_fighting( victim ) != ch ) ||
             ( !IS_NPC( victim ) && victim->pcdata && victim->pcdata->clan && IS_AWAKE(victim)
               && nifty_is_name( (char *)"Nowa Republika" , victim->pcdata->clan->name ) ) )
          {
	      do_yell( ch, (char *)"Giñ Rebeliancka Gnido!" );
              multi_hit( ch, victim, TYPE_UNDEFINED );
              return true;
          }
...
bool spec_new_republic_trooper( CHAR_DATA *ch )
{
	...
	v_next = victim->next_in_room;
	   ...
	   continue;
        if ( get_timer(victim, TIMER_RECENTFIGHT) > 0 )
	   continue;
        if ( ( IS_NPC( victim ) && nifty_is_name( (char *)"imperial" , victim->name )
               && victim->fighting && who_fighting( victim ) != ch ) ||
             ( !IS_NPC( victim ) && victim->pcdata && victim->pcdata->clan && IS_AWAKE(victim)
             && nifty_is_name( (char *)"Imperium" , victim->pcdata->clan->name ) ) )
          {
	      do_yell( ch, (char *)"W imiê Nowej Republiki!" );
              multi_hit( ch, victim, TYPE_UNDEFINED );
              return true;
          }
...
bool spec_guardian( CHAR_DATA *ch )
{
    ...
    {
	sprintf( buf, "%s to zasrany %s! No i oczywi¶cie TCHÓRZ!",
		victim->name, crime );
	do_yell( ch, buf );
	return true;
    }
    ...
    if ( victim )
    {
	sprintf( buf, "%s to zasrany %s! CHRONIÆ NIEWINNYCH !!!",
		victim->name, crime );
	do_shout( ch, buf );
	multi_hit( ch, victim, TYPE_UNDEFINED );
	return true;
    ...
    if ( ech )
    {
    act( COL_YELL, "$n krzyczy 'CHRONIÆ NIEWINNYCH !!!'",
...
bool spec_janitor( CHAR_DATA *ch )
{
	...
	||   trash->inquest )/* <-- bug fix by Thanos */
	    continue;
	if( trash->item_type == ITEM_DRINK_CON
	||   trash->item_type == ITEM_TRASH
	||   trash->cost < 10 )
	{
	    act( COL_ACTION, "$n podnosi z ziemi jakie¶ ¶mieci.", ch, NULL, NULL, TO_ROOM );
	    obj_from_room( trash );
...
bool spec_poison( CHAR_DATA *ch )
{
    ...
    act( COL_POISON, "$n gryzie ciê!",  ch, NULL, victim, TO_VICT    );
...
bool spec_thief( CHAR_DATA *ch )
{
	...
	{
	    act( COL_ACTION, "Odkrywasz rêkê $n$1 w swojej kieszeni!",
		ch, NULL, victim, TO_VICT );
	    act( COL_ACTION, "$N odkrywa rêkê $n$1 w swojej kieszeni!",
...
bool spec_auth( CHAR_DATA *ch )
{
	...
	for ( pban = first_ban; pban; pban = pban->next )
	{
	    ...
	    {
		ban = true;
		ch_tell( ch, victim, "O nie. Nie licz na autoryzacjê!" );
	...
	if( ban )
        ...
        if ( !IS_NPC(victim)
	&& ( pObjIndex = get_obj_index( OBJ_VNUM_SCHOOL_DIPLOMA ) ) != NULL )
        {
             hasdiploma = false;
             ...
             if ( !hasdiploma )
             {
               ...
               act( FG_CYAN, "$n wrêcza ci $p$3 i ¶ciska twoj± rêkê.",
	    	    ch, obj, victim, TO_VICT );
               act( FG_CYAN, "$n wrêcza $N$2 $p$3 i ¶ciska $G rêkê.",
	...
	||   !IS_SET(victim->pcdata->flags, PCFLAG_UNAUTHED) )
	    continue;
            ...
            sprintf( buf, "%s (#%d) authorized %s",
			 ch->przypadki[0], ch->pIndexData->vnum,
                         victim->przypadki[0] );
...
bool spec_prosecutor( CHAR_DATA *ch )
{
	...
	if( gest <= 10 )
	{
            act( PLAIN, "$n klnie jak szewc. To nie jest $s dzieñ.",
		ch, NULL, NULL, TO_ROOM );
	}
	else
	if( gest == 75 )
	{
	    ...
	    sprintf( buf, "Tu %s. Macie jeszcze tego Kalamariañskiego przemytnika?",
		ch->przypadki[0] );
	    do_say( ch, buf );
	    do_say( ch, (char *)"Ahh, stracony.. Có¿. Bywa i tak."  );
	}
