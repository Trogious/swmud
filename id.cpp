...
void start_auth(struct descriptor_data *d)
{
	...
	{
		if (d->ipv6)
			sock.sock6.sin6_port = htons(113);
		else
			sock.sock.sin_port = htons(113);
	}
	else
	{
		if (d->ipv6)
			sock.sock6.sin6_port = serv->s_port;
		else
			sock.sock.sin_port = serv->s_port;
	}
	sock.sock.sin_family = d->ipv6 ? AF_INET6 : AF_INET;

	if (connect(d->auth_fd, (struct sockaddr*) &sock, d->ipv6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in)) == -1
			&& errno != EINPROGRESS)
	{
		/* Identd Denied */
...
void send_auth(struct descriptor_data *d)
{
	...
	if (getsockname(d->descriptor, (struct sockaddr*) &us, &ulen) || getpeername(d->descriptor, (struct sockaddr*) &them, &tlen))
	{
	...
	/* compose request */
	if (d->ipv6)
		sprintf(authbuf, "%u , %u\r\n", (unsigned int) ntohs(them.sock6.sin6_port), (unsigned int) ntohs(us.sock6.sin6_port));
	else
		sprintf(authbuf, "%u , %u\r\n", (unsigned int) ntohs(them.sock.sin_port), (unsigned int) ntohs(us.sock.sin_port));
...
void read_auth(struct descriptor_data *d)
{
	char *s, *t;
	int len; /* length read */
	const size_t system_size = 8;
	char ruser[20], system[system_size + 1]; /* remote userid */
	...
	*system = *ruser = '\0';
	...
	if ((len = read(d->auth_fd, d->abuf + d->auth_inc, sizeof(d->abuf) - 1 - d->auth_inc)) >= 0)
	{
		d->auth_inc += len;
		d->abuf[d->auth_inc] = '\0';
	}
	...
	if ((len > 0) && (d->auth_inc != (sizeof(d->abuf) - 1))
			&& (sscanf(d->abuf, "%hd , %hd : USERID : %*[^:]: %10s", &remp, &locp, ruser) == 3))
	{
		s = rindex(d->abuf, ':');
		*s++ = '\0';
		...
		*t = '\0';
		...
		sprintf(log_buf, "auth reply ok, incoming user: %s@%s", ruser, d->host);
